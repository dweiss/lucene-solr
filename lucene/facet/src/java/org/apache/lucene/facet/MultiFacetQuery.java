 ...
 *
 * <p><b>NOTE:</b>This helper class is an alternative to {@link DrillDownQuery} especially in cases
 * where you don't intend to use {@link DrillSideways}
 *
 * @lucene.experimental
...
public class MultiFacetQuery extends TermInSetQuery {
  ...
  public MultiFacetQuery(
      final FacetsConfig facetsConfig, final String dimension, final String[]... paths) {
   ...
   * Creates a new {@code MultiFacetQuery} filtering the query on the given dimension.
   *
   * <p><b>NOTE:</b>Uses FacetsConfig.DEFAULT_DIM_CONFIG.
   */
  ...
  static Collection<BytesRef> toTerms(final String dimension, final String[]... paths) {
    ...
    for (String[] path : paths) terms.add(new BytesRef(FacetsConfig.pathToString(dimension, path)));
    return terms;
  }
}
