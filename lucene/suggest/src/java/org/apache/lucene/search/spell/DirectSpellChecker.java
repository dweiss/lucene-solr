 ...
 *
 * <p>Candidates are presented directly from the term dictionary, based on Levenshtein distance.
 * This is an alternative to {@link SpellChecker} if you are using an edit-distance-like metric such
 * as Levenshtein or {@link JaroWinklerDistance}.
 *
 * <p>A practical benefit of this spellchecker is that it requires no additional datastructures
 * (neither in RAM nor on disk) to do its work.
 *
...
public class DirectSpellChecker {
  /**
   * The default StringDistance, Damerau-Levenshtein distance implemented internally via {@link
   * LevenshteinAutomata}.
   *
   * <p>Note: this is the fastest distance metric, because Damerau-Levenshtein is used to draw
   * candidates from the term dictionary: this just re-uses the scoring.
   */
  ...
  /**
   * value in [0..1] (or absolute number &gt;= 1) representing the minimum number of documents (of
   * the total) where a term should appear.
   */
  ...
  /**
   * value in [0..1] (or absolute number &gt;= 1) representing the maximum number of documents (of
   * the total) a query term can appear in to be corrected.
   */
  ...
  /**
   * Sets the maximum number of Levenshtein edit-distances to draw candidate terms from. This value
   * can be 1 or 2. The default is 2.
   *
   * <p>Note: a large number of spelling errors occur with an edit distance of 1, by setting this
   * value to 1 you can increase both performance and precision at the cost of recall.
   */
   ...
   *
   * <p>This can improve both performance and accuracy of results, as misspellings are commonly not
   * the first character.
   */
   ...
   *
   * <p>Increasing this number can improve the accuracy of results, at the cost of performance.
   */
   ...
   *
   * <p>Very short query terms will often cause only bad suggestions with any distance metric.
   */
   ...
   *
   * <p>Long queries can be expensive to process and/or trigger exceptions.
   */
   ...
   *
   * <p>This is a convenience method, if your index field has more complicated analysis (such as
   * StandardTokenizer removing punctuation), it's probably better to turn this off, and instead run
   * your query terms through your Analyzer first.
   *
   * <p>If this option is not on, case differences count as an edit!
   */
  ...
  /**
   * Set the comparator for sorting suggestions. The default is {@link
  ...
  /**
   * Set the string distance metric. The default is {@link #INTERNAL_LEVENSHTEIN}
   *
   * <p>Note: because this spellchecker draws its candidates from the term dictionary using
   * Damerau-Levenshtein, it works best with an edit-distance-like string metric. If you use a
   * different metric than the default, you might want to consider increasing {@link
  ...
  /**
   * Calls {@link #suggestSimilar(Term, int, IndexReader, SuggestMode) suggestSimilar(term, numSug,
  ...
  /**
   * Calls {@link #suggestSimilar(Term, int, IndexReader, SuggestMode, float) suggestSimilar(term,
   * numSug, ir, suggestMode, this.accuracy)}
   */
  public SuggestWord[] suggestSimilar(
      Term term, int numSug, IndexReader ir, SuggestMode suggestMode) throws IOException {
  ...
  public SuggestWord[] suggestSimilar(
    ...
    if (textLength < minQueryLength || textLength > maxQueryLength) return new SuggestWord[0];
   ...
   */
  protected Collection<ScoreTerm> suggestSimilar(
      Term term,
      int numSug,
      IndexReader ir,
      int docfreq,
    ...
    while ((candidateTerm = e.next()) != null) {
      ...
      e.setMaxNonCompetitiveBoost(
          (stQueue.size() >= numSug) ? stQueue.peek().boost : Float.NEGATIVE_INFINITY);
    }
  ...
  protected static class ScoreTerm implements Comparable<ScoreTerm> {
    ...
    public int compareTo(ScoreTerm other) {
      if (term.bytesEquals(other.term)) return 0; // consistent with equals
