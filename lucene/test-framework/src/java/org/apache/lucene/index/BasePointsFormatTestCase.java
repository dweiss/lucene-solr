    ...
    for(int ord=0;ord<docValues.length;ord++) {
      for(int dim=0;dim<numDims;dim++) {
        if (ord == 0) {
          ...
          if (FutureArrays.compareUnsigned(docValues[ord][dim], 0, 0 + numBytesPerDim, expectedMinValues[dim], 0, 0 + numBytesPerDim) < 0) {
            System.arraycopy(docValues[ord][dim], 0, expectedMinValues[dim], 0, numBytesPerDim);
          }
          if (FutureArrays.compareUnsigned(docValues[ord][dim], 0, 0 + numBytesPerDim, expectedMaxValues[dim], 0, 0 + numBytesPerDim) > 0) {
            System.arraycopy(docValues[ord][dim], 0, expectedMaxValues[dim], 0, numBytesPerDim);
          }
        }
      }
    ...
    try {
      ...
      for(int iter=0;iter<iters;iter++) {
        ...
        for(int dim=0;dim<numDims;dim++) {    
          ...
          if (FutureArrays.compareUnsigned(queryMin[dim], 0, 0 + numBytesPerDim, queryMax[dim], 0, 0 + numBytesPerDim) > 0) {
            byte[] x = queryMin[dim];
            queryMin[dim] = queryMax[dim];
            queryMax[dim] = x;
          }
          ...
          dimValues.intersect(new PointValues.IntersectVisitor() {
              ...
              public void visit(int docID, byte[] packedValue) {
                ...
                for(int dim=0;dim<numDims;dim++) {
                  ...
                  if (FutureArrays.compareUnsigned(packedValue, dim * numBytesPerDim, dim * numBytesPerDim + numBytesPerDim, queryMin[dim], 0, 0 + numBytesPerDim) < 0 ||
                      FutureArrays.compareUnsigned(packedValue, dim * numBytesPerDim, dim * numBytesPerDim + numBytesPerDim, queryMax[dim], 0, 0 + numBytesPerDim) > 0) {
                    //System.out.println("  no");
                    return;
                  }
                }
              ...
              public Relation compare(byte[] minPacked, byte[] maxPacked) {
                ...
                for(int dim=0;dim<numDims;dim++) {
                  if (FutureArrays.compareUnsigned(maxPacked, dim * numBytesPerDim, dim * numBytesPerDim + numBytesPerDim, queryMin[dim], 0, 0 + numBytesPerDim) < 0 ||
                      FutureArrays.compareUnsigned(minPacked, dim * numBytesPerDim, dim * numBytesPerDim + numBytesPerDim, queryMax[dim], 0, 0 + numBytesPerDim) > 0) {
                    //System.out.println("  query_outside_cell");
                    return Relation.CELL_OUTSIDE_QUERY;
                  } else if (FutureArrays.compareUnsigned(minPacked, dim * numBytesPerDim, dim * numBytesPerDim + numBytesPerDim, queryMin[dim], 0, 0 + numBytesPerDim) < 0 ||
                             FutureArrays.compareUnsigned(maxPacked, dim * numBytesPerDim, dim * numBytesPerDim + numBytesPerDim, queryMax[dim], 0, 0 + numBytesPerDim) > 0) {
                    crosses = true;
                  }
                }
        ...
        for(int ord=0;ord<numValues;ord++) {
          ...
          for(int dim=0;dim<numDims;dim++) {
            ...
            if (FutureArrays.compareUnsigned(x, 0, 0 + numBytesPerDim, queryMin[dim], 0, 0 + numBytesPerDim) < 0 ||
                FutureArrays.compareUnsigned(x, 0, 0 + numBytesPerDim, queryMax[dim], 0, 0 + numBytesPerDim) > 0) {
              matches = false;
              break;
            }
          }
