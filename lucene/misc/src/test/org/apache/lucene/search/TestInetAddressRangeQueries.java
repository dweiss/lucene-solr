...
public class TestInetAddressRangeQueries extends BaseRangeFieldQueryTestCase {
  ...
  protected Range nextRange(int dimensions) throws Exception {
    ...
    if (FutureArrays.compareUnsigned(bMin, 0, 0 + bMin.length, bMax, 0, 0 + bMin.length) > 0) {
  ...
  private class IpRange extends Range {
    ...
    protected void setMin(int dim, Object val) {
      ...
      if (FutureArrays.compareUnsigned(min, 0, 0 + e.length, e, 0, 0 + e.length) < 0) {
        max = e;
        maxAddress = v;
      } else {
        min = e;
    ...
    protected void setMax(int dim, Object val) {
      ...
      if (FutureArrays.compareUnsigned(max, 0, 0 + e.length, e, 0, 0 + e.length) > 0) {
        min = e;
        minAddress = v;
      } else {
        max = e;
    ...
    protected boolean isDisjoint(Range o) {
      IpRange other = (IpRange)o;
      return FutureArrays.compareUnsigned(min, 0, 0 + min.length, other.max, 0, 0 + min.length) > 0 ||
          FutureArrays.compareUnsigned(max, 0, 0 + max.length, other.min, 0, 0 + max.length) < 0;
    }

    @Override
    protected boolean isWithin(Range o) {
      IpRange other = (IpRange)o;
      return FutureArrays.compareUnsigned(min, 0, 0 + min.length, other.min, 0, 0 + min.length) >= 0 &&
          FutureArrays.compareUnsigned(max, 0, 0 + max.length, other.max, 0, 0 + max.length) <= 0;
    }
