...
/**
 * {@link PostingsFormat} based on the Uniform Split technique and supporting Shared Terms.
 *
 * <p>Shared Terms means the terms of all fields are stored in the same block file, with multiple
 * fields associated to one term (one block line). In the same way, the dictionary trie is also
 * shared between all fields. This highly reduces the memory required by the field dictionary
 * compared to having one separate dictionary per field.
 *
 * @lucene.experimental
 */
public class STUniformSplitPostingsFormat extends UniformSplitPostingsFormat {
  ...
  /**
   * @see UniformSplitPostingsFormat#UniformSplitPostingsFormat(int, int, BlockEncoder,
   *     BlockDecoder, boolean)
   */
  public STUniformSplitPostingsFormat(
  ...
  protected STUniformSplitPostingsFormat(
      String name,
      int targetNumBlockLines,
      int deltaNumLines,
      BlockEncoder blockEncoder,
