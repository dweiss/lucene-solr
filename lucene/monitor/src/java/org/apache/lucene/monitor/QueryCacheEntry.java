...
class QueryCacheEntry {
   ...
   *
   * <p>Note that this may be different to {@link #cacheId} due to decomposition
   */
  ...
  private QueryCacheEntry(
      String cacheId, String queryId, Query matchQuery, Map<String, String> metadata) {
  ...
  static List<QueryCacheEntry> decompose(MonitorQuery mq, QueryDecomposer decomposer) {
    ...
    for (Query subquery : decomposer.decompose(mq.getQuery())) {
      cacheEntries.add(
          new QueryCacheEntry(mq.getId() + "_" + upto, mq.getId(), subquery, mq.getMetadata()));
      upto++;
    }
    return cacheEntries;
  }
