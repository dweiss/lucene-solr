...
public class LatLonBounds implements Bounds {
  ...
  // choice of longitude bounds.  But even with this, we still can't always choose whether the
  // actual shape
  // goes right or left.
  //
  ...
  // plane
  // solution would in general seem to boil down to the same logic that relies on points along the
  // path
  // to define the shape boundaries.  I guess the one thing that you do know for a bounded edge is
  // that
  // the endpoints are actually connected.  But it is not clear whether relationship helps in any
  // way.
  //
  // In any case, if we specify shapes by a sequence of planes, we should stipulate that multiple
  // sequences
  // are allowed, provided they progressively tile an area of the sphere that is connected and
  // sequential.
  // For example, paths do alternating rectangles and circles, in sequence.  Each sequence member is
  // described by a sequence of planes.  I think it would also be reasonable to insist that the
  // first segment
  ...
  // bounds
  // in the direction of the traversal.  So if the traversal is always known to be less than PI in
  // total longitude
  // angle, then it is possible to use the endpoints to determine the unambiguous extension of the
  // envelope.
  // For example, say you are currently at longitude -0.5.  The next point is at longitude PI-0.1.
  // You could say
  // that the difference in longitude going one way around would be beter than the distance the
  // other way
  // around, and therefore the longitude envelope should be extended accordingly.  But in practice,
  // when an
  // edge goes near a pole and may be inclined as well, the longer longitude change might be the
  // right path, even
  ...
  // a
  // heuristic, based on the idea that most areas are small compared to the circumference of the
  // globe.
  // We keep track of the last point we saw, and take each point as it arrives, and compute its
  // longitude.
  // Then, we have a choice as to which way to expand the envelope: we can expand by going to the
  // left or
  // to the right.  We choose the direction with the least longitude difference.  (If we aren't
  // sure,
