...
class GeoComplexPolygon extends GeoBasePolygon {
  ...
  private boolean isInSet(
    ...
    // System.out.println("\nIsInSet called for ["+x+","+y+","+z+"], testPoint="+testPoint+"; is in
    // set? "+testPointInSet);
  ...
  private static class Edge {
    ...
    public Edge(final PlanetModel pm, final GeoPoint startPoint, final GeoPoint endPoint) {
      ...
      // System.out.println("Recording edge ["+startPoint+" --> "+endPoint+"]; bounds =
      // "+planeBounds);
    }
  ...
  private class TraversalStrategy implements Comparable<TraversalStrategy> {
    ...
    public boolean apply(
      ...
      try {
        ...
        // System.out.println("  Intersection point in-set? "+intersectionPointInSet+" On edge?
        // "+intersectionPointOnEdge);

        // Now do the final leg
        // System.out.println(" Finding whether ["+x+","+y+","+z+"] is in-set, based on travel from
        // "+intersectionPoint+" along "+secondLegPlane+" (value="+secondLegValue+")");
  ...
  */
  ...
  private class FullLinearCrossingEdgeIterator implements CountingEdgeIterator {
    ...
    @Override
    public boolean matches(final Edge edge) {
      // System.out.println(" Edge ["+edge.startPoint+" --> "+edge.endPoint+"] potentially crosses
      // travel plane "+plane);
    ...
    private boolean edgeCrossesEnvelope(
      ...
      for (final GeoPoint adjoining : adjoiningPoints) {
        ...
        if (plane.evaluateIsZero(adjoining)
          ...
          // bound1.isWithin? "+bound1.isWithin(adjoining)+" bound2.isWithin?
          // "+bound2.isWithin(adjoining));
        }
      }
      return (withinCount & 1) != 0;
            ...
            if (testPointIntersections != null) {
              for (final GeoPoint otherIntersection : testPointIntersections) {
                if (edge.startPlane.strictlyWithin(otherIntersection)
                  ...
                  // System.out.println("  Points "+intersection+" and "+otherIntersection+" are
                  // duplicates");
                  notDup = false;
                  break;
                }
  ...
  private GeoPoint[] findAdjoiningPoints(
    ...
    // System.out.println("     Adjoining points not found.  Are planes parallel?  edge = "+plane+";
    // envelope = "+envelopePlane+"; perpendicular = "+perpendicular);
    return null;
  }
