...
public class PlanetModel implements SerializableObject {
  ...
  public double decodeValue(int x) {
    ...
    if (x == MIN_ENCODED_VALUE) {
      ...
      // floor value, and we don't ever want to
      // return a value outside of the planet's range (I think?).  The max value is "safe" because
      // we floor during encode:
      result = -MAX_VALUE;
  ...
  public double surfaceDistance(final GeoPoint pt1, final GeoPoint pt2) {
    ...
    do {
      ...
      if (Double.isNaN(cos2SigmaM)) cos2SigmaM = 0.0; // equatorial line: cosSqAlpha=0
      C =
          scaledFlattening
              / 16.0
              * cosSqAlpha
  ...
  public static class DocValueEncoder {
    ...
    // bits.
    // The formula we use to go from double to encoded value is:  Math.floor((value - minimum) *
    // factor + 0.5)
    ...
    // values -- that is, these are in the center of the range of input values that should return
    // the minimum or maximum when decoded
