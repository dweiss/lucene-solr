...
class GeoConvexPolygon extends GeoBasePolygon {
  ...
  public void addPoint(
    ...
    if (isInternalEdge) isInternalEdges.set(points.size() - 1);
  ...
  protected boolean localIsWithin(final double x, final double y, final double z) {
    for (final SidedPlane edge : edges) {
      if (!edge.isWithin(x, y, z)) return false;
    }
    return true;
  }
  ...
  public boolean equals(Object o) {
    if (!(o instanceof GeoConvexPolygon)) return false;
    final GeoConvexPolygon other = (GeoConvexPolygon) o;
    if (!super.equals(other)) return false;
    if (!other.isInternalEdges.equals(isInternalEdges)) return false;
