 ...
 * The left-right maximum extent for this shape is PI; for anything larger, use {@link
...
class GeoNorthRectangle extends GeoBaseBBox {
  ...
  public GeoNorthRectangle(
    ...
    if (extent > Math.PI) throw new IllegalArgumentException("Width of rectangle too great");
   ...
   */
  public GeoNorthRectangle(final PlanetModel planetModel, final InputStream inputStream)
      throws IOException {
    this(
        planetModel,
  ...
  public double getRadius() {
    ...
    // to be careful
    // to use the longest of three distances: the distance to a corner on the top; the distnace to a
    // corner on the bottom, and
  ...
  public boolean intersects(
    ...
    return p.intersects(
            ...
            rightPlane)
        || p.intersects(
            planetModel, leftPlane, notablePoints, leftPlanePoints, bounds, rightPlane, bottomPlane)
        || p.intersects(
            planetModel,
            rightPlane,
            notablePoints,
  ...
  public boolean intersects(final GeoShape geoShape) {
    return geoShape.intersects(bottomPlane, bottomPlanePoints, leftPlane, rightPlane)
  ...
  protected double outsideDistance(
    ...
    final double bottomDistance =
        distanceStyle.computeDistance(planetModel, bottomPlane, x, y, z, leftPlane, rightPlane);
    final double leftDistance =
        distanceStyle.computeDistance(planetModel, leftPlane, x, y, z, rightPlane, bottomPlane);
    final double rightDistance =
        distanceStyle.computeDistance(planetModel, rightPlane, x, y, z, leftPlane, bottomPlane);
    ...
    return Math.min(
        bottomDistance,
        Math.min(Math.min(leftDistance, rightDistance), Math.min(LRHCDistance, LLHCDistance)));
  }

  @Override
  public boolean equals(Object o) {
    if (!(o instanceof GeoNorthRectangle)) return false;
  ...
  public String toString() {
    return "GeoNorthRectangle: {planetmodel="
        + planetModel
        + ", bottomlat="
        + bottomLat
        + "("
