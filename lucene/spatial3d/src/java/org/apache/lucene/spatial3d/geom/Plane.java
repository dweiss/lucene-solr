...
public class Plane extends Vector {
  ...
  public double normalDistanceSquared(
    ...
    if (normal == Double.POSITIVE_INFINITY) return normal;
  ...
  protected GeoPoint[] findIntersections(
    ...
    // solves both plane equations
    // simultaneous.  For example, let's say that the vector is (0.5,0.5,1), and the two plane
    // equations are:
  ...
  */
  ...
  public void recordBounds(
    ...
    if (!boundsInfo.isSmallestMinZ(planetModel) || !boundsInfo.isLargestMaxZ(planetModel)) {
      ...
      // orientation, and
      // then intersect it with this one and with the ellipsoid.  This gives us zero, one, or two
      // points to use
      // as bounds.
  ...
  private static void addPoint(
    ...
    for (Membership bound : bounds) {
      if (!bound.isWithin(point)) return;
    }
    // Add the point
  ...
  private static boolean meetsAllBounds(
      final double x, final double y, final double z, final Membership[] bounds) {
    for (final Membership bound : bounds) {
      if (!bound.isWithin(x, y, z)) return false;
    }
    return true;
  }
