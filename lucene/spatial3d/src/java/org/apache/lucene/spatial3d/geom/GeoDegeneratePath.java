...
class GeoDegeneratePath extends GeoBasePath {
  ...
  public void addPoint(final double lat, final double lon) {
    if (isDone) throw new IllegalStateException("Can't call addPoint() if done() already called");
  ...
  public boolean equals(Object o) {
    if (!(o instanceof GeoDegeneratePath)) return false;
   ...
   * several different cases: (1) The path consists of a single endpoint. In this case, the
   * degenerate path consists of this one point. (2) This is the end of a path. There is a bounding
   * plane passed in which describes the part of the world that is considered to belong to this
   * endpoint. (3) Intersection. There are two cutoff planes, one for each end of the intersection.
   */
  private static class SegmentEndpoint {
    ...
    public boolean intersects(
      ...
      if (!p.evaluateIsZero(point)) return false;

      for (Membership m : bounds) {
        if (!m.isWithin(point)) return false;
      }
      return true;
    }
    ...
    @Override
    public boolean equals(Object o) {
      if (!(o instanceof SegmentEndpoint)) return false;
  ...
  private static class PathSegment {
    ...
    public final Map<DistanceStyle, Double> fullDistanceCache =
        new HashMap<DistanceStyle, Double>();
    ...
    public double pathCenterDistance(
      ...
      // that the point given
      // is insufficient to distinguish between a family of such planes.  This can happen only if
      // the point is one of the
      // "poles", imagining the normalized plane to be the "equator".  In that case, the distance
      ...
      if (intersectionPoints.length == 0)
        ...
        if (startCutoffPlane.isWithin(intersectionPoints[0])
            && endCutoffPlane.isWithin(intersectionPoints[0])) thePoint = intersectionPoints[0];
        else if (startCutoffPlane.isWithin(intersectionPoints[1])
            && endCutoffPlane.isWithin(intersectionPoints[1])) thePoint = intersectionPoints[1];
        else
          throw new RuntimeException(
              "Can't find world intersection for point x=" + x + " y=" + y + " z=" + z);
      }
