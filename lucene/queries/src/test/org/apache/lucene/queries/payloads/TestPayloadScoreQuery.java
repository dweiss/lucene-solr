...
public class TestPayloadScoreQuery extends LuceneTestCase {
  ...
  private static void checkQuery(
      SpanQuery query, PayloadFunction function, int[] expectedDocs, float[] expectedScores)
      throws IOException {
  ...
  private static void checkQuery(
    ...
    assertTrue(
        "Expected docs and scores arrays must be the same length!",
        expectedDocs.length == expectedScores.length);

    PayloadScoreQuery psq =
        new PayloadScoreQuery(query, function, PayloadDecoder.FLOAT_DECODER, includeSpanScore);
    ...
    for (int i = 0; i < hits.scoreDocs.length; i++) {
      ...
      assertEquals(
          "Bad score in document " + expectedDocs[i],
          expectedScores[i],
  ...
  public void testTermQuery() throws IOException {
    ...
    for (PayloadFunction fn :
      ...
      checkQuery(q, fn, new int[] {118, 218, 18}, new float[] {4.0f, 4.0f, 2.0f});
    }
  }
  ...
  public void testNearQuery() throws IOException {
    ...
    checkQuery(
        q, new MaxPayloadFunction(), new int[] {22, 122, 222}, new float[] {4.0f, 4.0f, 4.0f});
    checkQuery(
        q, new MinPayloadFunction(), new int[] {122, 222, 22}, new float[] {4.0f, 4.0f, 2.0f});
    checkQuery(
  ...
  public void testEquality() {
    ...
    PayloadScoreQuery query1 =
        new PayloadScoreQuery(sq1, minFunc, PayloadDecoder.FLOAT_DECODER, true);
    PayloadScoreQuery query2 =
        new PayloadScoreQuery(sq2, minFunc, PayloadDecoder.FLOAT_DECODER, true);
    PayloadScoreQuery query3 =
        new PayloadScoreQuery(sq2, maxFunc, PayloadDecoder.FLOAT_DECODER, true);
    PayloadScoreQuery query4 =
        new PayloadScoreQuery(sq2, maxFunc, PayloadDecoder.FLOAT_DECODER, false);
  ...
  public static void beforeClass() throws Exception {
    ...
    RandomIndexWriter writer =
        new RandomIndexWriter(
            random(),
            directory,
            newIndexWriterConfig(new PayloadAnalyzer()).setMergePolicy(NoMergePolicy.INSTANCE));
