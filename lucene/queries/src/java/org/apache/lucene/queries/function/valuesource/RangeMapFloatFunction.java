...
/**
 * <code>RangeMapFloatFunction</code> implements a map function over another {@link ValueSource}
 * whose values fall within min and max inclusive to target. <br>
...
public class RangeMapFloatFunction extends ValueSource {
  ...
  public FunctionValues getValues(Map<Object, Object> context, LeafReaderContext readerContext)
    ...
    final FunctionValues defaults =
        (this.defaultVal == null) ? null : defaultVal.getValues(context, readerContext);
    return new FloatDocValues(this) {
      ...
      public float floatVal(int doc) throws IOException {
        ...
        return (val >= min && val <= max)
            ...
            : (defaultVal == null ? val : defaults.floatVal(doc));
      }
      ...
      public String toString(int doc) throws IOException {
        return "map("
            ...
            + max
            + ",target="
            + targets.toString(doc)
            + ",defaultVal="
            + (defaults == null ? "null" : (defaults.toString(doc)))
            + ")";
      }
    };
  }
