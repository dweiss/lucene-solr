...
final class WANDScorer extends Scorer {
  ...
  /**
   * Return a scaling factor for the given float so that f x 2^scalingFactor would be in ]2^15,
   * 2^16]. Special cases: scalingFactor(0) = scalingFactor(MIN_VALUE) - 1 scalingFactor(+Infty) =
   * scalingFactor(MAX_VALUE) + 1
   */
  ...
  private static long scaleMinScore(float minScore, int scalingFactor) {
    ...
    return (long)
        Math.floor(
            scaled); // round down, cast might lower the value again if scaled > Long.MAX_VALUE,
    // which is fine
  }
