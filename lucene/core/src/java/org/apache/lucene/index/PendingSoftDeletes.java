...
final class PendingSoftDeletes extends PendingDeletes {
  ...
  boolean delete(int docID) throws IOException {
    FixedBitSet mutableBits =
        getMutableBits(); // we need to fetch this first it might be a shared instance with
    // hardDeletes
  ...
  void onNewReader(CodecReader reader, SegmentCommitInfo info) throws IOException {
    ...
    if (dvGeneration
        < info.getDocValuesGen()) { // only re-calculate this if we haven't seen this generation
