 ...
 *
 * <p>This class attempts to detect whether the index is on rotational storage (traditional hard
 * drive) or not (e.g. solid-state disk) and changes the default max merge and thread count
 * accordingly. This detection is currently Linux-only, and relies on the OS to put the right value
 * into /sys/block/&lt;dev&gt;/block/rotational. For all other operating systems it currently
 * assumes a rotational disk for backwards compatibility. To enable default settings for spinning or
 * solid state disks for such operating systems, use {@link
 * #setDefaultMaxMergesAndThreads(boolean)}.
 */
public class ConcurrentMergeScheduler extends MergeScheduler {

  /**
   * Dynamic default for {@code maxThreadCount} and {@code maxMergeCount}, used to detect whether
   * the index is backed by an SSD or rotational disk and set {@code maxThreadCount} accordingly. If
   * it's an SSD, {@code maxThreadCount} is set to {@code max(1, min(4, cpuCoreCount/2))}, otherwise
   * 1. Note that detection only currently works on Linux; other platforms will assume the index is
   * not on an SSD.
   */
