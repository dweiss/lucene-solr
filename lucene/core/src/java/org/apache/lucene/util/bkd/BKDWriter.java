...
public class BKDWriter implements Closeable {
  ...
  private static class BKDMergeQueue extends PriorityQueue<MergeReader> {
    ...
    public boolean lessThan(MergeReader a, MergeReader b) {
      ...
      int cmp = FutureArrays.compareUnsigned(a.state.scratchPackedValue, 0, 0 + bytesPerDim, b.state.scratchPackedValue, 0, 0 + bytesPerDim);
      if (cmp < 0) {
        return true;
      } else if (cmp > 0) {
        return false;
  ...
  */
  ...
  protected int split(byte[] minPackedValue, byte[] maxPackedValue, int[] parentSplits) {
    ...
    for(int dim=0;dim<numDims;dim++) {
      ...
      if (splitDim == -1 || FutureArrays.compareUnsigned(scratchDiff, 0, 0 + bytesPerDim, scratch1, 0, 0 + bytesPerDim) > 0) {
