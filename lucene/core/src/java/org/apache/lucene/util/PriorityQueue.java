...
public abstract class PriorityQueue<T> implements Iterable<T> {
   ...
   * new object.<br>
   * Those sentinel values should always compare worse than any non-sentinel value (i.e., {@link
   * #lessThan} should always favor the non-sentinel values).<br>
   * By default, the supplier returns null, which means the queue will not be filled with sentinel
   * values. Otherwise, the value returned will be used to pre-populate the queue.<br>
