 ...
 *
 * <p>The index stores statistics about terms in order to make term-based search more efficient.
 * Lucene's index falls into the family of indexes known as an <i>inverted index.</i> This is
 * because it can list, for a term, the documents that contain it. This is the inverse of the
 * natural relationship, in which documents list terms. <a id="Types_of_Fields"></a>
 *
 ...
 *   <li>{@link org.apache.lucene.codecs.lucene60.Lucene60FieldInfosFormat Field names}. This
 *       contains the set of field names used in the index.
 *   <li>Stored Field values. This contains, for each document, a list of attribute-value pairs,
 *       where the attributes are field names. These are used to store auxiliary information about
 *       the document, such as its title, url, or an identifier to access a database. The set of
 *       stored fields are what is returned for each hit when searching. This is keyed by document
 *       number.
