...
public class TestLucene60PointsFormat extends BasePointsFormatTestCase {
  ...
  public void testEstimatePointCount() throws IOException {
    ...
    final long pointCount = points.estimatePointCount(new IntersectVisitor() {
          ...
          @Override
          public Relation compare(byte[] minPackedValue, byte[] maxPackedValue) {
            if (FutureArrays.compareUnsigned(uniquePointValue, 0, 0 + 3, maxPackedValue, 0, 0 + 3) > 0 ||
                FutureArrays.compareUnsigned(uniquePointValue, 0, 0 + 3, minPackedValue, 0, 0 + 3) < 0) {
              return Relation.CELL_OUTSIDE_QUERY;
            }
    ...
    final long pointCount = points.estimatePointCount(new IntersectVisitor() {
        ...
        public Relation compare(byte[] minPackedValue, byte[] maxPackedValue) {
          for (int dim = 0; dim < 2; ++dim) {
            if (FutureArrays.compareUnsigned(uniquePointValue[dim], 0, 0 + 3, maxPackedValue, dim * 3, dim * 3 + 3) > 0 ||
                FutureArrays.compareUnsigned(uniquePointValue[dim], 0, 0 + 3, minPackedValue, dim * 3, dim * 3 + 3) < 0) {
              return Relation.CELL_OUTSIDE_QUERY;
            }
          }
          return Relation.CELL_CROSSES_QUERY;
