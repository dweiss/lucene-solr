...
public class TestBKD extends LuceneTestCase {
  ...
  public void testEstimatePointCount() throws IOException {
    ...
    final long pointCount = points.estimatePointCount(new IntersectVisitor() {
      ...
      @Override
      public Relation compare(byte[] minPackedValue, byte[] maxPackedValue) {
        if (FutureArrays.compareUnsigned(uniquePointValue, 0, 0 + numBytesPerDim, maxPackedValue, 0, 0 + numBytesPerDim) > 0 ||
            FutureArrays.compareUnsigned(uniquePointValue, 0, 0 + numBytesPerDim, minPackedValue, 0, 0 + numBytesPerDim) < 0) {
          return Relation.CELL_OUTSIDE_QUERY;
        }
        return Relation.CELL_CROSSES_QUERY;
      }
