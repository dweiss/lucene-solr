...
public abstract class BaseShapeTestCase extends LuceneTestCase {
          ...
          if (queryRelation == QueryRelation.CONTAINS && rectCrossesDateline(rect)) {
            ...
            expected =
                VALIDATOR
                    ...
                    .testBBoxQuery(qMinLat, qMaxLat, qMinLon, GeoUtils.MAX_LON_INCL, shapes[id]);
            if (expected) {
              expected =
                  VALIDATOR
                      .setRelation(queryRelation)
                      .testBBoxQuery(qMinLat, qMaxLat, GeoUtils.MIN_LON_INCL, qMaxLon, shapes[id]);
            }
          } else {
            ...
            expected =
                VALIDATOR
                    .setRelation(queryRelation)
                    .testBBoxQuery(qMinLat, qMaxLat, qMinLon, qMaxLon, shapes[id]);
          }
        }
  ...
  protected abstract static class Encoder {
    ...
    abstract double[] quantizeTriangle(
        double ax,
        double ay,
        boolean ab,
        double bx,
    ...
    abstract ShapeField.DecodedTriangle encodeDecodeTriangle(
        double ax,
        double ay,
        boolean ab,
        double bx,
  ...
  protected abstract static class Validator {
    ...
    public abstract boolean testBBoxQuery(
