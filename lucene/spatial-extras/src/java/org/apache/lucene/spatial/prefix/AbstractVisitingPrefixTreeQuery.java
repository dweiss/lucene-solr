...
/**
 * Traverses a {@link SpatialPrefixTree} indexed field, using the template and visitor design
 * patterns for subclasses to guide the traversal and collect matching documents.
 *
 * <p>Subclasses implement {@link #getDocIdSet(org.apache.lucene.index.LeafReaderContext)} by
 * instantiating a custom {@link VisitorTemplate} subclass (i.e. an anonymous inner class) and
...
public abstract class AbstractVisitingPrefixTreeQuery extends AbstractPrefixTreeQuery {
  ...
  //  differently depending on cell state like IsWithin and for fuzzy/accurate collection planned
  // improvements.  At
   ...
   * {@link SpatialPrefixTree} indexed field. An instance of this class is not designed to be
   * re-used across LeafReaderContext instances so simply create a new one per-leaf. The {@link
   * #getDocIdSet()} method here starts the work. It first checks that there are indexed terms; if
   * not it quickly returns null. Then it calls {@link #start()} so a subclass can set up a return
   * value, like an {@link org.apache.lucene.util.FixedBitSet}. Then it starts the traversal
   * process, calling {@link #findSubCellsToVisit(org.apache.lucene.spatial.prefix.tree.Cell)} which
   * by default finds the top cells that intersect {@code queryShape}. If there isn't an indexed
   * cell for a corresponding cell returned for this method then it's short-circuited until it finds
   * one, at which point {@link #visitPrefix(org.apache.lucene.spatial.prefix.tree.Cell)} is called.
   * At some depths, of the tree, the algorithm switches to a scanning mode that calls {@link
   * #visitScanned(org.apache.lucene.spatial.prefix.tree.Cell)} for each leaf cell found.
   *
   * @lucene.internal
   */
  public abstract class VisitorTemplate extends BaseTermsEnumTraverser {
    ...
    */
    ...
    private BytesRef curVNodeTerm =
        new BytesRef(); // curVNode.cell's term, without leaf. in main loop only
    ...
    public DocIdSet getDocIdSet() throws IOException {
      ...
      while (thisTerm != null) { // terminates for other reasons too!
        ...
        if (curVNode.children != null) {
          ...
          while (true) {
            if (parentVNode == null) break main; // all done
        ...
        if (compare < 0) {
          ...
          if (seekStatus == TermsEnum.SeekStatus.END) break; // all done
        ...
        final boolean descend =
            visitPrefix(curVNode.cell); // need to use curVNode.cell not indexedCell
        if (!nextTerm()) break;
    ...
    /**
     * Called when doing a divide and conquer to find the next intersecting cells of the query shape
     * that are beneath {@code cell}. {@code cell} is guaranteed to have an intersection and thus
     * this must return some number of nodes.
     */
    ...
    private boolean nextTerm() throws IOException {
      if ((thisTerm = termsEnum.next()) == null) return false;
      indexedCell = grid.readCell(thisTerm, indexedCell);
      return true;
    }
    ...
    /**
     * Visit an indexed non-leaf cell. The presence of a prefix cell implies there are leaf cells at
     * further levels. The cell passed should have it's {@link
     * org.apache.lucene.spatial.prefix.tree.Cell#getShapeRel()} set relative to the filtered shape.
     *
    ...
    /**
     * Called when an indexed leaf cell is found. An indexed leaf cell usually means associated
     * documents won't be found at further detail levels. However, if a document has multiple
     * overlapping shapes at different resolutions, then this isn't true.
     */
  ...
  /**
   * A visitor node/cell found via the query shape for {@link VisitorTemplate}. Sometimes these are
