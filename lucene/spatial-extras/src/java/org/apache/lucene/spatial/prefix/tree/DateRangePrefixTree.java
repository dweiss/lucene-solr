 ...
 * (e.g. years, months, ...). You pass in {@link Calendar} objects with the desired fields set and
 * the unspecified fields unset, which conveys the precision. The implementation makes some
 * optimization assumptions about a {@link java.util.GregorianCalendar}; others could probably be
 * supported easily.
 *
 * <p>Warning: If you construct a Calendar and then get something from the object like a field (e.g.
 * year) or milliseconds, then every field is fully set by side-effect. So after setting the fields,
 * pass it to this API first.
 *
 * @lucene.experimental
 */
public class DateRangePrefixTree extends NumberRangePrefixTree {
  ...
  */
  ...
  /**
   * The Java platform default {@link Calendar} with UTC &amp; ROOT Locale. Generally a {@link
   * GregorianCalendar}. Do <em>not</em> modify this!
   */
  ...
  /**
   * A Calendar instance compatible with {@link java.time.ZonedDateTime} as seen from {@link
   * GregorianCalendar#from(ZonedDateTime)}. Do <em>not</em> modify this!
   */
  ...
  private int slowSubCells(UnitNRShape lv) {
    ...
    return super.getNumSubCells(lv);
    Calendar cal =
        toCalendar(lv); // somewhat heavyweight op; ideally should be stored on UnitNRShape somehow
    return cal.getActualMaximum(field) - cal.getActualMinimum(field) + 1;
  }

  /**
   * Calendar utility method: Returns a clone of the {@link Calendar} passed to the constructor with
   * all fields cleared.
   */
  ...
  /**
   * Calendar utility method: Returns the spatial prefix tree level for the corresponding {@link
   * java.util.Calendar} field, such as {@link java.util.Calendar#YEAR}. If there's no match, the
   * next greatest level is returned as a negative value.
   */
  ...
  /**
   * Calendar utility method: Gets the Calendar field code of the last field that is set prior to an
   * unset field. It only examines fields relevant to the prefix tree. If no fields are set, it
   * returns -1.
   */
  ...
  /**
   * Converts {@code value} from a {@link Calendar} or {@link Date} to a {@link Shape}. Other
   * arguments result in a {@link java.lang.IllegalArgumentException}. If a Calendar is passed in,
   * there might be problems if it is not created via {@link #newCal()}.
   */
  @Override
  ...
  private int parseAndCheck(String str, int offset, int min, int max) {
    ...
    if (val < min || val > max) {
      throw new IllegalArgumentException(
          "Invalid value: " + val + "," + " expecting from " + min + " to " + max + "]");
    }
    return val;
  }
}
