...
/**
 * A basic implementation of {@link PrefixTreeStrategy} using a large {@link TermInSetQuery} of all
 * the cells from {@link
 * SpatialPrefixTree#getTreeCellIterator(org.locationtech.spatial4j.shape.Shape, int)}. It only
 * supports the search of indexed Point shapes.
 *
 * <p>The precision of query shapes (distErrPct) is an important factor in using this Strategy. If
 * the precision is too precise then it will result in many terms which will amount to a slower
 * query.
 *
 * @lucene.experimental
 */
public class TermQueryPrefixTreeStrategy extends PrefixTreeStrategy {
  ...
  public Query makeQuery(SpatialArgs args) {
    ...
    if (shape instanceof Point) GUESS_NUM_TERMS = detailLevel; // perfect guess
    else GUESS_NUM_TERMS = 4096; // should this be a method on SpatialPrefixTree?
