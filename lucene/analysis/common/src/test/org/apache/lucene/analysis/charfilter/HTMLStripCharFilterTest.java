...
public class HTMLStripCharFilterTest extends BaseTokenStreamTestCase {
  ...
  public void testHTML() throws Exception {
    ...
    assertTrue(
        "Entity not properly escaped", str.indexOf("&lt;") == -1); // there is one > in the text
    assertTrue(
        "Forrest should have been stripped out",
        str.indexOf("forrest") == -1 && str.indexOf("Forrest") == -1);
    assertTrue(
        "File should start with 'Welcome to Solr' after trimming",
        str.trim().startsWith("Welcome to Solr"));

    assertTrue(
        "File should start with 'Foundation.' after trimming", str.trim().endsWith("Foundation."));
  }
  ...
  public void testReserved() throws Exception {
    String test =
        "aaa bbb <reserved ccc=\"ddddd\"> eeee </reserved> ffff <reserved ggg=\"hhhh\"/> <other/>";
    ...
    assertTrue(
        "Escaped tag not preserved: " + result.indexOf("reserved"),
        result.indexOf("reserved") == 9);
    assertTrue(
  ...
  public void testCDATA() throws Exception {
    int maxNumElems = 100;
    String
        randomHtmlishString1 // Don't create a comment (disallow "<!--") and don't include a closing
        // ">"
        =
    ...
    String
        randomHtmlishString2 // Don't create a comment (disallow "<!--") and don't include a closing
        // ">"
        =
            TestUtil.randomHtmlishString(random(), maxNumElems)
                .replaceAll(">", " ")
      ...
      case 1:
        {
          for (int wordNum = 0; wordNum < numWords; ++wordNum) {
            text.append(
                TestUtil.randomRealisticUnicodeString(random(), minWordLength, maxWordLength));
            text.append(' ');
          }
          break;
        }
  ...
  public static void assertHTMLStripsTo(Reader input, String gold, Set<String> escapedTags)
      throws Exception {
    HTMLStripCharFilter reader;
    if (null == escapedTags) {
    ...
    assertEquals(
        "'" + builder.toString() + "' is not equal to '" + gold + "'", gold, builder.toString());
  }
}
