...
/**
 * Splits words into subwords and performs optional transformations on subword groups. Words are
 * split into subwords with the following rules:
 *
 * <ul>
 ...
 * </ul>
 *
 * One use for {@link WordDelimiterFilter} is to help match words with different subword delimiters.
 * For example, if the source text contained "wi-fi" one may want "wifi" "WiFi" "wi-fi" "wi+fi"
 * queries to all match. One way of doing so is to specify CATENATE options in the analyzer used for
 * indexing, and not in the analyzer used for querying. Given that the current {@link
 ...
 *
 * @deprecated Use {@link WordDelimiterGraphFilter} instead: it produces a correct token graph so
 *     that e.g. {@link PhraseQuery} works correctly when it's used in the search time analyzer.
 */
@Deprecated
public final class WordDelimiterFilter extends TokenFilter {
  ...
  public WordDelimiterFilter(
      TokenStream in, byte[] charTypeTable, int configurationFlags, CharArraySet protWords) {
    super(in);
  ...
  final class WordDelimiterConcatenation {
    ...
    /**
     * Convenience method for the common scenario of having to write the concetenation and then
     * clearing its state
     */
    void writeAndClear() {
      write();
