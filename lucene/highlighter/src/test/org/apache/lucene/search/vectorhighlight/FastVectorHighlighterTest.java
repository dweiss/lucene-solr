...
public class FastVectorHighlighterTest extends LuceneTestCase {
  ...
  public void testSimpleHighlightTest() throws IOException {
    ...
    FieldQuery fieldQuery =
        highlighter.getFieldQuery(new TermQuery(new Term("field", "foo")), reader);
    ...
    FieldQuery fieldQuery =
        highlighter.getFieldQuery(
            new FunctionScoreQuery(
                new TermQuery(new Term("field", "foo")), DoubleValuesSource.constant(1)),
            reader);
    String[] bestFragments =
        highlighter.getBestFragments(fieldQuery, reader, docId, "field", 54, 1);
    // highlighted results are centered
  ...
  public void testPhraseHighlightLongTextTest() throws IOException {
    ...
    String field = "text";
    {
      ...
      assertEquals(
          "first browser war it lost virtually all of its share to <b>Internet</b> <b>Explorer</b> Netscape was discontinued and support for all Netscape browsers",
          bestFragments[0]);
    }

    {
      ...
      assertEquals(1, bestFragments.length);
      assertEquals(
          "first browser war it lost virtually all of its share to <b>Internet Explorer</b> Netscape was discontinued and support for all Netscape browsers",
          bestFragments[0]);
    }
    reader.close();
    writer.close();
  ...
  public void testBoostedPhraseHighlightTest() throws IOException {
    ...
    // Now combine those results in a boolean query which should pull the phrases to the front of
    ...
    assertEquals(
        "junk junk junk junk junk junk junk junk <b>highlight words together</b> junk junk junk junk junk junk junk junk",
        fragment);
  ...
  public void testCommonTermsQueryHighlight() throws IOException {
    ...
    assertEquals(
        "This piece of <b>text</b> refers to Kennedy at the beginning then has a longer piece of <b>text</b> that is <b>very</b> <b>long</b> in the middle and finally ends with another reference to Kennedy",
        bestFragments[0]);
    ...
    assertEquals(
        "Hello this is a piece of <b>text</b> that is <b>very</b> <b>long</b> and contains too much preamble and the meat is really here which says kennedy has been shot",
        bestFragments[0]);
  ...
  public void testMultiValuedSortByScore() throws IOException {
    ...
    type.freeze();
    doc.add(
        new Field("field", "zero if naught", type)); // The first two fields contain the best match
    doc.add(
        new Field("field", "hero of legend", type)); // but total a lower score (3) than the bottom
    ...
    for (FragListBuilder fragListBuilder :
        new FragListBuilder[] {new SimpleFragListBuilder(), new WeightedFragListBuilder()}) {
      String[] bestFragments =
  ...
  public void testBooleanPhraseWithSynonym() throws IOException {
    ...
    // query2: match
    PhraseQuery pq2 =
        new PhraseQuery("field", "test", "httpwwwfacebookcom", "www", "facebook", "com");
  ...
  public void testPhrasesSpanningFieldValues() throws IOException {
    ...
    String[] bestFragments =
        highlighter.getBestFragments(fieldQuery, reader, docId, "field", 1000, 1000);
  ...
  private void matchedFieldsTestCase(String fieldValue, String expected, Query... queryClauses)
      throws IOException {
