...
public class BlendedInfixLookupFactory extends AnalyzingInfixLookupFactory {
   ...
   * Available blender types are: </br> linear: weight*(1 - 0.10*position) [default]</br>
  ...
  /** Factor to multiply the number of searched elements Default is 10 */
  ...
  public Lookup create(@SuppressWarnings({"rawtypes"}) NamedList params, SolrCore core) {
    ...
    int numFactor =
        params.get(NUM_FACTOR) != null
            ? Integer.parseInt(params.get(NUM_FACTOR).toString())
            : BlendedInfixSuggester.DEFAULT_NUM_FACTOR;

    Double exponent =
        params.get(EXPONENT) == null ? null : Double.valueOf(params.get(EXPONENT).toString());
