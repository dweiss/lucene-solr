 ...
 *   <li>It's OK to have some documents without a keyField in the file (defVal is used as the
 *       default)
 *   <li>It's OK for a keyField value to point to multiple documents (no uniqueness requirement)
 * </ul>
 *
 * The format of the external file is simply newline separated keyFieldValue=floatValue. <br>
 * Example: <br>
 ...
 *
 * <p>If any files of the latter pattern appear, the last (after being sorted by name) will be used
 * and previous versions will be deleted. This is to help support systems where one may not be able
 * to overwrite a file (like Windows, if the file is in use).
 *
 * <p>If the external file has already been loaded, and it is changed, those changes will not be
 * visible until a commit has been done.
 *
 * <p>The external file may be sorted or unsorted by the key field, but it will be substantially
...
public class ExternalFileField extends FieldType implements SchemaAware {
   ...
   * getFileFloatSource(SchemaField) if this may be called before the Core is fully initialised (eg
