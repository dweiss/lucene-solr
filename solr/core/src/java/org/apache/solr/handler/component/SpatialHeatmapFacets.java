...
/**
 * A 2D spatial faceting summary of a rectangular region. Used by {@link
 * org.apache.solr.handler.component.FacetComponent} and {@link
...
public class SpatialHeatmapFacets {
  ...
  private static FacetRequest createHeatmapRequest(
    ...
    jsonFacet.put(
        FacetHeatmap.GEOM_PARAM, params.getFieldParam(fieldKey, FacetParams.FACET_HEATMAP_GEOM));
    jsonFacet.put(
        FacetHeatmap.LEVEL_PARAM, params.getFieldInt(fieldKey, FacetParams.FACET_HEATMAP_LEVEL));
    jsonFacet.put(
        FacetHeatmap.DIST_ERR_PCT_PARAM,
        params.getFieldDouble(fieldKey, FacetParams.FACET_HEATMAP_DIST_ERR_PCT));
    jsonFacet.put(
        FacetHeatmap.DIST_ERR_PARAM,
        params.getFieldDouble(fieldKey, FacetParams.FACET_HEATMAP_DIST_ERR));
    jsonFacet.put(
        FacetHeatmap.MAX_CELLS_PARAM,
        params.getFieldInt(fieldKey, FacetParams.FACET_HEATMAP_MAX_CELLS));
    jsonFacet.put(
  ...
  public static void distribModifyRequest(
    ...
    for (HeatmapFacet facet : heatmapFacets.values()) {
      ...
      sreq.params.add(
          FacetParams.FACET_HEATMAP, newLocalParams.toLocalParamsString() + facet.facetOn);
    }
  }
  ...
  public static void distribHandleResponse(
      LinkedHashMap<String, HeatmapFacet> heatmapFacets,
      @SuppressWarnings({"rawtypes"}) NamedList srsp_facet_counts) {
  ...
  // Note: originally there was a lot more code here but it migrated to the JSON Facet API as
  // "FacetHeatmap"
