...
public class SnapShooter {
  ...
  private IndexCommit getAndSaveIndexCommit() throws IOException {
    ...
    if (null != commitName) {
      ...
      // We're going to tell the delPolicy to "save" this commit -- even though it's a named
      // snapshot
      // that will already be protected -- just in case another thread deletes the name.
      // Because of this, we want to sync on the delPolicy to ensure there is no window of time
      // after
  ...
  protected NamedList createSnapshot(final IndexCommit indexCommit) throws Exception {
    ...
    try {
      ...
      details.add(
          "snapshotCompletedAt", new Date().toString()); // bad; should be Instant.now().toString()
