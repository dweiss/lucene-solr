 ...
 *
 * <p>By default the handler returns status <code>200 OK</code> if all checks succeed, else it
 * returns status <code>503 UNAVAILABLE</code>:
 *
 * <ol>
    ...
    if (!cores.isZooKeeperAware()) {
      ...
      rsp.setException(
          new SolrException(
              ...
              "Health check is only available when running in SolrCloud mode"));
      return;
    }
    if (log.isDebugEnabled()) {
      log.debug(
          "Invoked HealthCheckHandler on [{}]", coreContainer.getZkController().getNodeName());
    }
