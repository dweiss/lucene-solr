...
abstract class CSVLoaderBase extends ContentStreamLoader {
  ...
  /**
   * Add a field to a document unless it's zero length. The FieldAdder hierarchy handles all the
   * complexity of further transforming or splitting field values to keep the main logic loop clean.
   * All implementations of add() must be MT-safe!
   */
  ...
  private class FieldTrimmer extends CSVLoaderBase.FieldAdder {
    ...
    FieldTrimmer(CSVLoaderBase.FieldAdder base) {
      this.base = base;
    }
  ...
  /**
   * map a single value. for just a couple of mappings, this is probably faster than using a
   * HashMap.
   */
  ...
  /** Split a single value into multiple values based on a CSVStrategy. */
  ...
  void doAdd(int line, String[] vals, SolrInputDocument doc, AddUpdateCommand template)
      throws IOException {
