 ...
 *
 * <p>The most common way to use this component is in conjunction with field that use {@link
 * ShingleFilterFactory} on both the <code>index</code> and <code>query</code> analyzers. An example
 ...
 * </pre>
 *
 * <p>...where the <code>query</code> analyzer's <code>maxShingleSize="7"</code> determines the
 * maximum possible phrase length that can be hueristically deduced, the <code>index</code>
 * analyzer's <code>maxShingleSize="3"</code> determines the accuracy of phrases identified. The
 * large the indexed <code>maxShingleSize</code> the higher the accuracy. Both analyzers must
 * include <code>minShingleSize="2" outputUnigrams="true"</code>.
 *
 * <p>With a field type like this, one or more fields can be specified (with weights) via a <code>
 * phrases.fields</code> param to request that this component identify possible phrases in the input
 * <code>q</code> param, or an alternative <code>phrases.q</code> override param. The identified
 ...
 *
 * <p><b>NOTE:</b> In a distributed request, this component uses a single phase (piggy backing on
...
public class PhrasesIdentificationComponent extends SearchComponent {
      ...
      if (0 != (sreq.purpose & SHARD_PURPOSE)) {
        for (ShardResponse shardRsp : sreq.responses) {
          ...
          if (!shardsHandled.get(shardNum)) {
            ...
            @SuppressWarnings({"unchecked"})
            final List<NamedList<Object>> shardPhrases =
                (List<NamedList<Object>>) phrasesWrapper.get("_all");
  ...
  private void scoreAndAddResultsToResponse(
    ...
    final List<Phrase> validScoringPhrasesSorted =
  ...
  public static final class PhrasesContextData {
     ...
     * invalid input, and returning a <code>PhrasesContextData</code> suitable for use in this
     * request.
     */
    ...
    private PhrasesContextData(final SolrQueryRequest req) throws SolrException {
      ...
      { // field weights & analysis field...
        ...
        final Map<String, Float> rawFields =
  ...
  public static final class Phrase {
    ...
    public static void populateStats(
        final List<Phrase> phrases, final List<NamedList<Object>> shardData) {
    ...
    /**
     * Returns the list of "individual" (ie: <code>getPositionLength()==1</code> terms. NOTE:
    ...
    /**
     * Returns the number of total TTF of this (indexed) Phrase <em>as term</em> in the specified
     ...
     *
     * <p>The current implementation returns scores in the range of <code>[0,1]</code>, but this may
     * change in future implementations. The only current garuntees are:
     *
     * <ul>
     *   <li>0 (or less) means this is garunteed to not be a phrase
     *   <li>larger numbers are higher confidence
     *
     * @see #populateStats
     * @see #populateScores
     * @see #getFieldScore(String)
   ...
   *
   * <p>Given an analyzer, inspects it to determine if:
   *
   * <ul>
   *   <li>it is a {@link TokenizerChain}
   *   <li>it contains exactly one instance of {@link ShingleFilterFactory}
   * </ul>
   *
   * <p>If these these conditions are met, then this method returns the <code>maxShingleSize</code>
