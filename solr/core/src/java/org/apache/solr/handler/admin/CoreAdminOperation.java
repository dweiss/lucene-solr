...
enum CoreAdminOperation implements CoreAdminOp {
  ...
  UNLOAD_OP(
      ...
      it -> {
        ...
        boolean deleteMetricsHistory =
            params.getBool(CoreAdminParams.DELETE_METRICS_HISTORY, false);
        ...
        if (deleteMetricsHistory) {
          MetricsHistoryHandler historyHandler =
              it.handler.coreContainer.getMetricsHistoryHandler();
  ...
  REQUESTSTATUS_OP(
      ...
      it -> {
        ...
        if (it.handler.getRequestStatusMap(RUNNING).containsKey(requestId)) {
          ...
          it.rsp.add(
              RESPONSE, it.handler.getRequestStatusMap(COMPLETED).get(requestId).getRspObject());
        } else if (it.handler.getRequestStatusMap(FAILED).containsKey(requestId)) {
          it.rsp.add(RESPONSE_STATUS, FAILED);
          it.rsp.add(
              RESPONSE, it.handler.getRequestStatusMap(FAILED).get(requestId).getRspObject());
        } else {
  ...
  static NamedList<Object> getCoreStatus(
