...
public class HttpShardHandlerFactory extends ShardHandlerFactory
  ...
  static final String SET_SOLR_DISABLE_SHARDS_WHITELIST_CLUE =
      " set -D" + INIT_SOLR_DISABLE_SHARDS_WHITELIST + "=true to disable shards whitelist checks";
  ...
  /**
   * Returns this Factory's {@link WhitelistHostChecker}. This method can be overridden to change
  ...
  private void initReplicaListTransformers(
      @SuppressWarnings({"rawtypes"}) NamedList routingConfig) {
  ...
  public void init(PluginInfo info) {
    ...
    this.whitelistHostChecker =
        new WhitelistHostChecker(
            args == null ? null : (String) args.get(INIT_SHARDS_WHITELIST),
            !getDisableShardsWhitelist());
  ...
  protected <T> T getParameter(
      @SuppressWarnings({"rawtypes"}) NamedList initArgs,
      String configKey,
      T defaultValue,
      StringBuilder sb) {
  ...
  public static class WhitelistHostChecker {
    ...
    /**
     * List of the whitelisted hosts. Elements in the list will be host:port (no protocol or
     * context)
     */
    ...
    static final Set<String> implGetShardsWhitelist(final String shardsWhitelist) {
      if (shardsWhitelist != null && !shardsWhitelist.isEmpty()) {
        return StrUtils.splitSmart(shardsWhitelist, ',').stream()
            .map(String::trim)
            .map(
                (hostUrl) -> {
                  URL url;
                  try {
                    if (!hostUrl.startsWith("http://") && !hostUrl.startsWith("https://")) {
                      ...
                      // going to use it. We just need a full URL.
    ...
    /**
     * Checks that all the hosts for all the shards requested in shards parameter exist in the
     * configured whitelist or in the ClusterState (in case of cloud mode)
     *
    ...
    protected void checkWhitelist(
        ClusterState clusterState, String shardsParamValue, List<String> shardUrls) {
              ...
              (shardUrl) -> {
                ...
                try {
                  if (!shardUrl.startsWith("http://") && !shardUrl.startsWith("https://")) {
                    // It doesn't really matter which protocol we set here because we are not going
                    // to use it. We just need a full URL.
                  ...
                  throw new SolrException(
                      ...
                      "Invalid URL syntax in \"shards\" parameter: " + shardsParamValue,
                      e);
                }
                if (url.getHost() == null || url.getPort() < 0) {
                  throw new SolrException(
                ...
                if (!localWhitelistHosts.contains(url.getHost() + ":" + url.getPort())) {
                  log.warn(
                      "The '{}' parameter value '{}' contained value(s) not on the shards whitelist ({}), shardUrl: '{}'",
                      ShardParams.SHARDS,
                      shardsParamValue,
                      localWhitelistHosts,
                      shardUrl);
                  throw new SolrException(
                      ...
                      "The '"
                          ...
                          + "' parameter value '"
                          + shardsParamValue
                          + "' contained value(s) not on the shards whitelist. shardUrl:"
                          + shardUrl
                          + "."
    ...
    public String toString() {
      return "WhitelistHostChecker [whitelistHosts="
          + whitelistHosts
