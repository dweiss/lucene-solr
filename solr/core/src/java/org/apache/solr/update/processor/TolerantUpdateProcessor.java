 ...
 * on the first error, at most <code>maxErrors</code> errors (or unlimited if <code>-1==maxErrors
 * </code>) are logged and recorded the batch continues. The client will receive a <code>status==200
 * </code> response, which includes a list of errors that were tolerated.
 *
 * <p>If more then <code>maxErrors</code> occur, the first exception recorded will be re-thrown,
 * Solr will respond with <code>status==5xx</code> or <code>status==4xx</code> (depending on the
 ...
 *
 * <p>NOTE: In cloud based collections, this processor expects to <b>NOT</b> be used on {@link
 * DistribPhase#FROMLEADER} requests (because any successes that occur locally on the leader are
 * considered successes even if there is some subsequent error on a replica). {@link
 * TolerantUpdateProcessorFactory} will short circut it away in those requests.
 *
...
public class TolerantUpdateProcessor extends UpdateRequestProcessor {
  ...
  /**
   * Number of errors this UpdateRequestProcessor will tolerate. If more then this occur, the
   * original exception will be thrown, interrupting the processing of the document batch
   */
  ...
  // ToleratedUpdateErrors from duplicate identical underlying requests w/o erroneously returning
  // identical
  ...
  public TolerantUpdateProcessor(
      SolrQueryRequest req,
      SolrQueryResponse rsp,
      UpdateRequestProcessor next,
      int maxErrors,
      DistribPhase distribPhase) {
    super(next);
    assert maxErrors >= -1;
  ...
  public void finish() throws IOException {
    ...
    // include in response so client knows what effective value was (may have been server side
    // config)
  ...
  /**
   * Returns the output of {@link org.apache.solr.schema.FieldType# indexedToReadable(BytesRef,
   * CharsRefBuilder)} of the field type of the uniqueKey on the {@link BytesRef} passed as
   * parameter. <code>ref</code> should be the indexed representation of the id -- if null (possibly
   * because it's missing in the update) this method will return {@link #UNKNOWN_ID}
   */
   ...
   *
   * <p>Only remembers the "first" exception encountered, and wraps it in a SolrException if needed,
   * so that it can later be annotated with the metadata our users expect and re-thrown.
   *
  ...
  private static final class FirstErrTracker {
    ...
    /**
     * Call this method immediately anytime an exception is caught from a down stream method -- even
     * if you are going to ignore it (for now). If you plan to rethrow the Exception, use {@link
     * #throwFirst} instead.
     */
    public void caught(Throwable t) {
      ...
      if (null == first) {
        if (t instanceof SolrException) {
          ...
          first =
              new SolrException(
                  ErrorCode.SERVER_ERROR, "Tolerantly Caught Exception: " + t.getMessage(), t);
        }
      }
    }
     ...
     * (already passed to the {@link #caught} method because maxErrors was exceeded is exceed.
     *
     * <p>This method will keep a record that this update processor has already thrown the
     * exception, and do nothing on future calls, so subsequent update processor methods can update
     * the metadata but won't inadvertently re-throw this (or any other) cascading exception by
     * mistake.
     */
    ...
    /**
     * Annotates the first exception (which may already have been thrown, or be thrown in the
     * future) with the metadata from this update processor. For use in {@link
     * TolerantUpdateProcessor#finish}
     */
