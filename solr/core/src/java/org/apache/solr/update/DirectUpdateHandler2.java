...
public class DirectUpdateHandler2 extends UpdateHandler
   ...
   * AddUpdateCommand#getLuceneDocument} must either be the uniqueKey field, or be DocValue only
   * fields.
   *
  ...
  private void updateDocOrDocValues(AddUpdateCommand cmd, IndexWriter writer) throws IOException {
    ...
    if (cmd.isInPlaceUpdate()) {
      ...
      Term idTerm =
          getIdTerm(
              isNested ? new BytesRef(cmd.getRootIdUsingRouteParam()) : cmd.getIndexedId(),
              isNested);
