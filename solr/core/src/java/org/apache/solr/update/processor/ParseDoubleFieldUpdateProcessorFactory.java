 ...
 * If required, rounding uses ceiling mode: {@link RoundingMode#CEILING}. Grouping separators (','
 * in the ROOT locale) are parsed.
 *
 * <p>The default selection behavior is to mutate both those fields that don't match a schema field,
 * as well as those fields that match a schema field with a double field type.
 *
 * <p>If all values are parseable as double (or are already Double), then the field will be mutated,
 * replacing each value with its parsed Double equivalent; otherwise, no mutation will occur.
 *
 ...
 * {@link Locale#ROOT} will be used. The following configuration specifies the Russian/Russia
 * locale, which will parse the string string "12 345,899" as double value 12345.899 (the grouping
 * separator character is U+00AO NO-BREAK SPACE).
 *
 ...
 * @since 4.4.0
 */
public class ParseDoubleFieldUpdateProcessorFactory
  ...
  private static final class ParseDoubleFieldUpdateProcessor
    ...
    ParseDoubleFieldUpdateProcessor(
