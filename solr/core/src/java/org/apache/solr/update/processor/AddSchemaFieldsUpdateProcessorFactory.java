 ...
 *
 * <p>By default, this processor selects all fields that don't match a schema field or dynamic
 * field. The "fieldName" and "fieldRegex" selectors may be specified to further restrict the
 * selected fields, but the other selectors ("typeName", "typeClass", and
 * "fieldNameMatchesSchemaField") may not be specified.
 *
 * <p>This processor is configured to map from each field's values' class(es) to the schema field
 * type that will be used when adding the new field to the schema. All new fields are then added to
 * the schema in a single batch. If schema addition fails for any field, addition is re-attempted
 * only for those that don’t match any schema field. This process is repeated, either until all new
 * fields are successfully added, or until there are no new fields (presumably because the fields
 * that were new when this processor started its work were subsequently added by a different update
 * request, possibly on a different node).
 *
 * <p>This processor takes as configuration a sequence of zero or more "typeMapping"-s from one or
 * more "valueClass"-s, specified as either an <code>&lt;arr&gt;</code> of <code>&lt;str&gt;</code>,
 * or multiple <code>&lt;str&gt;</code> with the same name, to an existing schema "fieldType".
 *
 * <p>If more than one "valueClass" is specified in a "typeMapping", field values with any of the
 * specified "valueClass"-s will be mapped to the specified target "fieldType". The "typeMapping"-s
 * are attempted in the specified order; if a field value's class is not specified in a
 * "valueClass", the next "typeMapping" is attempted. If no "typeMapping" succeeds, then either the
 * "typeMapping" configured with <code>&lt;bool name="default"&gt;true&lt;/bool&gt;</code> is used,
 * or if none is so configured, the <code>lt;str name="defaultFieldType"&gt;...&lt;/str&gt;</code>
 * is used.
 *
 * <p>Zero or more "copyField" directives may be included with each "typeMapping", using a <code>
 * &lt;lst&gt;</code>. The copy field source is automatically set to the new field name; "dest" must
 * specify the destination field or dynamic field in a <code>&lt;str&gt;</code>; and "maxChars" may
 * optionally be specified in an <code>&lt;int&gt;</code>.
 *
...
public class AddSchemaFieldsUpdateProcessorFactory extends UpdateRequestProcessorFactory
  ...
  public void init(@SuppressWarnings({"rawtypes"}) NamedList args) {
    ...
    inclusions.fieldNameMatchesSchemaField =
        false; // Explicitly (non-configurably) require unknown field names
    ...
    if (null != defaultFieldTypeParam) {
      if (!(defaultFieldTypeParam instanceof CharSequence)) {
        throw new SolrException(
            SERVER_ERROR, "Init param '" + DEFAULT_FIELD_TYPE_PARAM + "' must be a <str>");
      }
  ...
  private static List<TypeMapping> parseTypeMappings(
    ...
    for (Object typeMappingObj : typeMappingsParams) {
      ...
      if (!(fieldTypeObj instanceof CharSequence)) {
        throw new SolrException(
            SERVER_ERROR, "'" + FIELD_TYPE_PARAM + "' init param must be a <str>");
      }
      ...
      if (null != isDefaultObj) {
        if (!(isDefaultObj instanceof Boolean)) {
          throw new SolrException(
              SERVER_ERROR, "'" + IS_DEFAULT_PARAM + "' init param must be a <bool>");
        }
      ...
      while (typeMappingNamedList.get(COPY_FIELD_PARAM) != null) {
        ...
        if (!(destObj instanceof CharSequence)) {
          throw new SolrException(
              SERVER_ERROR, "'" + COPY_FIELD_PARAM + "' init param must be a <str>");
        }
        ...
        if (null != maxCharsObj) {
          if (!(maxCharsObj instanceof Integer)) {
            throw new SolrException(
                SERVER_ERROR, "'" + MAX_CHARS_PARAM + "' init param must be a <int>");
          }
  ...
  private void validateSelectorParams(SelectorParams params) {
    if (!params.typeName.isEmpty()) {
      throw new SolrException(
          SERVER_ERROR, "'typeName' init param is not allowed in this processor");
    }
    if (!params.typeClass.isEmpty()) {
      throw new SolrException(
          SERVER_ERROR, "'typeClass' init param is not allowed in this processor");
    }
    if (null != params.fieldNameMatchesSchemaField) {
      throw new SolrException(
          SERVER_ERROR,
          "'fieldNameMatchesSchemaField' init param is not allowed in this processor");
    }
  }
  ...
  private static class TypeMapping {
    ...
    public TypeMapping(
        ...
        Collection<String> valueClassNames,
        boolean isDefault,
    ...
    public void populateValueClasses(SolrCore core) {
      ...
      if (null == schema.getFieldTypeByName(fieldTypeName)) {
        throw new SolrException(
            SERVER_ERROR, "fieldType '" + fieldTypeName + "' not found in the schema");
      }
  ...
  private class AddSchemaFieldsUpdateProcessor extends UpdateRequestProcessor {
        ...
        for (final Map.Entry<String, List<SolrInputField>> entry : unknownFields.entrySet()) {
          ...
          if (typeMapping != null) {
            ...
            if (!typeMapping.copyFieldDefs.isEmpty()) {
              newCopyFields.put(
                  fieldName,
                  typeMapping.copyFieldDefs.stream()
                      .collect(Collectors.groupingBy(CopyFieldDef::getMaxChars)));
            }
          }
          newFields.add(
              oldSchema.newField(fieldName, fieldTypeName, Collections.<String, Object>emptyMap()));
        }
          ...
          try {
            ...
            for (Map.Entry<String, Map<Integer, List<CopyFieldDef>>> entry :
              ...
              for (Integer maxChars : entry.getValue().keySet()) {
                newSchema =
                    newSchema.addCopyFields(
                        srcField,
                        entry.getValue().get(maxChars).stream()
                            .map(f -> f.getDest(srcField))
            ...
            if (null != newSchema) {
              ...
              throw new SolrException(
                  SolrException.ErrorCode.SERVER_ERROR, "Failed to add fields and/or copyFields.");
            }
    ...
    private TypeMapping mapValueClassesToFieldType(List<SolrInputField> fields) {
      ...
      // Return the typeMapping marked as default, if we have one, else return null to use fallback
      // type
    ...
    private FieldNameSelector buildSelector(IndexSchema schema) {
      FieldNameSelector selector =
          FieldMutatingUpdateProcessor.createFieldNameSelector(
              ...
              schema,
              inclusions,
              fieldName -> null == schema.getFieldTypeNoEx(fieldName));
