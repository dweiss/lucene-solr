...
public class AddUpdateCommand extends UpdateCommand {
  ...
  /**
   * Creates and returns a lucene Document to index. Nested documents, if found, will cause an
   * exception to be thrown. Call {@link #getLuceneDocsIfNested()} for that. Any changes made to the
   * returned Document will not be reflected in the SolrInputDocument, or future calls to this
   * method. Note that the behavior of this is sensitive to {@link #isInPlaceUpdate()}.
   */
  public Document getLuceneDocument() {
    ...
    return DocumentBuilder.toDocument(
        solrInputDocument, req.getSchema(), isInPlaceUpdate(), ignoreNestedDocs);
  }

  /**
   * Returns the indexed ID for this document. The returned BytesRef is retained across multiple
  ...
  /**
   * Computes the final flattened Solr docs that are ready to be converted to Lucene docs. If no
   * flattening is performed then we return null, and the caller ought to use {@link
   * #getLuceneDocument()} instead. This should only be called once. Any changes made to the
   * returned Document(s) will not be reflected in the SolrInputDocument, or future calls to this
   * method.
   */
