...
public class DistributedUpdateProcessor extends UpdateRequestProcessor {
  ...
  boolean getUpdatedDocument(AddUpdateCommand cmd, long versionOnUpdate) throws IOException {
    ...
    SolrInputDocument oldRootDocWithChildren =
        RealTimeGetComponent.getInputDocument(
            cmd.getReq().getCore(), idBytes, RealTimeGetComponent.Resolution.ROOT_WITH_CHILDREN);

    if (oldRootDocWithChildren == null) {
      if (versionOnUpdate > 0) {
        // could just let the optimistic locking throw the error
        throw new SolrException(
            ErrorCode.CONFLICT, "Document not found for update.  id=" + idString);
    ...
    if (idField == null || oldRootDocWithChildren == null) {
      ...
      // Safety check: don't allow an update to an existing doc that has children, unless we
      // actually support this.
      if (req.getSchema()
              .isUsableForChildDocs() // however, next line we see it doesn't support child docs
          && req.getSchema().supportsPartialUpdatesOfChildDocs() == false
          && req.getSearcher().count(new TermQuery(new Term(IndexSchema.ROOT_FIELD_NAME, idBytes)))
              > 1) {
        throw new SolrException(
            ErrorCode.BAD_REQUEST,
      ...
      String oldRootDocRootFieldVal =
          (String) oldRootDocWithChildren.getFieldValue(IndexSchema.ROOT_FIELD_NAME);
      if (req.getSchema().savesChildDocRelations()
        ...
        SolrInputDocument sdocWithChildren =
            RealTimeGetComponent.getInputDocument(
                cmd.getReq().getCore(), idBytes, RealTimeGetComponent.Resolution.DOC_WITH_CHILDREN);
  ...
  /**
   * {@link AddUpdateCommand#isNested} is set in {@link
   * org.apache.solr.update.processor.NestedUpdateProcessorFactory}, which runs on leader and
   * replicas just before run time processor
   *
  ...
  private static boolean shouldRefreshUlogCaches(AddUpdateCommand cmd) {
    // should be set since this method should only be called after
