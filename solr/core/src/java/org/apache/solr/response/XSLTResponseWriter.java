...
/**
 * QueryResponseWriter which captures the output of the XMLWriter (in memory for now, not optimal
 * performancewise), and applies an XSLT transform to it.
 */
  ...
  protected Transformer getTransformer(SolrQueryRequest request) throws IOException {
    ...
    if (xslt == null) {
      throw new IOException(
          "'" + CommonParams.TR + "' request parameter is required to use the XSLTResponseWriter");
    }
    ...
    if (result == null) {
      result =
          TransformerProvider.instance.getTransformer(
              solrConfig, xslt, xsltCacheLifetimeSeconds.intValue());
