 ...
 *
 * <p>The "parentFilter" parameter is mandatory if the schema is not of nest/hierarchy.
 *
 ...
 * be returned per parent document. By default it's set to 10.
 *
...
public class ChildDocTransformerFactory extends TransformerFactory {
  ...
  private DocTransformer createChildDocTransformer(
    ...
    if (parentFilterStr == null) {
      if (!buildHierarchy) {
        throw new SolrException(
            ErrorCode.BAD_REQUEST, "Parent filter should be sent as parentFilter=filterCondition");
      }
    ...
    return new ChildDocTransformer(
        field, parentsFilter, childDocSet, childSolrReturnFields, buildHierarchy, limit);
  }
  ...
  // NOTE: THIS FEATURE IS PRESENTLY EXPERIMENTAL; WAIT TO SEE IT IN THE REF GUIDE.  FINAL SYNTAX IS
  // TBD.
