...
/**
 * A merge policy that can detect schema changes and write docvalues into merging segments when a
 * field has docvalues enabled Using UninvertingReader.
 *
 * <p>This merge policy will delegate to the wrapped merge policy for selecting merge segments
 */
public class UninvertDocValuesMergePolicyFactory extends WrapperMergePolicyFactory {
  ...
  public UninvertDocValuesMergePolicyFactory(
  ...
  protected MergePolicy getMergePolicyInstance(MergePolicy wrappedMP) {
    return new OneMergeWrappingMergePolicy(
        wrappedMP, (merge) -> new UninvertDocValuesOneMerge(merge.segments));
  }
   ...
   *
   * <p>This is going to blow up FieldCache, look into an alternative implementation that uninverts
   * without fieldcache
   */
  private class UninvertingFilterCodecReader extends FilterCodecReader {
    ...
    public UninvertingFilterCodecReader(
        CodecReader in, Map<String, UninvertingReader.Type> uninversionMap) {
      super(in);

      this.uninvertingReader = UninvertingReader.wrap(in, uninversionMap::get);
      this.docValuesProducer =
