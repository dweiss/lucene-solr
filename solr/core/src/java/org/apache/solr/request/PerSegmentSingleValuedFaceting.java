...
class PerSegmentSingleValuedFaceting {
  ...
  public PerSegmentSingleValuedFaceting(
    ...
    this(
        ...
        sort,
        prefix,
        new SubstringBytesRefFilter(contains, ignoreCase));
  }
  ...
  NamedList<Integer> getFacetCounts(Executor executor) throws IOException {
      ...
      try {
        ...
        if (cause instanceof RuntimeException) {
          ...
          throw new SolrException(
              ...
              "Error in per-segment faceting on field: " + fieldName,
              cause);
        }
      }
    ...
    while (queue.size() > 0) {
      ...
      do {
        ...
        do {
          ++seg.pos;
        } while ((seg.pos < seg.endTermIndex) // stop incrementing before we run off the end
            && (seg.tenum.next() != null
                || true) // move term enum forward with position -- dont care about value
  ...
  private NamedList<Integer> finalize(
      NamedList<Integer> res, int missingCount, boolean hasMissingCount) throws IOException {
    if (missing) {
      if (!hasMissingCount) {
  ...
  class SegFacet {
    ...
    void countTerms() throws IOException {
      ...
      if (prefix != null) {
        ...
        // TODO: we could constrain the lower endpoint if we had a binarySearch method that allowed
        // passing start/end
