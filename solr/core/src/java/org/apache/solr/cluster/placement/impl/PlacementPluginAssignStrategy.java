...
public class PlacementPluginAssignStrategy implements Assign.AssignStrategy {
  ...
  private static final PlacementPlanFactoryImpl PLACEMENT_PLAN_FACTORY =
  ...
  /**
   * @param collection the collection for which this assign request is done. In theory would be
   *     better to pass it into the {@link #assign} call below (which would allow reusing instances
   *     of {@link PlacementPluginAssignStrategy}, but for now doing it here in order not to change
   *     the other Assign.AssignStrategy implementations.
   */
  ...
  public List<ReplicaPosition> assign(
      SolrCloudManager solrCloudManager, Assign.AssignRequest assignRequest)
    ...
    PlacementRequestImpl placementRequest =
        PlacementRequestImpl.toPlacementRequest(cluster, collection, assignRequest);
    ...
    try {
      placementPlan =
          plugin.computePlacement(
              cluster,
              placementRequest,
              new AttributeFetcherImpl(solrCloudManager),
              PLACEMENT_PLAN_FACTORY);
