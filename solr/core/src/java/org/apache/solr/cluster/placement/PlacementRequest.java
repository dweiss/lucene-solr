...
public interface PlacementRequest {
  ...
  /**
   * Shard name(s) for which new replicas placement should be computed. The shard(s) might exist or
   * not (that's why this method returns a {@link Set} of {@link String}'s and not directly a set of
   * {@link Shard} instances).
   *
   * <p>Note the Collection API allows specifying the shard name or a {@code _route_} parameter. The
   * Solr implementation will convert either specification into the relevant shard name so the
   ...
   *
   * <p>When Collection API calls do not specify a specific set of target nodes, replicas can be
   * placed on any live node of the cluster. In such cases, this set will be equal to the set of all
   ...
   *     placement would be possible and the Solr infrastructure driving the plugin code would
