...
/**
 * Implements random placement for new collection creation while preventing two replicas of same
 * shard from being placed on same node.
 *
...
public class SamplePluginRandomPlacement implements PlacementPlugin {
  ...
  public PlacementPlan computePlacement(
    ...
    for (String shardName : request.getShardNames()) {
      // Shuffle the nodes for each shard so that replicas for a shard are placed on distinct yet
      // random nodes
