...
/**
 * Implements placing replicas in a way that replicate past Autoscaling config defined <a
 ...
 * Configuration options could be introduced to allow configuration base option selection as well...
 *
 ...
 * </pre>
 *
 * <p>The consequence will be the creation of an element in the Zookeeper file {@code
 * /clusterprops.json} as follows:
 *
 * <pre>
 *
 * "placement-plugin":{
 ...
 * fallback to either Legacy or rule based placement if configured for a collection), execute:
 *
 * <pre>
 *
 * curl -X POST -H 'Content-type:application/json' -d '{
...
public class SamplePluginAffinityReplicaPlacement implements PlacementPlugin {
   ...
   * instances of {@link SamplePluginAffinityReplicaPlacement}.
   */
  public static class Factory implements PlacementPluginFactory {

    /**
     * Empty public constructor is used to instantiate this factory. Using a factory pattern to
     * allow the factory to do one time costly operations if needed, and to only have to instantiate
     * a default constructor class by name, rather than having to call a constructor with more
  ...
  /**
   * The factory has decoded the configuration for the plugin instance and passes it the parameters
   * it needs.
   */
  private SamplePluginAffinityReplicaPlacement(
      long minimalFreeDiskGB, long deprioritizedFreeDiskGB) {
  ...
  public PlacementPlan computePlacement(
    ...
    // Split the set of nodes into 3 sets of nodes accepting each replica type (sets can overlap if
    // nodes accept multiple replica types)
    // These subsets sets are actually maps, because we capture the number of cores (of any replica
    // type) present on each node.
    // Also get the number of currently existing cores per node, so we can keep update as we place
    // new cores to not end up
    // always selecting the same node(s).
    Pair<EnumMap<Replica.ReplicaType, Set<Node>>, Map<Node, Integer>> p =
        getNodesPerReplicaType(nodes, attrValues);

    EnumMap<Replica.ReplicaType, Set<Node>> replicaTypeToNodes = p.first();
    Map<Node, Integer> coresOnNodes = p.second();

    // All available zones of live nodes. Due to some nodes not being candidates for placement, and
    // some existing replicas
    // being one availability zones that might be offline (i.e. their nodes are not live), this set
    // might contain zones
    ...
    // Let's now iterate on all shards to create replicas for and start finding home sweet homes for
    // the replicas
    for (String shardName : request.getShardNames()) {
      // Iterate on the replica types in the enum order. We place more strategic replicas first
      // (NRT is more strategic than TLOG more strategic than PULL). This is in case we eventually
      // decide that less
  ...
  private ImmutableSet<String> getZonesFromNodes(
   ...
   * {@link #AVAILABILITY_ZONE_SYSPROP} to then return {@link #UNDEFINED_AVAILABILITY_ZONE} as the
   * AZ name.
   */
  ...
  /**
   * This class captures an availability zone and the nodes that are legitimate targets for replica
   * placement in that Availability Zone. Instances are used as values in a {@link TreeMap} in which
   * the total number of already existing replicas in the AZ is the key. This allows easily picking
   * the set of nodes from which to select a node for placement in order to balance the number of
   * replicas per AZ. Picking one of the nodes from the set is done using different criteria
   * unrelated to the Availability Zone (picking the node is based on the {@link
   * CoresAndDiskComparator} ordering).
   */
   ...
   * packaged and returned in an EnumMap keyed by replica type (1st member of the Pair). Also builds
   * the number of existing cores on each node present in the returned EnumMap (2nd member of the
   * returned Pair). Nodes for which the number of cores is not available for whatever reason are
   * excluded from acceptable candidate nodes as it would not be possible to make any meaningful
   * placement decisions.
   *
   ...
   *     #REPLICA_TYPE_SYSPROP} as well as the number of cores on each node.
   */
  private Pair<EnumMap<Replica.ReplicaType, Set<Node>>, Map<Node, Integer>> getNodesPerReplicaType(
      Set<Node> nodes, final AttributeValues attrValues) {
    ...
    for (Node node : nodes) {
      ...
      if (attrValues.getFreeDisk(node).get() < minimalFreeDiskGB) {
        if (log.isWarnEnabled()) {
          log.warn(
              ...
              node.getName(),
              attrValues.getFreeDisk(node).get(),
              minimalFreeDiskGB);
        }
        continue;
      }
      ...
      String supportedReplicaTypes =
          attrValues.getSystemProperty(node, REPLICA_TYPE_SYSPROP).isPresent()
              ? attrValues.getSystemProperty(node, REPLICA_TYPE_SYSPROP).get()
              : null;
      // If property not defined or is only whitespace on a node, assuming node can take any replica
      // type
   ...
   *   <li>Balance as much as possible the number of replicas of the given {@link
   ...
   *   <li>Place replicas is possible on nodes having more than a certain amount of free disk space
   *       (note that nodes with a too small amount of free disk space were eliminated as placement
   *       targets earlier, in {@link #getNodesPerReplicaType}). There's a threshold here rather
   *       than sorting on the amount of free disk space, because sorting on that value would in
   *       practice lead to never considering the number of cores on a node.
   *   <li>Place replicas on nodes having a smaller number of cores (the number of cores considered
   *       for this decision includes decisions made during the processing of the placement request)
   * </ol>
   */
  @SuppressForbidden(
      reason =
  ...
  private void makePlacementDecisions(
      ...
      String shardName,
      ImmutableSet<String> availabilityZones,
    ...
    // Count existing replicas per AZ. We count only instances the type of replica for which we need
    // to do placement. This
    ...
    if (shard != null) {
      ...
      for (Replica replica : shard.replicas()) {
        ...
        // The node's AZ has to be counted as having a replica if it has a replica of the same type
        // as the one we need
        // to place here (remove the "if" below to balance the number of replicas per AZ across all
        // replica types rather
        // than within each replica type, but then there's a risk that all NRT replicas for example
        // end up on the same AZ).
        // Note that if in the cluster nodes are configured to accept a single replica type and not
        // multiple ones, the
        // two options are equivalent (governed by system property AVAILABILITY_ZONE_SYSPROP on each
        // node)
        if (replica.getType() == replicaType) {
          final String az = getNodeAZ(replica.getNode(), attrValues);
          if (azToNumReplicas.containsKey(az)) {
            // We do not count replicas on AZ's for which we don't have any node to place on because
            // it would not help
            // the placement decision. If we did want to do that, note the dereferencing below can't
            // be assumed as the
    ...
    // We also counted for the shard and replica type under consideration how many replicas were per
    // AZ, so we can place
    // (or try to place) replicas on AZ's that have fewer replicas

    // Get the candidate nodes per AZ in order to build (further down) a mapping of AZ to placement
    // candidates.
    ...
    CoresAndDiskComparator coresAndDiskComparator =
        new CoresAndDiskComparator(attrValues, coresOnNodes, deprioritizedFreeDiskGB);

    // Now we have for each AZ on which we might have a chance of placing a replica, the list of
    // candidate nodes for replicas
    // (candidate: does not already have a replica of this shard and is in the corresponding AZ).
    // We must now select those of the nodes on which we actually place the replicas, and will do
    // that based on the total
    // number of cores already present on these nodes as well as the free disk space.
    // We sort once by the order related to number of cores and disk space each list of nodes on an
    // AZ. We do not sort all
    // of them ahead of time because we might be placing a small number of replicas and it might be
    // wasted work.
    for (int i = 0; i < numReplicas; i++) {
      // Pick the AZ having the lowest number of replicas for this shard, and if that AZ has
      // available nodes, pick the
      // most appropriate one (based on number of cores and disk space constraints). In the process,
      // remove entries (AZ's)
      ...
      for (Iterator<Map.Entry<Integer, AzWithNodes>> it = azByExistingReplicas.entries().iterator();
          it.hasNext(); ) {
        Map.Entry<Integer, AzWithNodes> entry = it.next();
        if (!entry.getValue().availableNodesForPlacement.isEmpty()) {
          azWithNodesEntry = entry;
          // Remove this entry. Will add it back after a node has been removed from the list of
          // available nodes and the number
          // of replicas on the AZ has been increased by one (search for "azByExistingReplicas.put"
          // below).
          it.remove();
          break;
        } else {
      ...
      if (!azWithNodes.hasBeenSorted) {
        // Make sure we do not tend to use always the same nodes (within an AZ) if all conditions
        // are identical (well, this
        // likely is not the case since after having added a replica to a node its number of cores
        // increases for the next
        // placement decision, but let's be defensive here, given that multiple concurrent placement
        // decisions might see
        // the same initial cluster state, and we want placement to be reasonable even in that case
        // without creating an
        ...
        // Sort by increasing number of cores but pushing nodes with low free disk space to the end
        // of the list
      ...
      // Insert back a corrected entry for the AZ: one more replica living there and one less node
      // that can accept new replicas
      ...
      // Track that the node has one more core. These values are only used during the current run of
      // the plugin.
  ...
  /**
   * Comparator implementing the placement strategy based on free space and number of cores: we want
   * to place new replicas on nodes with the less number of cores, but only if they do have enough
   * disk space (expressed as a threshold value).
   */
  static class CoresAndDiskComparator implements Comparator<Node> {
     ...
     * the attributes and configuration. The number of cores per node is passed in a map whereas the
     * free disk is fetched from the attributes due to the fact that we update the number of cores
     * per node as we do allocations, but we do not update the free disk. The attrValues
     * correpsonding to the number of cores per node are the initial values, but we want to comapre
     * the actual value taking into account placement decisions already made during the current
     * execution of the placement plugin.
     */
    CoresAndDiskComparator(
        AttributeValues attrValues, Map<Node, Integer> coresOnNodes, long deprioritizedFreeDiskGB) {
