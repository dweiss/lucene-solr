...
public class AttributeFetcherImpl implements AttributeFetcher {
  ...
  public AttributeValues fetchAttributes() {
    ...
    if (requestedNodeCoreCount) {
      allSnitchTagsToInsertion.put(
          ...
          (node, value) -> nodeToCoreCount.put(node, ((Number) value).intValue()));
    }
    if (requestedNodeDiskType) {
      allSnitchTagsToInsertion.put(
          ImplicitSnitch.DISKTYPE,
          (node, value) -> {
    ...
    if (requestedNodeFreeDisk) {
      allSnitchTagsToInsertion.put(
          ...
          (node, value) ->
              nodeToFreeDisk.put(node, ((Number) value).longValue() / 1024 / 1024 / 1024));
    }
    ...
    for (String metricSnitch : requestedNodeMetricSnitchTags) {
      ...
      allSnitchTagsToInsertion.put(
          metricSnitch, (node, value) -> metricMap.put(node, (Double) value));
    }
  ...
  static String getMetricSnitchTag(String metricName, NodeMetricRegistry registry) {
    return SolrClientNodeStateProvider.METRICS_PREFIX
        + SolrMetricManager.getRegistryName(getGroupFromMetricRegistry(registry), metricName);
  }
