...
public abstract class DirectoryFactory implements NamedListInitializedPlugin, Closeable {
  ...
  // Stayed away from upper bounds of the int/long in case any other code tried to aggregate these
  // numbers.
   ...
   * On close all directories will be closed whether this has been called or not. This is simply to
   * allow early cleanup.
   *
   ...
   *
   * @param rawLockType A string value as passed in config. Every factory should at least support
   *     'none' to disable locking.
   * @throws IOException If there is a low-level I/O error.
   */
  protected abstract LockFactory createLockFactory(String rawLockType) throws IOException;

  /**
   * Returns true if a Directory exists for a given path in the underlying (stable) storage
   * <em>and</em> contains at least one file. Note that the existence of a {@link Directory}
   * <em>Object</em> as returned by a previous call to the {@link #get} method (on the specified
   * <code>path</code>) is not enough to cause this method to return true. Some prior user of that
   * Directory must have written &amp; synced at least one file to that Directory (and at least one
   ...
   * only makes sense for persistent directory factories.
   *
  ...
  /**
   * Returns the Directory for a given path, using the specified rawLockType. Will return the same
   * Directory instance for the same path.
   *
   * @throws IOException If there is a low-level I/O error.
   */
  ...
  /**
   * Increment the number of references to the given Directory. You must call release for every call
   * to this method.
   */
  ...
  /**
   * If your implementation can count on delete-on-last-close semantics or throws an exception when
   * trying to remove a file in use, return false (eg NFS). Otherwise, return true. Defaults to
   * returning false.
   *
   * @return true if factory impl requires that Searcher's explicitly reserve commit points.
   */
  ...
  public String getDataHome(CoreDescriptor cd) throws IOException {
    ...
    if (dataHomePath != null) {
      Path instanceDirLastPath =
          cd.getInstanceDir().getName(cd.getInstanceDir().getNameCount() - 1);
  ...
  /**
   * Create a new DirectoryFactory instance from the given SolrConfig and tied to the specified core
   * container.
   */
  static DirectoryFactory loadDirectoryFactory(
      SolrConfig config, CoreContainer cc, String registryName) {
