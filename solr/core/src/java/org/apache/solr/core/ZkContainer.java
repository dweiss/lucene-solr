    ...
    if (zookeeperHost != null) {
      ...
      try {
        ...
        boolean alreadyUsedChroot =
            cc.getConfig().isFromZookeeper() && zookeeperHost.equals(System.getProperty("zkHost"));
        if (!alreadyUsedChroot
            && !ZkController.checkChrootPath(
                zookeeperHost, (confDir != null) || boostrapConf || zkRunOnly)) {
        ...
        if (zkRun != null) {
          ...
          if (zkServer.getServers().size() > 1 && confDir == null && boostrapConf == false) {
            // we are part of an ensemble and we are not uploading the config - pause to give the
            // config time
            // to get up
            Thread.sleep(10000);
          }
