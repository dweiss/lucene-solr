...
public class SolrLogLayout extends AbstractStringLayout {
  ...
  @PluginFactory
  public static SolrLogLayout createLayout(
      @PluginAttribute(value = "charset", defaultString = "UTF-8") Charset charset) {
    return new SolrLogLayout(charset);
  }

  /**
   * Add this interface to a thread group and the string returned by getTag() will appear in log
   * statements of any threads under that group.
   */
  ...
  @SuppressForbidden(
      reason =
          "Need currentTimeMillis to compare against log event timestamp. "
  ...
  public String _format(LogEvent event) {
    ...
    String shortClassName =
        getShortClassName(event.getSource().getClassName(), event.getSource().getMethodName());
  ...
  public void setShorterFormat() {
    ...
    methodAlias.put(
        new Method("org.apache.solr.update.processor.LogUpdateProcessor", "finish"), "");
  }
