...
public class PayloadUtils {
  ...
  public static String getPayloadEncoder(FieldType fieldType) {
    // TODO: support custom payload encoding fields too somehow - maybe someone has a custom
  ...
  public static PayloadDecoder getPayloadDecoder(FieldType fieldType) {
    ...
    if ("integer".equals(encoder)) {
      decoder =
          (BytesRef payload) ->
              payload == null ? 1 : PayloadHelper.decodeInt(payload.bytes, payload.offset);
    }
    if ("float".equals(encoder)) {
      decoder =
          (BytesRef payload) ->
              payload == null ? 1 : PayloadHelper.decodeFloat(payload.bytes, payload.offset);
    }
    // encoder could be "identity" at this point, in the case of DelimitedTokenFilterFactory
    // encoder="identity"
  ...
  /**
   * The generated SpanQuery will be either a SpanTermQuery or an ordered, zero slop SpanNearQuery,
   * depending on how many tokens are emitted.
   */
  public static SpanQuery createSpanQuery(
      String field, String value, Analyzer analyzer, String operator) throws IOException {
    // adapted this from QueryBuilder.createSpanQuery (which isn't currently public) and added
    // reset(), end(), and close() calls
