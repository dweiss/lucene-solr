          ...
          try {
            ...
            switch (cmd) {
              ...
              case "add-key":
                ...
                repositoryManager.addKey(
                    FileUtils.readFileToByteArray(new File(keyFilename)),
                    Paths.get(keyFilename).getFileName().toString());
                break;
              case "list-installed":
  ...
  public Option[] getOptions() {
    return new Option[] {
      ...
      Option.builder("cluster")
          .required(false)
          .desc(
              "Needed to install cluster level plugins in a package. Run './solr package help' for more details.")
          .build(),
      Option.builder("p")
          .argName("PARAMS")
          .hasArgs()
          .required(false)
          .desc(
              "List of parameters to be used with deploy command. Run './solr package help' for more details.")
          .longOpt("param")
          .build(),
      Option.builder("u")
          .required(false)
          .desc(
              "If a deployment is an update over a previous deployment. Run './solr package help' for more details.")
          .longOpt("update")
          .build(),
      Option.builder("c")
          .required(false)
