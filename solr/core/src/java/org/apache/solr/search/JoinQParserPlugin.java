...
public class JoinQParserPlugin extends QParserPlugin {
  ...
  private static class JoinParams {
    ...
    public JoinParams(
        String fromField, String fromCore, Query fromQuery, long fromCoreOpenTime, String toField) {
  ...
  private enum Method {
    ...
    dvWithScore {
      @Override
      Query makeFilter(QParser qparser, JoinQParserPlugin plugin) throws SyntaxError {
        return new ScoreJoinQParserPlugin()
      ...
      @Override
      Query makeJoinDirectFromParams(JoinParams jParams) {
        return ScoreJoinQParserPlugin.createJoinQuery(
            jParams.fromQuery, jParams.fromField, jParams.toField, ScoreMode.None);
      }
    },
    topLevelDV {
      @Override
    ...
    crossCollection {
      @Override
      Query makeFilter(QParser qparser, JoinQParserPlugin plugin) throws SyntaxError {
        return new CrossCollectionJoinQParser(
                qparser.qstr,
                qparser.localParams,
                qparser.params,
                qparser.req,
    ...
    Query makeJoinDirectFromParams(JoinParams jParams) {
      throw new IllegalStateException(
          "Join method [" + name() + "] doesn't support qparser-less creation");
    }
  ...
  private static final EnumSet<Method> JOIN_METHOD_WHITELIST =
   ...
   * @param method indicates which implementation should be used to process the join. Currently only
   *     'index', 'dvWithScore', and 'topLevelDV' are supported.
   */
  public static Query createJoinQuery(
      Query subQuery, String fromField, String toField, String method) {
    ...
    if (!JOIN_METHOD_WHITELIST.contains(joinMethod)) {
      // TODO Throw something that the callers here (FacetRequest) can catch and produce a more
      // domain-appropriate error message for?
      throw new SolrException(
  ...
  private static Method parseMethodString(String method) {
    try {
      ...
      throw new SolrException(
          SolrException.ErrorCode.BAD_REQUEST,
          "Provided join method '" + method + "' not supported");
    }
  }
}
