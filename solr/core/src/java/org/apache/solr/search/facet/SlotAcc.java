...
public abstract class SlotAcc implements Closeable {
  ...
  abstract static class LongFuncSlotAcc extends FuncSlotAcc {
    ...
    public LongFuncSlotAcc(
        ValueSource values, FacetContext fcontext, int numSlots, long initialValue) {
      super(values, fcontext, numSlots);
      this.initialValue = initialValue;
  ...
  /**
   * Implemented by some SlotAccs if they are capable of being used for sweep collecting in
   * compatible facet processors
   *
   * @see FacetFieldProcessor#registerSweepingAccIfSupportedByCollectAcc()
   */
  static interface SweepableSlotAcc<T extends SlotAcc> {
    /**
     * Called by processors if they support sweeping. Implementations will often return self or null
     * (the latter indicating that all necessary collection will be covered by the "sweeping" data
     * structures registered with the specified baseSweepingAcc as a result of the call to this
     * method).
     *
     ...
     * take care that the returned {@link SlotAcc} is different from the {@link SlotAcc} registered
     * for the purpose of output -- with the former overriding {@link
     * SlotAcc#setValues(SimpleOrderedMap, int)} as a no-op, to prevent setting duplicate values.
     *
     * @param baseSweepingAcc - never null, where the SlotAcc may register domains for sweep
     *     collection, and must register mappings of new read-access SlotAccs that result from this
     *     call.
  ...
  /**
   * A simple data structure to {@link DocSet} domains with an associated {@link CountSlotAcc}. This
   * may be used to support sweep count accumulation over different {@link DocSet} domains, but the
   * concept is perfectly applicable to encapsulating the relevant state for simple "non-sweep"
   * collection as well (in which case {@link SweepCountAccStruct#docSet} would be {@link
   * FacetContext#base}, {@link SweepCountAccStruct#countAcc} would be {@link
   * FacetProcessor#countAcc}, and {@link SweepCountAccStruct#isBase} would trivially be "true").
   */
  ...
  static class SweepingCountSlotAcc extends CountSlotArrAcc {
    ...
    /**
     * When a {@link SweepableSlotAcc} replaces itself (for the purpose of collection) with a
     * different {@link SlotAcc} instance, it must register that replacement by calling this method
     * with itself as the fromAcc param, and with the new replacement {@link SlotAcc} as the toAcc
     * param. The two SlotAccs must have the same {@link SlotAcc#key}.
     *
     ...
     * {@link SlotAcc#setValues(SimpleOrderedMap, int)} method) is provided via this instance's
     * {@link #setValues(SimpleOrderedMap, int)} method.
     *
    ...
    /**
     * Populates the bucket with the values from all mapped "output" accumulators for the specified
     * slot.
     *
     * <p>This method exists because there are some contexts (namely SpecialSlotAcc, for allBuckets,
     * etc.) in which "base" count is tracked differently, via getSpecialCount(). For such cases, we
     * need a method that allows the caller to directly coordinate calling {@link SlotAcc#setValues}
     * on the sweeping output accs, while avoiding the inclusion of {@link CountSlotAcc#setValues
     * CountSlotAcc.setValues}
     */
     ...
     * processor is not using sweeping
     *
     * @returns empty list unless the {@link FacetProcessor#countAcc} is a {@link
