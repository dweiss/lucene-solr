...
/**
 * Finds documents whose specified field has any of the specified values. It's like {@link
 * TermQParserPlugin} but multi-valued, and supports a variety of internal algorithms. <br>
 * Parameters: <br>
 * <code>f</code>: The field name (mandatory) <br>
 * <code>separator</code>: the separator delimiting the values in the query string, defaulting to a
 * comma. If it's a " " then it splits on any consecutive whitespace. <br>
 * <code>method</code>: Any of termsFilter (default), booleanQuery, automaton, docValuesTermsFilter.
 *
 * <p>Note that if no values are specified then the query matches no documents.
 */
  ...
  private static enum Method {
    ...
    docValuesTermsFilter { // on 4x this is FieldCacheTermsFilter but we use the 5x name any way
      ...
      Query makeFilter(String fname, BytesRef[] byteRefs) {
        ...
        return (byteRefs.length > 700)
            ? docValuesTermsFilterTopLevel.makeFilter(fname, byteRefs)
            : docValuesTermsFilterPerSegment.makeFilter(fname, byteRefs);
      }
    },
  ...
  public QParser createParser(
    ...
    return new QParser(qstr, localParams, params, req) {
      ...
      public Query parse() throws SyntaxError {
        ...
        if (sepIsSpace) qstr = qstr.trim();
        if (qstr.length() == 0) return new MatchNoDocsQuery();
        final String[] splitVals =
