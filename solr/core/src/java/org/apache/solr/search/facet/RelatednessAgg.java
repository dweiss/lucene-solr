 ...
 *
 * @see <a href="https://arxiv.org/pdf/1609.00464.pdf">The Semantic Knowledge Graph: A compact,
...
public class RelatednessAgg extends AggValueSource {
  ...
  private static final class SweepSKGSlotAcc extends SlotAcc {
    ...
    @Override
    public void collect(int perSegDocId, int slot, IntFunction<SlotContext> slotContext)
        throws IOException {
  ...
  private static class BucketData implements Comparable<BucketData> {
     ...
     * represent the <code>allBuckets</code> bucket.
     *
     * @see #incCounts
     */
    ...
    /**
     * Increment both the foreground &amp; background <em>counts</em> for the current bucket,
   ...
   * may give a missleading impression of added precision.
   */
