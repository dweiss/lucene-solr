 ...
 * "maintain" means, given the current state of the alias and some information from a routed field
 * in a document that may imply a need for changes, create, delete or otherwise modify collections
 * as required.
 */
abstract class AliasCmd implements OverseerCollectionMessageHandler.Cmd {
  ...
  static NamedList createCollectionAndWait(
    ...
    try {
      ...
      ocmh.commandMap
          .get(CollectionParams.CollectionAction.CREATE)
          .call(clusterState, new ZkNodeProps(createMsgMap), results);
    ...
    CollectionsHandler.waitForActiveCollection(
        createCollName, ocmh.overseer.getCoreContainer(), new OverseerSolrResponse(results));
    CollectionProperties collectionProperties =
        new CollectionProperties(ocmh.zkStateReader.getZkClient());
    collectionProperties.setCollectionProperty(
        createCollName, ROUTED_ALIAS_NAME_CORE_PROP, aliasName);
    while (!ocmh.zkStateReader
        .getCollectionProperties(createCollName, 1000)
        .containsKey(ROUTED_ALIAS_NAME_CORE_PROP)) {
      Thread.sleep(50);
    }
    return results;
  }
