...
public class SetAliasPropCmd implements Cmd {
  ...
  public void call(
    ...
    // Ensure we see the alias.  This may be redundant but SetAliasPropCmd isn't expected to be
    ...
    if (aliasesManager.getAliases().getCollectionAliasMap().get(aliasName) == null) {
      ...
      throw new SolrException(
          BAD_REQUEST, String.format(Locale.ROOT, "Can't modify non-existent alias %s", aliasName));
    }

    @SuppressWarnings("unchecked")
    Map<String, String> properties =
        new LinkedHashMap<>((Map<String, String>) message.get(PROPERTIES));
