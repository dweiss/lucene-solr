...
public class MaintainRoutedAliasCmd extends AliasCmd {
  ...
  public void call(
    ...
    for (RoutedAlias.Action action : actions) {
      boolean exists =
          ocmh.zkStateReader.getClusterState().getCollectionOrNull(action.targetCollection) != null;
          ...
          if (!exists) {
            ...
            if (!ra.getCollectionList(aliases).contains(action.targetCollection)) {
              ...
              Map<String, String> collectionProperties =
                  ocmh.zkStateReader.getCollectionProperties(action.targetCollection, 1000);
              if (!collectionProperties.containsKey(RoutedAlias.ROUTED_ALIAS_NAME_CORE_PROP)) {
                ...
                props.setCollectionProperty(
                    action.targetCollection, RoutedAlias.ROUTED_ALIAS_NAME_CORE_PROP, aliasName);
              }
            }
          }
          break;
  ...
  public void addTargetCollection(
    ...
    NamedList createResults =
        createCollectionAndWait(
            clusterState, aliasName, aliasMetadata, action.targetCollection, ocmh);
