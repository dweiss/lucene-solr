...
public class ReindexCollectionCmd implements OverseerCollectionMessageHandler.Cmd {
  ...
  public void call(
    ...
    Map<String, Object> reindexingState =
        getReindexingState(ocmh.cloudManager.getDistribStateManager(), collection);
    ...
    String configName =
        message.getStr(
            ZkStateReader.CONFIGNAME_PROP, ocmh.zkStateReader.readConfigName(collection));
    ...
    try {
      ...
      if (clusterState.hasCollection(chkCollection)) {
        ...
        cmd =
            new ZkNodeProps(
                ...
                CommonParams.NAME,
                chkCollection,
                CoreAdminParams.DELETE_METRICS_HISTORY,
                "true");
        ocmh.commandMap
            .get(CollectionParams.CollectionAction.DELETE)
            .call(clusterState, cmd, cmdResults);
      ...
      if (sameTarget) {
        ...
        cmdResults = new NamedList<>();
        ocmh.commandMap
            .get(CollectionParams.CollectionAction.CREATEALIAS)
            .call(clusterState, cmd, cmdResults);
        ocmh.checkResults(
  ...
  private long getNumberOfDocs(String collection) {
    CloudSolrClient solrClient =
        ocmh.overseer.getCoreContainer().getSolrClientCache().getCloudSolrClient(zkHost);
    try {
  ...
  private void waitForDaemon(
      ...
      throws Exception {
    HttpClient client =
        ocmh.overseer.getCoreContainer().getUpdateShardHandler().getDefaultHttpClient();
