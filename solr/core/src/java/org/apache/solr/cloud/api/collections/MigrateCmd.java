  ...
  public void call(
    ...
    if (followAliases) {
      sourceCollectionName =
          ocmh.cloudManager.getClusterStateProvider().resolveSimpleAlias(extSourceCollectionName);
      targetCollectionName =
          ocmh.cloudManager.getClusterStateProvider().resolveSimpleAlias(extTargetCollectionName);
    } else {
  ...
  private void migrateKey(
    ...
    if (clusterState.hasCollection(tempSourceCollectionName)) {
      ...
      Map<String, Object> props =
          makeMap(Overseer.QUEUE_OPERATION, DELETE.toLower(), NAME, tempSourceCollectionName);
    ...
    DocRouter.Range splitRange =
        ocmh.intersect(
            targetSlice.getRange(), ocmh.intersect(sourceSlice.getRange(), keyHashRange));
    if (splitRange == null) {
    ...
    String coreNodeName =
        ocmh.waitForCoreNodeName(
            tempSourceCollectionName, sourceLeader.getNodeName(), tempCollectionReplica1);
    ...
    String tempCollectionReplica2 =
        Assign.buildSolrCoreName(
            ocmh.overseer.getSolrCloudManager().getDistribStateManager(),
    ...
    ((AddReplicaCmd) ocmh.commandMap.get(ADDREPLICA))
