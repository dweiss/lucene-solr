...
public class AddReplicaCmd implements OverseerCollectionMessageHandler.Cmd {
  ...
  List<ZkNodeProps> addReplica(
    ...
    if (followAliases) {
      collectionName =
          ocmh.cloudManager.getClusterStateProvider().resolveSimpleAlias(extCollectionName);
    } else {
    ...
    List<CreateReplica> createReplicas =
        buildReplicaPositions(
                ocmh.cloudManager, clusterState, collectionName, message, replicaTypesVsCount)
            .stream()
            .map(
                replicaPosition ->
                    assignReplicaDetails(ocmh.cloudManager, clusterState, message, replicaPosition))
  ...
  private ModifiableSolrParams getReplicaParams(
    ...
    if (coll.getStr(WITH_COLLECTION) != null) {
      ...
      if (withCollection.getActiveSlices().size() > 1) {
        throw new SolrException(
            SolrException.ErrorCode.BAD_REQUEST,
            "The `withCollection` must have only one shard, found: "
                + withCollection.getActiveSlices().size());
      }
      ...
      if (replicas == null || replicas.isEmpty()) {
        ...
        ZkNodeProps props =
            new ZkNodeProps(
                ...
                "node",
                createReplica.node,
                // since we already computed node assignments (which include assigning a node for
                // this withCollection replica) we want to skip the assignment step
                CollectionAdminParams.SKIP_NODE_ASSIGNMENT,
                "true",
                CommonAdminParams.WAIT_FOR_FINAL_STATE,
                Boolean.TRUE
                    .toString()); // set to true because we want `withCollection` to be ready after
        // this collection is created
    ...
    params.set(
        ...
        ocmh.waitToSeeReplicasInState(
                collectionName, Collections.singletonList(createReplica.coreName))
