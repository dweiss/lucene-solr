...
public class TimeRoutedAlias extends RoutedAlias {
  ...
  public static final Set<String> REQUIRED_ROUTER_PARAMS =
      Set.of(CommonParams.NAME, ROUTER_TYPE_NAME, ROUTER_FIELD, ROUTER_START, ROUTER_INTERVAL);
  ...
  static final DateTimeFormatter DATE_TIME_FORMATTER =
      new DateTimeFormatterBuilder()
          ...
          .appendPattern("[_HH[_mm[_ss]]]") // brackets mean optional
          .parseDefaulting(ChronoField.HOUR_OF_DAY, 0)
          .parseDefaulting(ChronoField.MINUTE_OF_HOUR, 0)
          .parseDefaulting(ChronoField.SECOND_OF_MINUTE, 0)
          .toFormatter(Locale.ROOT)
          .withZone(ZoneOffset.UTC); // deliberate -- collection names disregard TZ
  ...
  TimeRoutedAlias(String aliasName, Map<String, String> aliasMetadata) throws SolrException {
    ...
    if (!aliasMetadata.keySet().containsAll(TimeRoutedAlias.REQUIRED_ROUTER_PARAMS)) {
      throw new SolrException(
          BAD_REQUEST,
          "A time routed alias requires these params: "
              + TimeRoutedAlias.REQUIRED_ROUTER_PARAMS
    ...
    if (!"time".equals(type)) {
      throw new SolrException(
          ...
          "Only 'time' routed aliases is supported by TimeRoutedAlias, found:" + type);
    }
    ...
    // the date math configured is an interval to be subtracted from the most recent collection's
    // time stamp
    ...
    if (autoDeleteAgeMath != null) {
      try {
        ...
        if (now.before(before)) {
          throw new SolrException(
              BAD_REQUEST, "duration must round or subtract to produce a time in the past");
        }
      } catch (Exception e) {
        throw new SolrException(
            BAD_REQUEST, "bad " + TimeRoutedAlias.ROUTER_AUTO_DELETE_AGE + ", " + e, e);
      }
    }
    if (preemptiveCreateMath != null) {
      try {
        ...
        throw new SolrException(
            BAD_REQUEST, "Invalid date math for preemptiveCreateMath:" + preemptiveCreateMath);
      }
    }
  ...
  public static Instant parseInstantFromCollectionName(String aliasName, String collection) {
    ...
    if (collection.contains(separatorPrefix)) {
      dateTimePart =
          collection.substring(collection.lastIndexOf(separatorPrefix) + separatorPrefix.length());
    } else {
  ...
  public void validateRouteValue(AddUpdateCommand cmd) throws SolrException {
    ...
    if (docTimestamp.isBefore(startTime)) {
      throw new SolrException(
          BAD_REQUEST,
          "The document couldn't be routed because "
              + docTimestamp
              + " is before the start time for this alias "
              + start
              + ")");
    }
  }
  ...
  private Instant calcPreemptNextColCreateTime(
      String preemptiveCreateMath, Instant nextCollTimestamp) {
    ...
    try {
      ...
      throw new SolrException(
          SolrException.ErrorCode.BAD_REQUEST,
          "Invalid Preemptive Create Window Math:'" + preemptiveCreateMath + '\'',
          e);
    }
  }
  ...
  private Instant parseRouteKey(Object routeKey) {
    ...
    if (routeKey instanceof Instant) {
      ...
      throw new SolrException(
          SolrException.ErrorCode.BAD_REQUEST, "Unexpected type of routeKey: " + routeKey);
    }
    return docTimestamp;
  }
        ...
        if (!docTimestamp.isBefore(colStartTime)) { // (inclusive lower bound)
          ...
          if (i == 0) {
            if (docTimestamp.isBefore(next)) { // (exclusive upper bound)
              ...
              while (!docTimestamp.isBefore(possibleDestTime)
                  || docTimestamp.equals(possibleDestTime)) {
                colDestTime = possibleDestTime;
                possibleDestTime = computeNextCollTimestamp(colDestTime);
              }
              String destCol =
                  TimeRoutedAlias.formatCollectionNameFromInstant(getAliasName(), colDestTime);
          ...
          if (candidate.getCreationType() == NONE
            ...
            // preemptive creation?
            Instant time2Create =
                calcPreemptNextColCreateTime(
                    getPreemptiveCreateWindow(), computeNextCollTimestamp(mostRecentCol));
            if (!docTimestamp.isBefore(time2Create)) {
              String destinationCollection =
                  candidate.getDestinationCollection(); // dest doesn't change
              String creationCollection = calcNextCollection(mostRecentCol);
              return new CandidateCollection(
                  ASYNC_PREEMPTIVE, // add next collection
  ...
  private List<Action> calcDeletes(List<Action> actions) {
    ...
    for (Map.Entry<Instant, String> parsedCollection : parsedCollectionsDesc) {
      ...
      if (colInstant.isBefore(delBefore) || colInstant.equals(delBefore)) {
        if (log.isDebugEnabled()) { // don't perform formatting unless debugging
          ...
          if (log.isDebugEnabled()) {
            log.debug(
                "{} is equal to or before {} deletions may be required",
                dtf.format(colInstant),
                dtf.format(delBefore));
          }
        }
        break;
      } else {
        if (log.isDebugEnabled()) { // don't perform formatting unless debugging
          ...
          if (log.isDebugEnabled()) {
            log.debug(
                "{} is not before {} and will be retained",
                dtf.format(colInstant),
                dtf.format(delBefore));
          }
        }
    ...
    final List<String> targetList =
        parsedCollectionsDesc.stream().map(Map.Entry::getValue).collect(Collectors.toList());
