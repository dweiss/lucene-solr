...
public abstract class ManagedResource {
  ...
  /**
   * Initializes this managed resource, including setting up JSON-based storage using the provided
   * storageIO implementation, such as ZK.
   */
  ...
  /**
   * Called once during core initialization to get the managed data loaded from storage and notify
   * observers.
   */
  ...
  /**
   * Notifies all registered observers that the ManagedResource is initialized. This event only
   * occurs once when the core is loaded. Thus, you need to reload the core to get updates applied
   * to the analysis components that depend on the ManagedResource data.
   */
  protected void notifyObserversDuringInit(
      NamedList<?> args, Collection<ManagedResourceObserver> observers) throws SolrException {
   ...
   * implementation. The default implementation uses JSON as the storage format and delegates the
   * loading and saving of JSON bytes to the supplied StorageIO class.
   */
  ...
  /**
   * Gets the ServerResource class to register this endpoint with the Rest API router; in most
   * cases, the default RestManager.ManagedEndpoint class is sufficient but ManagedResource
   * implementations can override this method if a different ServerResource class is needed.
   */
  ...
  /**
   * Called from {@link #doPut(BaseSolrResource,Object)} to update this resource's init args using
   * the given updatedArgs
   */
  ...
  /**
   * Method called after data has been loaded from storage to give the concrete implementation a
   * chance to post-process the data.
   */
  ...
  public String getUpdatedSinceInitialization() {
    return lastUpdateSinceInitialization == null
        ? null
        : lastUpdateSinceInitialization.toInstant().toString();
  }
    ...
    if (managedData != null) {
      if (managedData instanceof List || managedData instanceof Set) {
        ...
        throw new IllegalArgumentException(
            "Invalid managed data type "
                + managedData.getClass().getName()
  ...
  /**
   * Just calls {@link #doPut(BaseSolrResource,Object)}; override to change the behavior of POST
   * handling.
   */
  ...
  public synchronized void doPut(BaseSolrResource endpoint, Object json) {
    ...
    if (log.isInfoEnabled()) {
      log.info(
          "Processing update to {}: {} is a {}", getResourceId(), json, json.getClass().getName());
    }
    ...
    if (json instanceof Map) {
      ...
      throw new SolrException(
          ErrorCode.BAD_REQUEST, "Unsupported update format " + json.getClass().getName());
    }
