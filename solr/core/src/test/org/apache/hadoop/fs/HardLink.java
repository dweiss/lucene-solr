...
/**
 * Class for creating hardlinks. Supports Unix/Linux, Windows via winutils , and Mac OS X.
 *
 * <p>The HardLink class was formerly a static inner class of FSUtil, and the methods provided were
 * blatantly non-thread-safe. To enable volume-parallel Update snapshots, we now provide static
 * threadsafe methods that allocate new buffer string arrays upon each call. We also provide an API
 * to hardlink all files in a directory with a single command, which is up to 128 times more
...
public class HardLink {
  ...
  /**
   * This abstract class bridges the OS-dependent implementations of the needed functionality for
   * querying link counts. The particular implementation class is chosen during static
   * initialization phase of the HardLink class. The "getter" methods construct shell command
   * strings.
   */
  ...
  /**
   * Creates hardlinks from multiple existing files within one parent directory, into one target
   * directory.
   *
   * @param parentDir - directory containing source files
   * @param fileBaseNames - list of path-less file names, as returned by parentDir.list()
  ...
  public static void createHardLinkMult(File parentDir, String[] fileBaseNames, File linkDir)
    ...
    if (fileBaseNames == null) {
      throw new IOException(
          "invalid arguments to createHardLinkMult: " + "filename list can be empty but not null");
    }
    if (!linkDir.exists()) {
  ...
  private static IOException createIOException(
      File f, String message, String error, int exitvalue, Exception cause) {

    final String s =
        "Failed to get link count on file "
            + f
            + ": message="
            + message
            + "; error="
  ...
  /**
   * HardLink statistics counters and methods. Not multi-thread safe, obviously. Init is called
   * during HardLink instantiation, above.
   *
   * <p>These are intended for use by knowledgeable clients, not internally, because many of the
   * internal methods are static and can't update these per-instance counters.
   */
  public static class LinkStats {
    ...
    public String report() {
      return "HardLinkStats: "
          ...
          + countMultLinks
          + " multi-Link operations, linking "
          + countFilesMultLinks
          + " files, total "
          + (countSingleLinks + countFilesMultLinks)
