...
public class UninvertDocValuesMergePolicyTest extends SolrTestCaseJ4 {
  ...
  public static void beforeTests() throws Exception {
    System.setProperty(
        SOLR_TESTS_SKIP_INTEGRITY_CHECK, (random().nextBoolean() ? "true" : "false"));
  }
        ...
        topReader -> {
          ...
          for (LeafReaderContext ctx : topReader.leaves()) {
            ...
            for (int i = 0; i < r.numDocs(); ++i) {
              ...
              assertEquals(
                  DocValuesType.SORTED, r.getFieldInfos().fieldInfo(TEST_FIELD).getDocValuesType());
              assertEquals(
                  DocValuesType.NONE, r.getFieldInfos().fieldInfo(ID_FIELD).getDocValuesType());
              assertEquals(v, id);
  ...
  private static void implUpdateSchemaField(
      TestHarness h, String fieldName, IntUnaryOperator propertiesModifier) {
