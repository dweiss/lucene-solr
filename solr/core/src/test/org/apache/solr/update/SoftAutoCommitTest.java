 ...
 *
 * <p>AutoCommitTest is an abomination that is way to brittle in how it tries to check that commits
 * happened, and when they happened. The goal of this test class is to (ultimately) completely
 * replace all of the functionality of that test class using:
 *
 * <ul>
 *   <li>A more robust monitor of commit/newSearcher events that records the times of those events
 *       in a queue that can be polled. Multiple events in rapid succession are not lost.
 *   <li>Timing checks that are forgiving of slow machines and use knowledge of how slow A-&gt;B was
...
public class SoftAutoCommitTest extends SolrTestCaseJ4 {
  ...
  public void testSoftAndHardCommitMaxDocs() throws Exception {
    ...
    assertTrue(
        "softCommit @ "
            + firstSoftNanos
            + "ns is before the maxDocs should have triggered it @ "
            + minSoftCommitNanos
            + "ns",
    ...
    assertNull("Got a hard commit we weren't expecting", monitor.hard.poll(1000, MILLISECONDS));
  ...
  private void doTestSoftAndHardCommitMaxTimeMixedAdds(final CommitWithinType commitWithinType)
    ...
    assertTrue(
        "soft529 occurred too fast, in "
            + soft529Ms
            + "ms, less than soft commit interval "
    ...
    assertTrue(
        "hard529 occurred too fast, in "
            + hard529Ms
            + "ms, less than hard commit interval "
    ...
    final long softCommitToSearcherOpenMs =
        TimeUnit.MILLISECONDS.convert(searcher529 - soft529, TimeUnit.NANOSECONDS);
    assertTrue(
        "searcher529 wasn't soon enough after soft529: Took "
            + softCommitToSearcherOpenMs
            + "ms, >= acceptable "
            + slowTestFudge
            + "ms (fudge)",
    ...
    assertTrue(
        "hard529 was before searcher529: " + searcher529 + " !<= " + hard529,
        searcher529 <= hard529);
    ...
    assertEquals(
        "Tracker reports too many hard commits",
        (null == hard530 ? 1 : 2),
    ...
    if (null != soft530) {
      assertEquals(
          "Tracker reports too many soft commits",
          2,
          softTracker.getCommitCount() - startingSoftCommits);
      if (null != hard530) {
        assertTrue("soft530 after hard530: " + soft530 + " !<= " + hard530, soft530 <= hard530);
      } else {
        assertTrue(
            "soft530 after hard529 but no hard530: " + soft530 + " !<= " + hard529,
            soft530 <= hard529);
      }
    } else {
      assertEquals(
    ...
    if (null != soft530 || null != hard530) {
      assertNotNull(
          "at least one extra commit for 530, but no searcher",
  ...
  public void doTestSoftAndHardCommitMaxTimeRapidAdds(final CommitWithinType commitWithinType)
    ...
    // even if evenly divisible, need +1 to account for possible "last" commit triggered by "last"
    // doc
    ...
    monitor.assertSaneOffers();
    assertRapidMultiCommitQueues(
        "softCommit", preFirstNanos, softCommitWaitMillis, maxExpectedSoft, monitor.soft);
    monitor.assertSaneOffers();
    assertRapidMultiCommitQueues(
        "hardCommit", preFirstNanos, hardCommitWaitMillis, maxExpectedHard, monitor.hard);
  ...
  private static void assertRapidMultiCommitQueues(
      ...
      final long commitWaitMillis,
      final long maxNumCommits,
  ...
  private static enum CommitWithinType {
    NONE {
      @Override
      public int useValue(final int softCommitWaitMillis, final int hardCommitWaitMillis) {
        return -1;
      }
    },
    SOFT {
