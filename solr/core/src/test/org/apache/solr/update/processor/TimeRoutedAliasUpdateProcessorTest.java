...
public class TimeRoutedAliasUpdateProcessorTest extends RoutedAliasUpdateProcessorTest {
  ...
  public void test() throws Exception {
    ...
    List<String> retrievedConfigSetNames =
    ...
    assertTrue(
        "ConfigNames should include :" + expectedConfigSetNames,
        retrievedConfigSetNames.containsAll(expectedConfigSetNames));
    ...
    addDocsAndCommit(
        false, // send these to alias & collections
    ...
    final SolrDocumentList checkIncResults =
        solrClient.query(alias, params("q", "NOT " + getIntField() + ":1")).getResults();
    ...
    numDocsDeletedOrFailed +=
        solrClient.query(col23rd, params("q", "*:*", "rows", "0")).getResults().getNumFound();
  ...
  /**
   * Test that the Update Processor Factory routes documents to leader shards and thus avoids the
   * possibility of introducing an extra hop to find the leader.
   *
  ...
  public void testSliceRouting() throws Exception {
    ...
    assertUpdateResponse(
        solrClient.add(
            alias, new SolrInputDocument("id", "1", "timestamp_dt", "2017-10-25T00:00:00Z")));
    assertUpdateResponse(solrClient.commit(alias));
  ...
  public void testPreemptiveCreation() throws Exception {
    ...
    // needed to verify that preemptive creation in one alias doesn't inhibit preemptive creation in
    // another
    CollectionAdminRequest.createTimeRoutedAlias(
            alias2,
            "2017-10-23T00:00:00Z",
    ...
    ExecutorService executorService =
        ExecutorUtil.newMDCAwareCachedThreadPool(
            "TimeRoutedAliasProcessorTestx-testPreemptiveCreation");
    ...
    waitCoreCount(
        getSaferTestName() + "foo" + TRA + "2017-10-23",
        4); // prove this works, for confidence in deletion checking below.
    assertUpdateResponse(
        solrClient.add(
            getSaferTestName() + "foo",
            sdoc(
                ...
                "1",
                "timestamp_dt",
                "2017-10-23T00:00:00Z") // no extra collections should be created
            ));
    assertUpdateResponse(solrClient.commit(getSaferTestName() + "foo"));

    List<String> foo =
        solrClient.getClusterStateProvider().resolveAlias(getSaferTestName() + "foo");
    ...
    CollectionAdminRequest.setAliasProperty(alias)
        .addProperty(TimeRoutedAlias.ROUTER_PREEMPTIVE_CREATE_MATH, "3DAY")
        .process(solrClient);
        ...
        add(
            ...
            Arrays.asList(
                sdoc(
                    ...
                    "7",
                    "timestamp_dt",
                    "2017-10-25T23:01:00Z"), // should cause preemptive creation of 10-27 now
                sdoc(
                    "id",
                    "71",
                    "timestamp_dt",
                    "2017-10-25T23:02:00Z")), // should not cause preemptive creation of 10-28 now
            params));
    assertUpdateResponse(solrClient.commit(alias));
        ...
        add(
            ...
            Collections.singletonList(
                sdoc(
                    ...
                    "8",
                    "timestamp_dt",
                    "2017-10-25T23:01:00Z")), // should cause preemptive creation of 10-28 now
            params));
    assertUpdateResponse(solrClient.commit(alias));
        ...
        add(
            ...
            Arrays.asList(
                ...
                sdoc(
                    ...
                    "10",
                    "timestamp_dt",
                    "2017-10-28T23:01:00Z"), // should be ignored due to in progress creation
                sdoc(
                    "id",
                    "11",
                    "timestamp_dt",
                    "2017-10-28T23:02:00Z"), // should be ignored due to in progress creation
                sdoc(
                    "id",
                    "12",
                    "timestamp_dt",
                    "2017-10-28T23:03:00Z")), // should be ignored due to in progress creation
            params));
    assertUpdateResponse(solrClient.commit(alias));
    ...
    assertNumDocs(
        "2017-10-28", 3, alias); // should get through even though preemptive creation ignored it.
    ...
    //
    // waitColAndAlias(alias, TRA, "2017-10-31", numShards); // spooky! async case arising in middle
    // of sync creation!!
        ...
        add(
            ...
            Collections.singletonList(
                sdoc(
                    ...
                    "14",
                    "timestamp_dt",
                    "2017-10-30T23:01:00Z")), // should cause preemptive creation 10-31
            params));
        ...
        add(
            ...
            Collections.singletonList(
                sdoc(
                    ...
                    "15",
                    "timestamp_dt",
                    "2017-10-30T23:01:00Z")), // should cause preemptive creation 11-01
            params));
        ...
        add(
            ...
            Collections.singletonList(
                sdoc(
                    ...
                    "16",
                    "timestamp_dt",
                    "2017-10-30T23:01:00Z")), // should cause preemptive creation 11-02
            params));
        ...
        add(
            ...
            Collections.singletonList(
                sdoc(
                    ...
                    "17",
                    "timestamp_dt",
                    "2017-10-30T23:01:00Z")), // should NOT cause preemptive creation 11-03
            params));
        ...
        add(
            ...
            Collections.singletonList(
                sdoc(
                    ...
                    "18",
                    "timestamp_dt",
                    "2017-10-31T23:01:00Z")), // should cause preemptive creation 11-03
            params));
        ...
        add(
            ...
            Arrays.asList(
                ...
                sdoc(
                    ...
                    "5",
                    "timestamp_dt",
                    "2017-10-25T23:00:00Z")), // should cause preemptive creation
            params));
    assertUpdateResponse(solrClient.commit(alias));
        ...
        add(
            ...
            Collections.singletonList(
                sdoc(
                    ...
                    "6",
                    "timestamp_dt",
                    "2017-10-25T23:01:00Z")), // might cause duplicate preemptive creation
            params));
    assertUpdateResponse(solrClient.commit(alias));
  }
  ...
  private void addOneDocSynchCreation(int numShards, String alias)
    ...
    // cause some collections to be created
    assertUpdateResponse(
        solrClient.add(alias, sdoc("id", "1", "timestamp_dt", "2017-10-25T00:00:00Z")));
    assertUpdateResponse(solrClient.commit(alias));
    ...
    waitColAndAlias(
        ...
        TRA,
        "2017-10-25",
        numShards); // Create 2nd synchronously (ensure this is not broken)
  ...
  private void validateCollectionCountAndAvailability(String alias, int expected, String message)
      throws SolrServerException, IOException {
    List<String> cols;
    cols =
        new CollectionAdminRequest.ListAliases().process(solrClient).getAliasesAsLists().get(alias);
  ...
  private void assertNumDocs(final String datePart, int expected, String alias)
      throws SolrServerException, IOException {
    QueryResponse resp =
        solrClient.query(
            alias + TRA + datePart,
  ...
  private void testFailedDocument(Instant timestamp, String errorMsg)
      throws SolrServerException, IOException {
    try {
  ...
  /**
   * Need to ensure that the existing TRA's gracefully handle, old, new and mixtures thereof. TRA's
   * with an autoDeleteAge setting will gracefully convert to the new format over time.
   */
  @Test
  public void handleLegacyCollectionNames() throws Exception {
    ...
    assertUpdateResponse(
        solrClient.add(alias, sdoc("id", "3", "timestamp_dt", "2017-10-23T00:00:01Z")));
    solrClient.commit(alias);
    resp =
        solrClient.query(
            alias,
    ...
    assertUpdateResponse(
        solrClient.add(
            alias, sdoc("id", "4", "timestamp_dt", "2017-10-24T23:00:01Z") // preemptive
            ));
    solrClient.commit(alias);
    ...
    assertUpdateResponse(
        solrClient.add(
            alias, sdoc("id", "5", "timestamp_dt", "2017-10-25T12:00:01Z") // preemptive
            ));
    solrClient.commit(alias);
    ...
    assertUpdateResponse(
        solrClient.add(
            alias, sdoc("id", "6", "timestamp_dt", "2017-10-26T12:00:01Z") // preemptive
            ));
    ...
    assertUpdateResponse(
        solrClient.add(
            alias, sdoc("id", "7", "timestamp_dt", "2017-10-27T12:00:01Z") // preemptive
            ));
    ...
    assertUpdateResponse(
        solrClient.add(
            alias, sdoc("id", "8", "timestamp_dt", "2017-10-28T12:00:01Z") // preemptive
            ));
  ...
  private void manuallyConstructLegacyTRA() throws Exception {
    ...
    CollectionAdminRequest.createTimeRoutedAlias(
            ...
            CollectionAdminRequest.createCollection("_unused_", configName, numShards, numReplicas))
        .setPreemptiveCreateWindow("3HOUR")
        .setAutoDeleteAge("/DAY-3DAYS")
        .process(solrClient);

    // now create collections that look like the legacy (pre __TRA__) names...
    String legacy23 = alias + "_" + "2017-10-23";
    CollectionAdminRequest.createCollection(legacy23, configName, numShards, numReplicas)
        .process(solrClient);
    String legacy24 = alias + "_" + "2017-10-24";
    CollectionAdminRequest.createCollection(legacy24, configName, numShards, numReplicas)
        .process(solrClient);
    ...
    assertUpdateResponse(
        solrClient.add(legacy23, sdoc("id", "1", "timestamp_dt", "2017-10-23T00:00:01Z")));
    assertUpdateResponse(
        solrClient.add(legacy24, sdoc("id", "2", "timestamp_dt", "2017-10-24T00:00:01Z")));
    ...
    CollectionAdminRequest.setCollectionProperty(
            legacy23, RoutedAlias.ROUTED_ALIAS_NAME_CORE_PROP, alias)
        .process(solrClient);
    CollectionAdminRequest.setCollectionProperty(
            legacy24, RoutedAlias.ROUTED_ALIAS_NAME_CORE_PROP, alias)
        .process(solrClient);
