...
public class AtomicUpdatesTest extends SolrTestCaseJ4 {
  ...
  public void testRemove() throws Exception {
    ...
    assertQ(
        req("q", "cat:ccc", "indent", "true"),
        "//result[@numFound = '3']"); // remove only removed first occurrence
  ...
  public void testRemoveInteger() throws Exception {
    ...
    assertQ(
        req("q", "intRemove:333", "indent", "true"),
        "//result[@numFound = '3']"); // remove only removed first occurrence
  ...
  public void testRemoveIntegerUsingDoubleType() throws Exception {
    ...
    doc.setField("id", "1001");
    doc.setField(
        "intRemove", new String[] {"11111111", "22222222", "33333333", "33333333", "44444444"});
    assertU(adoc(doc));

    doc = new SolrInputDocument();
    doc.setField("id", "1002");
    doc.setField(
        "intRemove", new String[] {"11111111", "22222222", "22222222", "33333333", "44444444"});
    assertU(adoc(doc));
    ...
    assertQ(
        req("q", "intRemove:33333333", "indent", "true"),
        "//result[@numFound = '3']"); // remove only removed first occurrence
  ...
  public void testRemoveDateUsingStringType() throws Exception {
    ...
    doc.setField(
        "dateRemove",
        new String[] {
          "2014-09-01T12:00:00Z",
          "2014-09-02T12:00:00Z",
          "2014-09-03T12:00:00Z",
          "2014-09-03T12:00:00Z",
          "2014-09-04T12:00:00Z"
        });
    assertU(adoc(doc));
    ...
    doc.setField(
        "dateRemove",
        new String[] {"2014-09-01T12:00:00Z", "2014-09-03T12:00:00Z", "2014-09-04T12:00:00Z"});
    assertU(adoc(doc));
    ...
    doc.setField(
        "dateRemove",
        new String[] {"2014-09-01T12:00:00Z", "2014-09-02T12:00:00Z", "2014-09-04T12:00:00Z"});
    assertU(adoc(doc));

    assertU(commit());

    boolean isPointField =
        h.getCore().getLatestSchema().getField("dateRemove").getType().isPointField();
    if (isPointField) {
    ...
    assertQ(
        req("q", "dateRemove:\"2014-09-02T12:00:00Z\"", "indent", "true"),
        "//result[@numFound = '3']");
    assertQ(
        req("q", "dateRemove:\"2014-09-03T12:00:00Z\"", "indent", "true"),
        "//result[@numFound = '3']");
    ...
    assertQ(
        req("q", "dateRemove:\"2014-09-01T12:00:00Z\"", "indent", "true"),
        "//result[@numFound = '3']");
  }
  ...
  public void testRemoveDateUsingDateType() throws Exception {
    ...
    doc.setField(
        "dateRemove",
        new Date[] {
          DateMathParser.parseMath(null, "2014-02-01T12:00:00Z"),
    ...
    //    assertQ(req("q", "dateRemove:"+URLEncoder.encode(dateString, "UTF-8"), "indent", "true"),
    // "//result[@numFound = '3']");
    //    assertQ(req("q", "dateRemove:\"2014-09-02T12:00:00Z\"", "indent", "true"),
    // "//result[@numFound = '3']");
    //    assertQ(req("q", "dateRemove:"+dateString, "indent", "true"), "//result[@numFound =
    // '3']"); //Sun Feb 02 10:00:00 FNT 2014
    assertQ(
        req("q", "dateRemove:\"Sun Feb 02 10:00:00 FNT 2014\"", "indent", "true"),
        "//result[@numFound = '3']"); // Sun Feb 02 10:00:00 FNT 2014
  ...
  public void testRemoveFloatUsingStringType() throws Exception {
    ...
    doc.setField("id", "10001");
    doc.setField(
        "floatRemove", new String[] {"111.111", "222.222", "333.333", "333.333", "444.444"});
    ...
    doc.setField("id", "10002");
    doc.setField(
        "floatRemove", new String[] {"111.111", "222.222", "222.222", "333.333", "444.444"});
    assertU(adoc(doc));
    ...
    assertQ(req("q", "floatRemove:\"222.222\"", "indent", "true"), "//result[@numFound = '2']");
    assertQ(
        req("q", "floatRemove:\"333.333\"", "indent", "true"),
        "//result[@numFound = '3']"); // remove only removed first occurrence
  ...
  public void testRemoveregex() throws Exception {
    ...
    assertQ(
        req("q", "cat:ccc", "indent", "true"),
        "//result[@numFound = '2']"); // removeregex does remove all occurrences
  ...
  public void testRemoveregexMustMatchWholeValue() throws Exception {
    ...
    assertQ(req("q", "cat:*", "indent", "true"), "//result[@numFound = '1']");
    assertQ(
        req("q", "cat:bbb", "indent", "true"),
        "//result[@numFound = '1']"); // Was not removed - regex didn't match whole value
    ...
    assertQ(
        req("q", "cat:bbb", "indent", "true"),
        "//result[@numFound = '0']"); // Was removed now - regex matches
  }
  ...
  public void testAddDistinct() throws Exception {
    ...
    assertQ(
        req("q", "cat:bbb", "indent", "true"),
        "//result[@numFound = '2']"); // 'cat' field not present, do 'add' atomic operation
  }
  ...
  public void testAddMultiple() throws Exception {
    ...
    assertQ(
        req("q", "cat:bbb", "indent", "true"),
        "//result[@numFound = '1']"); // Should now have 2 occurrences of bbb
    ...
    assertQ(
        req("q", "cat:bbb", "indent", "true"),
        "//result[@numFound = '1']"); // remove only removed first occurrence
    ...
    assertQ(
        req("q", "cat:bbb", "indent", "true"),
        "//result[@numFound = '0']"); // remove now removed last occurrence
  }
  ...
  public void testAtomicUpdatesOnNonStoredDocValuesCopyField() throws Exception {
    ...
    // Do each one twice... the first time it will be retrieved from the index, and the second time
  ...
  /**
   * Test what happens if we try to update the parent of a doc with children. This fails because
   * _root_ is not stored which is currently required for doing this.
   */
  @Test
  public void testUpdateNestedDocUnsupported() throws Exception {
    ...
    try {
      assertU(
          adoc(
              sdoc("id", "1", "cat", Collections.singletonMap("add", Arrays.asList("parentCat")))));
      fail("expected a failure");
    } catch (Exception e) {
      assertEquals(
          "org.apache.solr.common.SolrException: "
              + "This schema does not support partial updates to nested docs. See ref guide.",
          e.toString());
    }
  }
