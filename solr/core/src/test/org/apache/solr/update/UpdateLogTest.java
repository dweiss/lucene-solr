...
public class UpdateLogTest extends SolrTestCaseJ4 {
  ...
  public void testApplyPartialUpdatesOnMultipleInPlaceUpdatesInSequence() {
    ...
    long returnVal =
        ulog.applyPartialUpdates(DOC_1_INDEXED_ID, prevPointer, prevVersion, null, partialDoc);
  ...
  public void testApplyPartialUpdatesAfterMultipleCommits() {
    ...
    // Do 3 commits, then in-place update, and verify that applying partial updates can't find full
    // doc
  ...
  public void testApplyPartialUpdatesDependingOnNonAddShouldThrowException() {
    ...
    SolrException ex =
        expectThrows(
            ...
            () -> {
              long returnVal =
                  ulog.applyPartialUpdates(
                      DOC_1_INDEXED_ID, prevPointer, prevVersion, null, partialDoc);
              fail(
                  "502 depends on 501, 501 depends on 500, but 500 is a"
                      + " DELETE. This should've generated an exception. returnVal is: "
                      + returnVal);
            });
  ...
  public void testApplyPartialUpdatesWithDelete() throws Exception {
    ...
    if (dbq) {
      assertNull(
          ulog.lookup(
              DOC_1_INDEXED_ID)); // any DBQ clears out the ulog, so this document shouldn't exist
  ...
  /**
   * Helper method to construct an <code>AddUpdateCommand</code> for a <code>SolrInputDocument
   * </code> in the context of the specified <code>SolrQueryRequest</code>.
   *
   * <p>NOTE: For test simplicity, the Solr input document must include the <code>_version_</code>
   * field.
   */
  public static AddUpdateCommand buildAddUpdateCommand(
