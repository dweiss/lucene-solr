...
public class LukeRequestHandlerTest extends SolrTestCaseJ4 {
  ...
  private void assertHistoBucket(int slot, int in) {
    assertEquals(
        "histobucket: " + in, slot, 32 - Integer.numberOfLeadingZeros(Math.max(0, in - 1)));
  }
  ...
  public void testIndexHeapUsageBytes() throws Exception {
    try (SolrQueryRequest req = req("qt", "/admin/luke")) {
      ...
      Assert.assertTrue(
          "indexHeapUsageBytes should be > 0, but was " + num.intValue(), num.intValue() > 0);
    }
  }
  ...
  public void testCatchAllCopyField() throws Exception {
    ...
    for (IndexSchema.DynamicCopy dcf : schema.getDynamicCopyFields()) {
      foundCatchAllCopyField =
          dcf.getRegex().equals("*") && dcf.getDestFieldName().equals("catchall_t");
    ...
    String result =
        TestHarness.validateXPath(
            xml, field("bday") + "/arr[@name='copyDests']/str[.='catchall_t']");
