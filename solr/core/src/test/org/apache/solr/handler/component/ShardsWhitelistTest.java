...
public class ShardsWhitelistTest extends MultiSolrCloudTestCase {
  ...
  /**
   * The cluster with this key will include an explicit list of host whitelisted (all hosts in both
   * the clusters)
   */
  private static final String EXPLICIT_CLUSTER_KEY = "explicitCluster";
  /**
   * The cluster with this key will not include an explicit list of host whitelisted, will rely on
   * live_nodes
   */
  private static final String IMPLICIT_CLUSTER_KEY = "implicitCluster";

  private static final String EXPLICIT_WHITELIST_PROPERTY =
          ...
          public MiniSolrCloudCluster apply(String clusterId) {
            try {
              final MiniSolrCloudCluster cluster =
                  new SolrCloudTestCase.Builder(nodesPerCluster(clusterId), createTempDir())
                      .addConfig("conf", configset("cloud-dynamic"))
                      .withSolrXml(
                          MiniSolrCloudCluster.DEFAULT_CLOUD_SOLR_XML.replace(
                              MiniSolrCloudCluster.SOLR_TESTS_SHARDS_WHITELIST,
                              EXPLICIT_WHITELIST_PROPERTY + clusterId))
                      .build();
              return cluster;
            } catch (Exception e) {
  ...
  public void test() throws Exception {
    assertThat(
        getShardHandlerFactory(EXPLICIT_CLUSTER_KEY).getWhitelistHostChecker().getWhitelistHosts(),
        notNullValue());
    assertThat(
        getShardHandlerFactory(IMPLICIT_CLUSTER_KEY).getWhitelistHostChecker().getWhitelistHosts(),
        nullValue());

    assertThat(
        getShardHandlerFactory(EXPLICIT_CLUSTER_KEY)
            .getWhitelistHostChecker()
            .hasExplicitWhitelist(),
        is(true));
    assertThat(
        getShardHandlerFactory(IMPLICIT_CLUSTER_KEY)
            .getWhitelistHostChecker()
            .hasExplicitWhitelist(),
        is(false));
    ...
    // explicit whitelist includes all the nodes in both clusters. Requests should be allowed to go
    // through
    assertThat(
    ...
    assertForbidden(
        "id:explicitCluster*", "shard1," + getShardUrl("shard2", explicitCluster), implicitCluster);

    assertForbidden(
        "id:explicitCluster*", getShardUrl("shard1", explicitCluster) + ",shard2", implicitCluster);
  ...
  private void assertForbidden(String query, String shards, MiniSolrCloudCluster cluster)
    ...
    try {
      ...
      assertThat(
          ((SolrException) e.getCause()).getMessage(),
          containsString("not on the shards whitelist"));
    }
