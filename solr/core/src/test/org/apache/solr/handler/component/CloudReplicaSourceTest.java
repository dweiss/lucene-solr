...
public class CloudReplicaSourceTest extends SolrTestCaseJ4 {
  ...
  public void testSimple_ShardsParam() {
    ...
    HttpShardHandlerFactory.WhitelistHostChecker whitelistHostChecker =
        Mockito.mock(HttpShardHandlerFactory.WhitelistHostChecker.class);
    ModifiableSolrParams params = new ModifiableSolrParams();
    params.set("shards", "slice1,slice2");
    try (ZkStateReader zkStateReader =
        ClusterStateMockUtil.buildClusterState("csr*sr2", "baseUrl1_", "baseUrl2_")) {
      ...
      assertEquals(
          "http://baseUrl1/slice1_replica1/", cloudReplicaSource.getReplicasBySlice(0).get(0));
      assertEquals(1, cloudReplicaSource.getReplicasBySlice(1).size());
      assertEquals(
          "http://baseUrl2/slice2_replica2/", cloudReplicaSource.getReplicasBySlice(1).get(0));
    }
  }
  ...
  public void testShardsParam_DeadNode() {
    ...
    try (ZkStateReader zkStateReader =
        ClusterStateMockUtil.buildClusterState("csr*sr2", "baseUrl1_")) {
      CloudReplicaSource cloudReplicaSource =
          new CloudReplicaSource.Builder()
              .collection("collection1")
              .onlyNrt(false)
  ...
  public void testShardsParam_DownReplica() {
    ...
    try (ZkStateReader zkStateReader =
        ClusterStateMockUtil.buildClusterState(
            "csr*sr2r3D", "baseUrl1_", "baseUrl2_", "baseUrl3_")) {
      CloudReplicaSource cloudReplicaSource =
          new CloudReplicaSource.Builder()
              .collection("collection1")
              .onlyNrt(false)
  ...
  public void testMultipleCollections() {
    ...
    try (ZkStateReader zkStateReader =
        ClusterStateMockUtil.buildClusterState("csr*sr2csr*", "baseUrl1_", "baseUrl2_")) {
      CloudReplicaSource cloudReplicaSource =
          new CloudReplicaSource.Builder()
              .collection("collection1")
              .onlyNrt(false)
  ...
  public void testSimple_OnlyNrt() {
    ...
    // the leader
    try (ZkStateReader zkStateReader =
        ClusterStateMockUtil.buildClusterState("csrr*st2t2", "baseUrl1_", "baseUrl2_")) {
      CloudReplicaSource cloudReplicaSource =
          new CloudReplicaSource.Builder()
              .collection("collection1")
  ...
  public void testMultipleCollections_OnlyNrt() {
    ...
    try (ZkStateReader zkStateReader =
        ClusterStateMockUtil.buildClusterState("csrr*st2t2cst", "baseUrl1_", "baseUrl2_")) {
      CloudReplicaSource cloudReplicaSource =
          new CloudReplicaSource.Builder()
              .collection("collection1")
              .onlyNrt(true) // enable only nrt mode
