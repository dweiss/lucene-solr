...
public class PhrasesIdentificationComponentTest extends SolrTestCaseJ4 {
  ...
  public void testWhiteBoxPhraseParsingShortInput() throws Exception {
    ...
    for (Map.Entry<String, Integer> entry : fields.entrySet()) {
      try {
        ...
        assertEquals(
            0, Phrase.extractPhrases(random().nextBoolean() ? "" : "  ", field, 3, maxQ).size());
  ...
  /**
   * Asserts some basic rules that should be enforced about all Phrases &amp; their linkages to oher
   * phrases
   */
  private void assertBasicSanityChecks(
      final List<Phrase> phrases,
    ...
    assertEmptyStream(
        "no phrase should have a start offset < 0",
        phrases.stream().filter(p -> p.getOffsetStart() < 0));
    assertEmptyStream(
        "no phrase should have a start position < 1",
        phrases.stream().filter(p -> p.getPositionStart() < 1));

    assertEmptyStream(
        "If a phrase has a start offset of 0, then it must have position 1",
        phrases.stream().filter(p -> (p.getOffsetStart() == 0) && (p.getPositionStart() != 1)));

    final Phrase first = phrases.get(0);
    final Phrase last = phrases.get(phrases.size() - 1);

    assertEmptyStream(
        "no phrase should have a start offset < first phrase",
        phrases.stream().filter(p -> p.getOffsetStart() < first.getOffsetStart()));
    assertEmptyStream(
        "no phrase should have an end offset > last phrase",
        phrases.stream().filter(p -> last.getOffsetEnd() < p.getOffsetEnd()));

    assertEmptyStream(
        "no phrase should have a start position < first phrase",
        phrases.stream().filter(p -> p.getPositionStart() < first.getPositionStart()));
    assertEmptyStream(
        "no phrase should have an end position > last phrase",
    ...
    for (int n = 1; n <= maxQueryPositionLength; n++) {
      ...
      final List<Phrase> sizeN =
          phrases.stream().filter(p -> p.getPositionLength() == len).collect(Collectors.toList());
      assertEquals("Expected # phrases of size " + n + ": " + sizeN, expected, sizeN.size());
    }

    // check the quantities of sub-terms/phrases...
    assertEmptyStream(
        "no phrase should have num indexed terms != pos_len",
        phrases.stream()
            .filter(p -> last.getPositionLength() != last.getIndividualIndexedTerms().size()));
    assertEmptyStream(
        "no phrase should have num sub-phrases != max(1, 1 + pos_len - "
            + maxIndexedPositionLength
            + ")",
        phrases.stream()
            .filter(
                p ->
                    (Math.max(1, 1 + last.getPositionLength() - maxIndexedPositionLength)
                        != last.getLargestIndexedSubPhrases().size())));
    // NOTE: indexed super phrases can be of various lengths, and differing quantities near
    // begining/end of input so don't worry about an exact count, just check their properties
    // (below)
    ...
    for (Phrase phrase : phrases) {
      ...
      assertEmptyStream(
          debug + " should not have any indexed terms where pos_len != 1",
          phrase.getIndividualIndexedTerms().stream()
              .filter(term -> 1 != term.getPositionLength()));

      assertEmptyStream(
          debug
              + " should not have any sub-phrases where pos_len > min(pos_len, "
              + maxIndexedPositionLength
              + ")",
          phrase.getLargestIndexedSubPhrases().stream()
              .filter(
                  inner ->
                      (Math.min(phrase.getPositionLength(), maxIndexedPositionLength)
                          < inner.getPositionLength())));

      assertEmptyStream(
          debug
              + " should not have any super-phrases where super.len <= phrase.len or "
              + maxIndexedPositionLength
              + " < super.len",
          phrase.getIndexedSuperPhrases().stream()
              .filter(
                  outer ->
                      (outer.getPositionLength() <= phrase.getPositionLength()
  ...
  public void testWhiteboxStats() throws Exception {
    ...
    try (SolrQueryRequest req = req()) {
      Phrase.populateStats(
          phrasesLocal, Arrays.asList("multigrams_body", "multigrams_title"), req.getSearcher());
    }
  ...
  public void testWhiteboxScores() throws Exception {
    ...
    for (Phrase xyz : phrases.subList(0, 7)) {
      // first 7 all start with xyz which isn't in index (in either field) so all scores should be
      // -1
    ...
    // (in both fields)
    for (Phrase term :
        phrases.subList(7, phrases.size()).stream()
            .filter((p -> 1 == p.getPositionLength()))
  ...
  public void testWhiteboxScorcesStopwords() throws Exception {
    ...
    { // If our analysisField uses all terms,
      ...
      assertEquals(
          why_the_lazy.toString(),
          -1.0D,
          why_the_lazy.getFieldScore("multigrams_title_stop"),
          0.0D);
      ...
      assertEquals(
          the_lazy_dog.toString(),
          -1.0D,
          the_lazy_dog.getFieldScore("multigrams_title_stop"),
          0.0D);
  ...
  public void testExpectedUserErrors() throws Exception {
    assertQEx(
        ...
        req("q", "foo", "phrases", "true", "phrases.fields", " "),
        ErrorCode.BAD_REQUEST);

    assertQEx(
        "bogus field name should error",
        "does not exist",
        req("q", "foo", "phrases", "true", "phrases.fields", "bogus1 bogus2"),
        ErrorCode.BAD_REQUEST);

    assertQEx(
        "lack of shingles should cause error",
        "Unable to determine max position length",
        req("q", "foo", "phrases", "true", "phrases.fields", "title"),
        ErrorCode.BAD_REQUEST);
    ...
    assertQEx(
        ...
        req(
            ...
            "true",
            "phrases.fields",
            "multigrams_title multigrams_title_short"),
        ErrorCode.BAD_REQUEST);

    assertQEx(
        "analysis field must exist",
        "does not exist",
        req(
            "q",
            "foo",
  ...
  public void testMaxShingleSizeHelper() throws Exception {
    ...
    assertEquals(
        3,
        PhrasesIdentificationComponent.getMaxShingleSize(
  ...
  public void testSimpleSearchRequests() throws Exception {
    ...
    for (SolrQueryRequest req :
        Arrays.asList( // simple, using 3/7 defaults
            ...
            req(
                ...
                input,
                "phrases.fields",
                "multigrams_body_short multigrams_title_short^2"),
            ...
            req(
                ...
                input,
                "phrases.fields",
                "multigrams_body multigrams_title_short^2",
                "phrases.analysis.field",
  ...
  public void testGreyboxShardSearchRequests() throws Exception {
    ...
    assertTrue(
        "sanity check failed, stage was modified in code w/o updating test",
  ...
  /**
   * Trivial Helper method that collects &amp; compares to an empty List so the assertion shows the
