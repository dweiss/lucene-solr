...
public class ZookeeperStatusHandlerTest extends SolrCloudTestCase {
  ...
  */
  ...
  public void testEnsembleStatusMock() {
    ...
    when(zkStatusHandler.getZkRawResponse("zoo1:2181", "mntr"))
        .thenReturn(
            Arrays.asList(
                "zk_version\t3.5.5-390fe37ea45dee01bf87dc1c042b5e3dcce88653, built on 05/03/2019 12:07 GMT",
                "zk_avg_latency\t1"));
    when(zkStatusHandler.getZkRawResponse("zoo1:2181", "conf"))
        .thenReturn(
            Arrays.asList(
                "clientPort=2181", "secureClientPort=-1", "thisIsUnexpected", "membership: "));
    ...
    when(zkStatusHandler.getZkRawResponse("zoo3:2181", "mntr"))
        .thenReturn(
            Arrays.asList(
                "mntr is not executed because it is not in the whitelist.")); // Actual response
    // from ZK if not
    // whitelisted
    when(zkStatusHandler.getZkRawResponse("zoo3:2181", "conf"))
        .thenReturn(Arrays.asList("clientPort=2181"));
    ...
    when(zkStatusHandler.validateZkRawResponse(ArgumentMatchers.any(), any(), any()))
    ...
    ZkDynamicConfig zkDynamicConfig =
        ZkDynamicConfig.parseLines(
            "server.1=zoo1:2780:2783:participant;0.0.0.0:2181\n"
                ...
                + "version=400000003");
    Map<String, Object> mockStatus =
        zkStatusHandler.getZkStatus("zoo4:2181,zoo5:2181,zoo6:2181", zkDynamicConfig);
    String expected =
        "{\n"
  ...
  public void testMntrBugZk36Solr14463() {
    ...
    Map<String, Object> mockStatus =
        zkStatusHandler.getZkStatus("zoo1:2181", ZkDynamicConfig.fromZkConnectString("zoo1:2181"));
    String expected =
        "{\n"
            + "  \"mode\":\"ensemble\",\n"
            + "  \"dynamicReconfig\":true,\n"
  ...
  public void testZkWithPrometheusSolr14752() {
    ...
    // Verifying that parsing the status strings with floating point no longer triggers a
    // NumberFormatException, although floats are still displayed in UI
    Map<String, Object> mockStatus =
        zkStatusHandler.getZkStatus("zoo1:2181", ZkDynamicConfig.fromZkConnectString("zoo1:2181"));
    String expected =
        "{\n"
