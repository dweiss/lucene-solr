...
public class TestApiFramework extends SolrTestCaseJ4 {
  ...
  public void testFramework() {
    ...
    assertNotNull(api);
    assertConditions(
        api.getSpec(), Utils.makeMap("/methods[0]", "POST", "/commands/create", NOT_NULL));
    ...
    assertConditions(
        api.getSpec(), Utils.makeMap("/methods[0]", "POST", "/commands/force-leader", NOT_NULL));
    assertEquals("hello", parts.get("collection"));
  ...
  public static void assertConditions(
      @SuppressWarnings({"rawtypes"}) Map root, @SuppressWarnings({"rawtypes"}) Map conditions) {
    for (Object o : conditions.entrySet()) {
      ...
      if (e.getValue() instanceof ValidatingJsonMap.PredicateWithErrMsg) {
        @SuppressWarnings({"rawtypes"})
        ValidatingJsonMap.PredicateWithErrMsg value =
            (ValidatingJsonMap.PredicateWithErrMsg) e.getValue();
