...
public class LoggingHandlerTest extends SolrTestCaseJ4 {
  ...
  public void testLogLevelHandlerOutput() throws Exception {
    ...
    assertQ(
        ...
        "//arr[@name='loggers']/lst/str[.='"
            + CLASS_LOGGER_NAME
            + "']/../str[@name='level'][.='DEBUG']",
        "//arr[@name='loggers']/lst/str[.='" + PARENT_LOGGER_NAME + "']/../null[@name='level']");

    assertQ(
        "Set a (new) level",
        req(CommonParams.QT, "/admin/logging", "set", PARENT_LOGGER_NAME + ":TRACE"),
        "//arr[@name='loggers']/lst/str[.='"
            + PARENT_LOGGER_NAME
            + "']/../str[@name='level'][.='TRACE']");
    ...
    assertQ(
        "Remove a level",
        req(CommonParams.QT, "/admin/logging", "set", PARENT_LOGGER_NAME + ":null"),
        "//arr[@name='loggers']/lst/str[.='"
            + PARENT_LOGGER_NAME
            + "']/../str[@name='level'][.='OFF']");
