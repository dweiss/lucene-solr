...
public class TestDistribPackageStore extends SolrCloudTestCase {
  ...
  public void testPackageStoreManagement() throws Exception {
    ...
    try {
      ...
      //      cluster.getZkClient().create("/keys/exe/pub_key512.der", derFile,
  ...
  public static void checkAllNodesForFile(
      MiniSolrCloudCluster cluster, String path, Map expected, boolean verifyContent)
      throws Exception {
    for (JettySolrRunner jettySolrRunner : cluster.getJettySolrRunners()) {
      String baseUrl = jettySolrRunner.getBaseUrl().toString().replace("/solr", "/api");
  ...
  public static NavigableObject assertResponseValues(
      ...
      @SuppressWarnings({"rawtypes"}) Map vals)
      throws Exception {
  ...
  @SuppressWarnings({"unchecked"})
  public static NavigableObject assertResponseValues(
      int repeats, Callable<NavigableObject> callable, @SuppressWarnings({"rawtypes"}) Map vals)
      throws Exception {
  ...
  public static void uploadKey(byte[] bytes, String path, MiniSolrCloudCluster cluster)
    ...
    checkAllNodesForFile(
        cluster,
        path,
        Utils.makeMap(":files:" + path + ":name", (Predicate<Object>) Objects::nonNull),
        false);
  }
