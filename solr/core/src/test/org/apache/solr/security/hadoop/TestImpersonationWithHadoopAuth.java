...
public class TestImpersonationWithHadoopAuth extends SolrCloudTestCase {
  ...
  public static void setupClass() throws Exception {
    ...
    Map<String, String> proxyUserConfigs =
        (Map<String, String>)
            authConfig.getOrDefault(HadoopAuthPlugin.PROXY_USER_CONFIGS, new HashMap<>());
    ...
    proxyUserConfigs.put(
        "proxyuser.localHostAnyGroup.hosts",
    ...
    proxyUserConfigs.put(
        "proxyuser.anyHostUsersGroup.groups", ImpersonationUtil.getUsersFirstGroup());
  ...
  private SolrClient newSolrClient() {
    return new HttpSolrClient.Builder(cluster.getJettySolrRunner(0).getBaseUrl().toString())
        .build();
  }
