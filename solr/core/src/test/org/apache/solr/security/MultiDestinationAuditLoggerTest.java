...
public class MultiDestinationAuditLoggerTest extends SolrTestCaseJ4 {
  ...
  public void init() throws IOException {
    ...
    assertEquals(
        0,
        ((MockAuditLoggerPlugin) al.plugins.get(1)).events.size()); // not configured for ANONYMOUS
    al.doAudit(new AuditEvent(AuditEvent.EventType.AUTHENTICATED).setUsername("me"));
    assertEquals(
        1,
        ((MockAuditLoggerPlugin) al.plugins.get(1)).events.size()); // configured for authenticated
