...
public class PKIAuthenticationIntegrationTest extends SolrCloudAuthTestCase {
  ...
  public static void setupCluster() throws Exception {
    final String SECURITY_CONF =
        Utils.toJSONString(
            makeMap(
                "authorization", singletonMap("class", MockAuthorizationPlugin.class.getName()),
  ...
  public void testPkiAuth() throws Exception {
    ...
    for (JettySolrRunner jetty : cluster.getJettySolrRunners()) {
      ...
      verifySecurityStatus(
          ...
          20);
      verifySecurityStatus(
          httpClient, baseUrl + "/admin/authentication", "authentication.enabled", "true", 20);
    }
    ...
    MockAuthenticationPlugin.predicate =
        servletRequest -> {
    ...
    assertTrue(
        "all nodes must get the user solr , no:of nodes got solr : " + count.get(),
        count.get() > 2);
