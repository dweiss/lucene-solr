...
public class TestSolrCloudSnapshots extends SolrCloudTestCase {
  ...
  public void testSnapshots() throws Exception {
    ...
    CollectionAdminRequest.Create create =
        CollectionAdminRequest.createCollection(collectionName, "conf1", NUM_SHARDS, NUM_REPLICAS);
    ...
    int expectedCoresWithSnapshot =
        stoppedCoreName.isPresent() ? (NUM_SHARDS * NUM_REPLICAS) - 1 : (NUM_SHARDS * NUM_REPLICAS);

    CollectionAdminRequest.CreateSnapshot createSnap =
        new CollectionAdminRequest.CreateSnapshot(collectionName, commitName);
    createSnap.process(solrClient);

    Collection<CollectionSnapshotMetaData> collectionSnaps =
    ...
    CollectionAdminRequest.DeleteSnapshot deleteSnap =
        new CollectionAdminRequest.DeleteSnapshot(collectionName, commitName);
    ...
    // Verify if the collection deletion result in proper cleanup of snapshot metadata.
    {
      ...
      CollectionAdminRequest.CreateSnapshot createSnap_2 =
          new CollectionAdminRequest.CreateSnapshot(collectionName, commitName_2);
  ...
  private Collection<CollectionSnapshotMetaData> listCollectionSnapshots(
      SolrClient adminClient, String collectionName) throws Exception {
  ...
  private Collection<SnapshotMetaData> listCoreSnapshots(SolrClient adminClient, String coreName)
    ...
    for (int i = 0; i < apiResult.size(); i++) {
      ...
      String indexDirPath =
          (String) ((NamedList) apiResult.get(commitName)).get(SolrSnapshotManager.INDEX_DIR_PATH);
      long genNumber =
          Long.parseLong(
              (String)
                  ((NamedList) apiResult.get(commitName)).get(SolrSnapshotManager.GENERATION_NUM));
