...
public class TestConfig extends SolrTestCaseJ4 {
  ...
  public void testAutomaticDeprecationSupport() {
    ...
    ShowFileRequestHandler handler =
        (ShowFileRequestHandler) h.getCore().getRequestHandler("/admin/file");
  ...
  public void testDefaults() throws Exception {
    ...
    ++numDefaultsTested;
    assertEquals("default ramPerThreadHardLimitMB", -1, sic.ramPerThreadHardLimitMB);
    ++numDefaultsTested;
    ...
    ++numDefaultsTested;
    assertEquals("default maxCommitMergeWaitTime", -1, sic.maxCommitMergeWaitMillis);
  ...
  public void testConvertAutoCommitMaxSizeStringToBytes() {
    ...
    RuntimeException thrown =
        expectThrows(
            ...
            () -> {
              SolrConfig.convertHeapOptionStyleConfigStringToBytes(
                  "3jbk32k"); // valid suffix but non-numeric prefix
            });
    ...
    thrown =
        expectThrows(
            ...
            () -> {
              SolrConfig.convertHeapOptionStyleConfigStringToBytes(
                  "300x"); // valid prefix but invalid suffix
            });
    assertTrue(thrown.getMessage().contains("Invalid"));
  }
