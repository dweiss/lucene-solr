...
public class HdfsDirectoryFactoryTest extends SolrTestCaseJ4 {
  ...
  public void testInitArgsOrSysPropConfig() throws Exception {
    try (HdfsDirectoryFactory hdfsFactory = new HdfsDirectoryFactory()) {
      ...
      System.setProperty(
          HdfsDirectoryFactory.HDFS_HOME, HdfsTestUtil.getURI(dfsCluster) + "/solr1");
      ...
      assertEquals(
          4, hdfsFactory.getConfig(HdfsDirectoryFactory.NRTCACHINGDIRECTORY_MAXMERGESIZEMB, 0));
      ...
      assertEquals(
          3, hdfsFactory.getConfig(HdfsDirectoryFactory.NRTCACHINGDIRECTORY_MAXMERGESIZEMB, 0));
      assertTrue(hdfsFactory.getConfig(HdfsDirectoryFactory.BLOCKCACHE_ENABLED, false));
      ...
      assertEquals(
          0, hdfsFactory.getConfig(HdfsDirectoryFactory.NRTCACHINGDIRECTORY_MAXMERGESIZEMB, 0));
  ...
  public void testCleanupOldIndexDirectories() throws Exception {
    try (HdfsDirectoryFactory hdfsFactory = new HdfsDirectoryFactory()) {
      ...
      try (FileSystem hdfs = FileSystem.get(HdfsTestUtil.getClientConfiguration(dfsCluster))) {
        ...
        org.apache.hadoop.fs.Path currentIndexDirPath =
            new org.apache.hadoop.fs.Path(dataHomePath, "index");
        ...
        org.apache.hadoop.fs.Path oldIndexDirPath =
            new org.apache.hadoop.fs.Path(dataHomePath, "index." + timestamp1);
        ...
        hdfsFactory.cleanupOldIndexDirectories(
            dataHomePath.toString(), currentIndexDirPath.toString(), false);
  ...
  private boolean checkHdfsDirectory(FileSystem hdfs, org.apache.hadoop.fs.Path path)
      throws IOException {
    try {
  ...
  public void testLocalityReporter() throws Exception {
    ...
    try (HdfsDirectoryFactory factory = new HdfsDirectoryFactory()) {
      ...
      try (Directory dir = factory.create(path, NoLockFactory.INSTANCE, DirContext.DEFAULT)) {
        ...
        assertEquals(
            "Wrong number of total bytes counted: "
                + statistics.get(HdfsLocalityReporter.LOCALITY_BYTES_TOTAL),
            long_bytes,
            statistics.get(HdfsLocalityReporter.LOCALITY_BYTES_TOTAL));
        assertEquals(
            "Wrong number of total blocks counted: "
                + statistics.get(HdfsLocalityReporter.LOCALITY_BLOCKS_TOTAL),
            1,
            statistics.get(HdfsLocalityReporter.LOCALITY_BLOCKS_TOTAL));
        assertEquals(
            "Counted block as local when bad hostname set: "
                + statistics.get(HdfsLocalityReporter.LOCALITY_BLOCKS_LOCAL),
            0,
            statistics.get(HdfsLocalityReporter.LOCALITY_BLOCKS_LOCAL));
