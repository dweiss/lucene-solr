...
public class TestCodecSupport extends SolrTestCaseJ4 {
  ...
  public void testDocValuesFormats() {
    ...
    assertEquals(
        TestUtil.getDefaultDocValuesFormat().getName(),
  ...
  public void testDynamicFieldsDocValuesFormats() {
    ...
    assertEquals(
        TestUtil.getDefaultDocValuesFormat().getName(),
        format.getDocValuesFormatForField("foo_disk").getName());
    assertEquals(
        TestUtil.getDefaultDocValuesFormat().getName(),
  ...
  private void doTestCompressionMode(String propertyValue, String expectedModeString)
      throws IOException {
            ...
            searcher -> {
              ...
              assertEquals(
                  "Expecting compression mode string to be "
                      + expectedModeString
                      + " but got: "
                      + info.getAttribute(Lucene87StoredFieldsFormat.MODE_KEY)
                      + "\n SegmentInfo: "
                      + info
                      + "\n SegmentInfos: "
                      + infos
  ...
  public void testCompressionMode() throws Exception {
    assertEquals(
        "incompatible change in compressionMode property",
        "compressionMode",
        SchemaCodecFactory.COMPRESSION_MODE);
  ...
  public void testCompressionModeDefault() throws IOException {
    ...
    SolrConfig config =
        TestHarness.createConfig(testSolrHome, previousCoreName, "solrconfig_codec2.xml");
    assertEquals(
        "Unexpected codec factory for this test.",
        "solr.SchemaCodecFactory",
        config.get("codecFactory/@class"));
    assertNull(
    ...
    try {
      ...
      assertEquals(
          "We are not using the correct core",
          "solrconfig_codec2.xml",
          h.getCore().getConfigResource());
      assertU(add(doc("string_f", "foo")));
      assertU(commit());
