...
public class SolrCloudReportersTest extends SolrCloudTestCase {
  ...
  public void testExplicitConfiguration() throws Exception {
    String solrXml =
        IOUtils.toString(
            SolrCloudReportersTest.class.getResourceAsStream("/solr/solr-solrreporter.xml"),
            "UTF-8");
    configureCluster(2).withSolrXml(solrXml).configure();
    cluster.uploadConfigSet(
        Paths.get(TEST_PATH().toString(), "configsets", "minimal", "conf"), "test");
    ...
    waitForState(
        "Expected test_collection with 2 shards and 2 replicas",
        "test_collection",
        clusterShape(2, 4));
            ...
            jetty -> {
              ...
              for (String name : cc.getLoadedCoreNames()) {
                ...
                try {
                  ...
                  assertEquals(
                      "solr.core." + collectionName + "." + shardId + "." + replicaName,
                      registryName);
                  assertEquals(
                      "solr.collection." + collectionName + "." + shardId + ".leader",
                      leaderRegistryName);
              ...
              Map<String, SolrMetricReporter> reporters =
                  metricManager.getReporters("solr.cluster");
              ...
              SolrCoreContainerReporter solrCoreContainerReporter =
                  (SolrCoreContainerReporter) reporter;
              assertNotNull(solrCoreContainerReporter.getCoreContainer());
              for (String registryName :
                  metricManager.registryNames(".*\\.shard[0-9]\\.replica.*")) {
  ...
  public void testDefaultPlugins() throws Exception {
    String solrXml =
        IOUtils.toString(
            SolrCloudReportersTest.class.getResourceAsStream("/solr/solr.xml"), "UTF-8");
    configureCluster(2).withSolrXml(solrXml).configure();
    cluster.uploadConfigSet(
        Paths.get(TEST_PATH().toString(), "configsets", "minimal", "conf"), "test");
