...
public class RankFieldTest extends SolrTestCaseJ4 {
  ...
  public void testInternalFieldName() {
    assertEquals(
        "RankField.INTERNAL_RANK_FIELD_NAME changed in an incompatible way",
        "_rank_",
        RankField.INTERNAL_RANK_FIELD_NAME);
  }

  public void testBasic() {
    assertNotNull(h.getCore().getLatestSchema().getFieldOrNull(RANK_1));
    assertEquals(
        RankField.class, h.getCore().getLatestSchema().getField(RANK_1).getType().getClass());
  }
  ...
  public void testAddRandom() {
    for (int i = 0; i < random().nextInt(TEST_NIGHTLY ? 10000 : 100); i++) {
      assertU(adoc("id", String.valueOf(i), RANK_1, Float.toString(random().nextFloat())));
    }
    assertU(commit());
  }

  public void testSkipEmpty() {
    assertU(adoc("id", "1", RANK_1, ""));
  }

  public void testBasicAdd() throws IOException {
    assertU(adoc("id", "testBasicAdd", RANK_1, "1"));
    assertU(commit());
    ...
    h.getCore()
        .withSearcher(
            (searcher) -> {
              ...
              assertTrue(
                  reader
                      .terms(RankField.INTERNAL_RANK_FIELD_NAME)
                      .iterator()
                      .seekExact(new BytesRef(RANK_1.getBytes(StandardCharsets.UTF_8))));
              return null;
            });
  }

  public void testMultipleRankFields() throws IOException {
    assertU(adoc("id", "testMultiValueAdd", RANK_1, "1", RANK_2, "2"));
    assertU(commit());
    // assert that the document made it in
            ...
            (searcher) -> {
              ...
              assertTrue(
                  reader
                      .terms(RankField.INTERNAL_RANK_FIELD_NAME)
                      .iterator()
                      .seekExact(new BytesRef(RANK_2.getBytes(StandardCharsets.UTF_8))));
              assertTrue(
                  reader
                      .terms(RankField.INTERNAL_RANK_FIELD_NAME)
                      .iterator()
  ...
  public void testSortFails() throws IOException {
    ...
    assertU(commit());
    assertQEx(
        "Can't sort on rank field", req("q", "id:testSortFails", "sort", RANK_1 + " desc"), 400);
  }
  ...
  public void testRankQParserQuery() throws IOException {
    ...
    assertQ(
        req(
            "q",
            "str_field:foo _query_:{!rank f='" + RANK_1 + "' function='log' scalingFactor='1'}"),
        "//*[@numFound='2']",
    ...
    assertQ(
        req(
            "q",
            "str_field:foo _query_:{!rank f='" + RANK_2 + "' function='log' scalingFactor='1'}"),
        "//*[@numFound='2']",
    ...
    assertQ(
        req(
            ...
            "str_field^10",
            "bq",
            "{!rank f='" + RANK_1 + "' function='log' scalingFactor='1'}"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.='2']",
        "//result/doc[2]/str[@name='id'][.='1']");
    ...
    assertQ(
        req(
            ...
            "str_field^10",
            "bq",
            "{!rank f='" + RANK_2 + "' function='log' scalingFactor='1'}"),
        "//*[@numFound='2']",
        "//result/doc[1]/str[@name='id'][.='1']",
        "//result/doc[2]/str[@name='id'][.='2']");
  }
  ...
  public void testScoreChanges() throws Exception {
    assertU(adoc("id", "1", "str_field", "foo", RANK_1, "1"));
    assertU(commit());
    ModifiableSolrParams params =
        params(
            "q", "foo", "defType", "dismax", "qf", "str_field^10", "fl", "id,score", "wt", "xml");

    double scoreBefore =
        (Double)
            TestHarness.evaluateXPath(
                h.query(req(params)),
                "//result/doc[1]/float[@name='score']",
