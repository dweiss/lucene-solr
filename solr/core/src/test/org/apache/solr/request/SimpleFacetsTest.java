...
public class SimpleFacetsTest extends SolrTestCaseJ4 {
  ...
  static void randomCommit(int percent_chance) {
    if (random().nextInt(100) <= percent_chance) assertU(commit());
  }
  ...
  public void testDvMethodNegativeFloatRangeFacet() throws Exception {
    ...
    assertTrue(
        ...
        h.getCore().getLatestSchema().getField(field).hasDocValues());
    assertEquals(
        "Unexpected schema configuration",
        NumberType.FLOAT,
        h.getCore().getLatestSchema().getField(field).getType().getNumberType());
    assertFalse(
        "Unexpected schema configuration",
        h.getCore().getLatestSchema().getField(field).multiValued());
    ...
    assertQ(
        req(commonParams, "facet.range", field, "facet.range.method", "filter"),
        String.format(Locale.ROOT, countAssertion, field));
    assertQ(
        req(commonParams, "facet.range", field, "facet.range.method", "dv"),
        String.format(Locale.ROOT, countAssertion, field));
  }
  ...
  public void testCachingBigTerms() throws Exception {
    assertQ(
        req(
            ...
            "true",
            "facet.field",
            "foo_s" // big terms should cause foo_s:A to be cached
            ),
        "*[count(//doc)=6]");
  ...
  private void testSimpleGroupedQueryRangeFacets(String facetLimit) {
    ...
    // repeat the same query using DV method. This is not supported and the query should use filter
    // method instead
    assertQ(
        req(
            "q", "*:*",
  ...
  public void testSimpleGroupedFacets() throws Exception {
    assumeFalse(
        "SOLR-10844: group.facet doesn't play nice with points *OR* DocValues",
  ...
  public void testFacetMatches() {
    ...
    for (String[] aSwitch : uifSwitch) {
      for (String[] methodParam : new String[][] {none, aSwitch}) {
        assertQ(
            "check facet.match filters facets returned",
            req(
                methodParam,
                "q",
                "id:[42 TO 47]",
  ...
  public void testFacetMissing() {
    SolrParams commonParams =
        params("q", "foo_s:A", "rows", "0", "facet", "true", "facet.missing", "true");
  ...
  public void testSimpleFacetCounts() {
    ...
    assertQ(
        "standard request handler returns all matches",
        req("id_i1:[42 TO 47]"),
        "*[count(//doc)=6]");

    assertQ(
        "filter results using fq",
        req(
            "q", "id_i1:[42 TO 46]",
            "fq", "id_i1:[43 TO 47]"),
        "*[count(//doc)=4]");
    ...
    assertQ(
        "don't filter results using blank fq",
        req(
            "q", "id_i1:[42 TO 46]",
            "fq", " "),
        "*[count(//doc)=5]");
    ...
    assertQ(
        "filter results using multiple fq params",
        req(
            "q", "id_i1:[42 TO 46]",
    ...
    final String[] uifSwitch =
        new String[] {(random().nextBoolean() ? "" : "f.trait_s.") + "facet.method", "uif"};
    final String[] none = new String[] {};

    for (String[] methodParam : new String[][] {none, uifSwitch}) {
      assertQ(
          "check counts for facet queries",
          req(
              methodParam,
              "q",
              "id_i1:[42 TO 47]",
      ...
      assertQ(
          "check multi-select facets with naming",
          req(
              methodParam,
              "q",
              "id_i1:[42 TO 47]",
    ...
    assertQ(
        req(
            ...
            "{!key=foo}id:42",
            "facet.query",
            "{!ex=main key=bar}id:42" // only matches when we exclude main query
            ),
    ...
    for (String[] methodParam : new String[][] {none, uifSwitch}) {
      assertQ(
          "check counts for applied facet queries using filtering (fq)",
          req(
              methodParam,
              "q",
              "id_i1:[42 TO 47]",
      ...
      assertQ(
          "check counts with facet.zero=false&facet.missing=true using fq",
          req(
              methodParam,
              "q",
              "id_i1:[42 TO 47]",
      ...
      assertQ(
          "check counts with facet.mincount=1&facet.missing=true using fq",
          req(
              methodParam,
              "q",
              "id_i1:[42 TO 47]",
      ...
      assertQ(
          "check counts with facet.mincount=2&facet.missing=true using fq",
          req(
              methodParam,
              "q",
              "id_i1:[42 TO 47]",
      ...
      assertQ(
          req(
              ...
              "3",
              "facet.sort",
              "false" // false means sort by index order
              ),
          "*[count(//lst[@name='trait_s']/int)=3]",
    ...
    for (String method : new String[] {"fc", "uif"}) {
      assertQ(
          req(
              ...
              "facet.field",
              "zerolen_s",
              (random().nextBoolean() ? "" : "f.zerolen_s.") + "facet.method",
              method),
          "*[count(//lst[@name='zerolen_s']/int[@name=''])=1]");
    }

    assertQ(
        "a facet.query that analyzes to no query shoud not NPE",
        req(
            "q", "*:*",
            "facet", "true",
  ...
  public void testBehaviorEquivilenceOfUninvertibleFalse() throws Exception {
...

    {
      ...
      for (String min : Arrays.asList("0", "1")) {
        ...
        paramSets.add(
            params(
                "facet.field", "{!key=x}trait_s_not_indexed_sS",
                "facet.mincount", min,
                "facet.method", "enum"));
      }
      for (SolrParams p : paramSets) {
        ...
        assertQ(
            "expect no buckets when field is not-indexed or not-uninvertible",
            req(p, "rows", "0", "q", "id_i1:[42 TO 47]", "fq", "id_i1:[42 TO 45]", "facet", "true"),
            "//*[@numFound='4']",
            "*[count(//lst[@name='x'])=1]",
...

    {
      ...
      for (String min : Arrays.asList("0", "1")) {
        paramSets.add(
            params(
                "facet.field", "{!key=x}trait_s_not_uninvert",
                "facet.method", "enum"));
        for (String okField : Arrays.asList("trait_s", "trait_s_not_uninvert_dv")) {
          ...
          for (String method : Arrays.asList("enum", "fc", "fcs", "uif")) {
            paramSets.add(params("facet.field", "{!key=x}" + okField, "facet.method", method));
          }
        }
        for (SolrParams p : paramSets) {
          assertQ(
  ...
  private void helpTestDateFacets(final String fieldName, final FacetRangeMethod rangeFacetMethod) {
    ...
    assertQ(
        "check counts for month of facet by day",
        req(
            "q",
            "*:*",
            "rows",
            ...
            p + ".method",
            rangeFacetMethod
                .toString() // This only applies to range faceting, won't be use for date faceting
            ),
        "*[count(" + pre + "/int)=31]",
    ...
    assertQ(
        "check counts for month of facet by day with global mincount = 1",
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "check counts for month of facet by day with field mincount = 1",
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "check before is not inclusive of upper bound by default",
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "check after is not inclusive of lower bound by default (for dates)",
        req(
            "q",
            "*:*",
            "rows",
  ...
  private void helpTestDateRangeFacetsWithIncludeOption(final String fieldName) {
    ...
    assertQ(
        "checking counts for lower & upper",
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "checking counts for upper & edge",
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "checking counts for upper & outer",
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "checking counts for lower & edge",
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "checking counts for lower & outer",
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "checking counts for lower & edge & outer",
        req(
            "q",
            "*:*",
            "rows",
  ...
  private void helpTestDateRangeFacetsWithTz(final String fieldName) {
    ...
    assumeTrue(
        "Test requires JVM to know about about TZ: " + TZ,
        TimeZoneUtils.KNOWN_TIMEZONE_IDS.contains(TZ));

    assertQ(
        "checking facet counts for fixed now, using TZ: " + TZ,
        req(
            "q",
            "*:*",
            "rows",
    ...
    assertQ(
        "checking facet counts arround DST change for TZ: " + TZ,
        req(
            "q",
            "*:*",
            "rows",
  ...
  public void doFacets(String f, String... params) {
    ...
    assertQ(
        ...
        req(params, "q", "id:[* TO *]", "indent", "true", "facet", "true", "facet.field", f),
    ...
    assertQ(
        "check counts for facet with generous limit",
        req(params, "q", "id:[* TO *]", "facet", "true", "facet.limit", "100", "facet.field", f),
        "*[count(//lst[@name='facet_fields']/lst/int)=7]",
        pre + "/int[1][@name='G'][.='5']",
        pre + "/int[2][@name='E'][.='3']",
        pre + "/int[3][@name='C'][.='2']",
    ...
    assertQ(
        "check counts for limited facet",
        req(params, "q", "id:[* TO *]", "facet", "true", "facet.limit", "2", "facet.field", f),
    ...
    assertQ(
        "test sorted facet paging with zero (don't count in limit)",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test sorted facet paging with zero (test offset correctness)",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test facet unsorted paging, mincount=2",
        req(
            params,
            "q",
            "id:[* TO *]",
  ...
  public void testFacetExistsShouldThrowExceptionForMincountGreaterThanOne() throws Exception {
    ...
    assertQEx(
        "checking global method or per field",
        msg.get(0),
        req(
            "q",
            "id:[* TO *]",
            ...
            "facet",
            "true",
            random().nextBoolean() ? "facet.exists" : "f." + f + ".facet.exists",
            "true",
            "facet.field",
            f,
  ...
  public void testFacetPrefixSingleValuedFcs() {
    ...
    doFacetPrefix(
        "tt_s1", "{!threads=-1}", "", "facet.method", "fcs"); // default / unlimited threads
  ...
  private void doFacetExclude(String f, String g, String termSuffix, String... params) {
    ...
    final SolrQueryRequest groupReq =
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test facet.exclude for grouped facets",
        groupReq,
    ...
    assertQ(
        "test facet.exclude for grouped facets with facet.limit=0, facet.missing=true",
        groupReq,
        "*[count(//lst[@name='facet_fields']/lst/int)=1]",
        pre + "/int[.='1']");
  }
  ...
  private void doFacetContainsAndExclude(
      String f,
      String g,
      String termSuffix,
      String contains,
    ...
    assertQ(
        "test facet.contains with facet.exclude",
        req,
        "*[count(//lst[@name='facet_fields']/lst/int)=2]",
    ...
    assertQ(
        "test facet.contains with facet.exclude for grouped facets",
        groupReq,
  ...
  public void testFacetContainsUif() {
    doFacetContains(
        "contains_s1", "contains_group_s1", "Astra", "BAst", "Ast", "facet.method", "uif");
  ...
  public void testFacetContains() {
    doFacetContains(
        "contains_s1", "contains_group_s1", "Astra", "BAst", "Ast", "facet.method", "enum");
    doFacetContains(
        "contains_s1", "contains_group_s1", "Astra", "BAst", "Ast", "facet.method", "fcs");
    doFacetContains(
        "contains_s1", "contains_group_s1", "Astra", "BAst", "Ast", "facet.method", "fc");
    doFacetContains(
        "contains_s1",
        "contains_group_s1",
        "Astra",
  ...
  public void doFacetPrefix(String f, String local, String termSuffix, String... params) {
    ...
    assertQ(
        "test facet.prefix middle, exact match first term",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test facet.prefix middle, exact match first term, unsorted",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test facet.prefix middle, paging",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test facet.prefix end, not exact match",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test facet.prefix end, exact match",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test facet.prefix at start, exact match",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        "test facet.prefix at Start, not exact match",
        req(
            params,
            "q",
            "id:[* TO *]",
    ...
    assertQ(
        ...
        pre + "/int[1][@name='AAA" + termSuffix + "'][.='1']");
    assertQ(
        "test facet.prefix before start",
        req(
            params,
            "q",
            "id:[* TO *]",
  ...
  public void doFacetContains(
    ...
    assertQ(
        "test facet.contains for grouped facets",
        req(
            params,
            "q",
            "id:[* TO *]",
  ...
  /**
   * kind of an absurd test because if there is an infinite loop, it would never finish -- but at
   * least it ensures that <i>if</i> one of these requests return, they return an error
   */
  public void testRangeFacetInfiniteLoopDetection() {

    for (String field : new String[] {"foo_f", "foo_d", "foo_i"}) {
      assertQEx(
          "no zero gap error: " + field,
          req(
              "q", "*:*",
              "facet", "true",
              "facet.range", field,
    ...
    String field = "foo_dt";
    assertQEx(
        "no zero gap error for facet.range: " + field,
        req(
            "q", "*:*",
            "facet", "true",
            "facet.range", field,
    ...
    field = "foo_f";
    assertQEx(
        "no float underflow error: " + field,
        req(
            "q", "*:*",
            "facet", "true",
            "facet.range", field,
    ...
    field = "foo_d";
    assertQEx(
        "no double underflow error: " + field,
        req(
            "q", "*:*",
            "facet", "true",
            "facet.range", field,
  ...
  private void doTestRangeQueryHardEndParam(String field, FacetRangeMethod method) {
    assertQ(
        ...
        "*[count(//lst[@name='facet_ranges']/lst[@name='"
            + field
            + "']/lst[@name='counts']/int)=1]",
        "//lst[@name='facet_ranges']/lst[@name='"
            + field
            + "']/lst[@name='counts']/int[@name='43'][.='5']",
  ...
  private void doTestRangeQueryOtherParam(String field, FacetRangeMethod method) {
    ...
    for (SolrQueryRequest req :
        new SolrQueryRequest[] {
          ...
          req(
              ...
              "id," + field,
              "facet.range",
              "{!facet.range.other=before facet.range.other=after}" + field,
              "facet.range.method",
              method.toString(),
              "facet.range.start",
              "43",
      ...
      assertQ(
          "Test facet.range.other: " + req.toString(),
          req,
          "*[count(//lst[@name='facet_ranges']/lst)=1]",
          "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "'])=1]",
          "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "']/lst[@name='counts'])=1]",
    ...
    assertQ(
        ...
        req(
            ...
            "1",
            "facet.range.other",
            FacetRangeOther.ALL.toString(),
            "facet.range.include",
            FacetRangeInclude.LOWER.toString()),
        "*[count(//lst[@name='facet_ranges']/lst)=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "'])=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "']/lst[@name='counts'])=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='"
    ...
    assertQ(
        ...
        req(
            ...
            FacetRangeOther.ALL.toString(),
            "facet.range.include",
            FacetRangeInclude.UPPER.toString()),
        "*[count(//lst[@name='facet_ranges']/lst)=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "'])=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "']/lst[@name='counts'])=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='"
    ...
    assertQ(
        ...
        req(
            ...
            FacetRangeOther.ALL.toString(),
            "facet.range.include",
            FacetRangeInclude.EDGE.toString()),
        "*[count(//lst[@name='facet_ranges']/lst)=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "'])=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "']/lst[@name='counts'])=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='"
    ...
    assertQ(
        ...
        req(
            ...
            FacetRangeOther.ALL.toString(),
            "facet.range.include",
            FacetRangeInclude.OUTER.toString()),
        "*[count(//lst[@name='facet_ranges']/lst)=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "'])=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='" + field + "']/lst[@name='counts'])=1]",
        "*[count(//lst[@name='facet_ranges']/lst[@name='"
            ...
            () ->
                h.query(
                    req(
                        ...
                        "facet.interval",
                        "range_facet_l",
                        "f.range_facet_l.facet.interval.set",
                        "(43,60]")));
    assertEquals(ErrorCode.BAD_REQUEST.code, ex.code());
  ...
  public void testRangeFacetFilterVsDocValuesRandom() throws Exception {
    for (int i = 0; i < atLeast(100); i++) {
      ...
      try {
        ...
        rangeFacetsFilter =
            (NamedList<Object>)
                ((NamedList<Object>) rsp.getValues().get("facet_counts")).get("facet_ranges");
      } finally {
        req.close();
      }
      ...
      assertSameResults(
          "Different results obtained when using 'filter' and 'dv' methods for Range Facets using params."
              + params
              + "\n"
              + "Filter:"
              + rangeFacetsFilter
              + "\n DV: "
              + rangeFacetsDv,
          (NamedList<Object>) rangeFacetsFilter.get(field),
          (NamedList<Object>) rangeFacetsDv.get(field));
    }
  }
  ...
  public void testFacetPrefixWithFacetThreads() throws Exception {
    assertQ(
        ...
        req(
            ...
            "true",
            "facet.field",
            "{!key=key1 facet.prefix=foo}myfield_s",
            "facet.field",
            "{!key=key2 facet.prefix=bar}myfield_s",
            "facet.threads",
            "1"),
  ...
  private void assertSameResults(
      String message, NamedList<Object> rangeFacetsFilter, NamedList<Object> rangeFacetsDv) {
    assertEquals(
        message + " Different number of elements.", rangeFacetsFilter.size(), rangeFacetsDv.size());
    ...
    assertEquals(
        message + "Different counts", rangeFacetsFilter.get("counts"), rangeFacetsDv.get("counts"));
  }

  private ModifiableSolrParams getRandomParamsInt() {
    String field =
        new String[] {
              "range_facet_l_dv", "range_facet_i_dv", "range_facet_l", "duration_i1", "id_i1"
            }
            [random().nextInt(5)];
  ...
  private static final String[] DATE_GAP_UNITS =
      new String[] {"SECONDS", "MINUTES", "HOURS", "DAYS", "MONTHS", "YEARS"};
  ...
  private void addCommonRandomRangeParams(ModifiableSolrParams params) {
    for (int i = 0; i < random().nextInt(2); i++) {
      params.add(
          ...
          FacetRangeOther.values()[random().nextInt(FacetRangeOther.values().length)].toString());
    }
    if (random().nextBoolean()) {
      params.add(
          FacetParams.FACET_RANGE_INCLUDE,
          FacetRangeInclude.values()[random().nextInt(FacetRangeInclude.values().length)]
              .toString());
    }
    if (random().nextBoolean()) {
