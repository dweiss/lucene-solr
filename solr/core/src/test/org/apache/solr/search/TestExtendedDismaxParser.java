...
public class TestExtendedDismaxParser extends SolrTestCaseJ4 {
  ...
  public void testFocusQueryParser() {
    ...
    assertQ(
        req(
            ...
            "true"),
        nor);
    // When sow=false, the per-field query structures differ (no "Terminator" query on integer field
    // foo_i),
    // so a dismax-per-field is constructed.  As a result, mm=100% is applied per-field instead of
    // per-term;
    // since there is only one term (100) required in the foo_i field's dismax, the query can match
    // docs that
    // only have the 100 term in the foo_i field, and don't necessarily have "Terminator" in any
    // field.
    assertQ(
        req(
            "defType",
    ...
    assertQ(
        req(
            ...
            "Terminator: 100",
            "qf",
            "movies_t foo_i"), // default sow=false
        oner);

    assertQ(
        req("defType", "edismax", "mm", "100%", "q", "Terminator: 8", "qf", "movies_t foo_i"),
        oner);
  ...
  public void testAutoGeneratePhraseQueries() throws Exception {
    ...
    for (SolrParams params : Arrays.asList(noSowParams, sowFalseParams)) {
      try (SolrQueryRequest req = req(params)) {
        ...
        Query q = qParser.getQuery();
        assertEquals(
            "+(((text:\"crow blackbird\" text:grackl))"
        ...
        q = qParser.getQuery();
        assertEquals(
            "+(((text:\"crow blackbird\" text:grackl) text:wifi)"
  ...
  public void testSowFalseWithBoost() throws Exception {
    ...
    try (SolrQueryRequest req = req("sow", "false", "qf", "subject title^5")) {
      ...
      Query q = qParser.getQuery();
      assertEquals(
          "+(((title:one)^5.0 | subject:on) ((title:two)^5.0 | subject:two))", q.toString());
    }
    try (SolrQueryRequest req = req("sow", "false", "qf", "subject^3 title")) {
      QParser qParser = QParser.getParser("one two", "edismax", req);
      Query q = qParser.getQuery();
      assertEquals(
          "+((title:one | (subject:on)^3.0) (title:two | (subject:two)^3.0))", q.toString());
    }
    try (SolrQueryRequest req = req("sow", "false", "qf", "subject^10 title^20")) {
      QParser qParser = QParser.getParser("one two", "edismax", req);
      Query q = qParser.getQuery();
      assertEquals(
          "+(((title:one)^20.0 | (subject:on)^10.0) ((title:two)^20.0 | (subject:two)^10.0))",
          q.toString());
    }
  }
  ...
  public void testValidateQueryFields() throws Exception {
    ...
    assertTrue(
        response.contains(
            "+DisjunctionMaxQuery((title:olive | "
