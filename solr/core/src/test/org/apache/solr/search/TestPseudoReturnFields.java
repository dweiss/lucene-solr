...
public class TestPseudoReturnFields extends SolrTestCaseJ4 {
  ...
  public static String[] SCORE_AND_REAL_FIELDS = new String[] {"score,*", "*,score"};
  ...
  public void testMultiValued() throws Exception {
    ...
    for (String name : new String[] {"score", "val_ss"}) {
      ...
      assertNotNull(
          "Test depends on a (dynamic) field mtching '"
              + name
  ...
  public void testFunctionsAndScore() throws Exception {
    ...
    assertQ(
        ...
        req("q", "*:*", "rows", "1", "fl", "score,log(val_i),abs(val_i)"),
        "//result[@numFound='5']",
        "//result/doc/float[@name='score']",
        "//result/doc/double[@name='log(val_i)']",
        "//result/doc/float[@name='abs(val_i)']",
    ...
    assertQ(
        ...
        req("q", "*:*", "rows", "1", "fl", "score", "fl", "log(val_i)", "fl", "abs(val_i)"),
        "//result[@numFound='5']",
        "//result/doc/float[@name='score']",
        "//result/doc/double[@name='log(val_i)']",
        "//result/doc/float[@name='abs(val_i)']",
  ...
  public void testGlobs() throws Exception {
    ...
    for (SolrParams p :
        Arrays.asList(
            params("q", "*:*", "rows", "1", "fl", "val_*,subj*,ss*"),
    ...
    for (String id : Arrays.asList("42", "99")) {
      for (SolrParams p :
          Arrays.asList(
              ...
              params(
                  "fl", "[docid],[shard],abs(val_i)", "fl", "[explain],x_alias:[value v=10 t=int]"),
              params(
                  "fl",
                  "[docid],[shard]",
                  "fl",
                  "[explain],x_alias:[value v=10 t=int]",
                  "fl",
                  "abs(val_i)"),
              params(
                  "fl",
    ...
    for (String id : Arrays.asList("42", "99")) {
      assertQ(
          ...
          req(
              ...
              "xml",
              "fl",
              "x_alias:[value v=10 t=int],score,abs(val_i),[docid]"),
          "//doc/int[@name='[docid]'][.>=-1]",
          "//doc/float[@name='abs(val_i)'][.='1.0']",
          "//doc/int[@name='x_alias'][.=10]",
          "//doc[count(*)=3]");
      for (SolrParams p :
          Arrays.asList(
              params("fl", "[docid],x_alias:[value v=10 t=int],[explain],score,abs(val_i)"),
              params(
                  "fl", "x_alias:[value v=10 t=int],[explain]", "fl", "[docid],score,abs(val_i)"),
              params(
                  "fl",
                  "[docid]",
                  "fl",
  ...
  public void testAugmentersGlobsExplicitAndScoreOhMyRTG() throws Exception {
    ...
    final List<String> fl =
        Arrays.asList("id", "[explain]", "score", "val_*", "subj*", "abs(val_i)", "[docid]");
