  ...
  public void test() {
    ...
    String[] should_matches =
        new String[] {
          ...
          // "{!payload_check f=vals_dpi payloads='1 2.0'}A B",  // ideally this should pass, but
          // IntegerEncoder can't handle "2.0"
          "{!payload_check f=vals_dpi payloads='1 2 3'}A B C",
          "{!payload_check f=vals_dpf payloads='1 2'}one two",
          "{!payload_check f=vals_dpf payloads='1 2.0'}one two", // shows that FloatEncoder can
          // handle "1"
          "{!payload_check f=vals_dps payloads='NOUN VERB'}cat jumped"
        };

    String[] should_not_matches =
        new String[] {
    ...
    for (String should_not_match : should_not_matches) {
      assertQ(
          should_not_match, req("fl", "*,score", "q", should_not_match), "//result[@numFound='0']");
    }
  }
}
