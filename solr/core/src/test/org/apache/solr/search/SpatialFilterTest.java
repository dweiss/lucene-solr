...
public class SpatialFilterTest extends SolrTestCaseJ4 {
  ...
  private void checkHits(
      String fieldName, boolean exact, String pt, double distance, int count, int... docIds) {
    ...
    assertQ(
        req(
            ...
            "id",
            "q",
            "*:* OR foo_i:" + random().nextInt(100),
            "rows",
            "1000",
            "fq",
            "{!" + method + " sfield=" + fieldName + "}",
            "pt",
            pt,
            "d",
    ...
    assertEquals(
        postFilterCount,
        DelegatingCollector.setLastDelegateCount); // post filtering shouldn't be used
    ...
    assertQ(
        req(
            ...
            "1000",
            "fq",
            "{!" + method + " sfield=" + fieldName + " cache=false" + "}",
            "pt",
            pt,
            "d",
            String.valueOf(distance)),
    ...
    if (fieldName.endsWith("ll")) {
      ...
      assertQ(
          req(
              ...
              "id",
              "q",
              "*:* OR foo_i:" + random().nextInt(100) + 100,
              "rows",
              "1000",
              "fq",
              "{!" + method + " sfield=" + fieldName + " cache=false cost=150" + "}",
              "pt",
              pt,
              "d",
              String.valueOf(distance)),
      ...
      assertEquals(
          postFilterCount + 1,
          DelegatingCollector.setLastDelegateCount); // post filtering *should* have been used
    }
  }
}
