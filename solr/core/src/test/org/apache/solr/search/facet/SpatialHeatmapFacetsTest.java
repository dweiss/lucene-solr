...
public class SpatialHeatmapFacetsTest extends BaseDistributedSearchTestCase {
  ...
  public void testClassicFacets() throws Exception { // AKA SimpleFacets
    ...
    SolrParams baseParams =
        params("q", "*:*", "rows", "0", "facet", "true", FacetParams.FACET_HEATMAP, FIELD);
    ...
    assertEquals(
        6,
        getHmObj(query(params(baseParams, FacetParams.FACET_HEATMAP_DIST_ERR_PCT, "0.10")))
            .get("gridLevel"));

    // test key output label doing 2 heatmaps with different settings on the same field
    {
      final ModifiableSolrParams params =
          params(baseParams, FacetParams.FACET_HEATMAP_DIST_ERR_PCT, "0.10");
      ...
      params.add(
          ...
          "{!key=course "
              + FacetParams.FACET_HEATMAP_LEVEL
              + "=2 "
              + FacetParams.FACET_HEATMAP_FORMAT
              + "="
              + courseFormat
              + "}"
              + FIELD);
    ...
    QueryResponse response =
        query(
            params(
                baseParams,
                "fq",
                "{!tag=excludeme}id:0", // filter to only be id:0
                FacetParams.FACET_HEATMAP,
                "{!ex=excludeme}" + FIELD, // exclude the filter
                FacetParams.FACET_HEATMAP_GEOM,
                "[\"50 20\" TO \"180 90\"]",
                FacetParams.FACET_HEATMAP_LEVEL,
                "4"));
    ...
    assertEquals(
        Arrays.asList(
            ...
            null,
            null,
            null // no data here (below edge of rect 0)
            ),
        counts);
    ...
    assertNull(
        getHmObj(
                query(
                    params(
                        baseParams, FacetParams.FACET_HEATMAP_GEOM, "ENVELOPE(0, 10, -80, -90)")))
            .get("counts_ints2D"));

    Object v =
        getHmObj(query(params(baseParams, FacetParams.FACET_HEATMAP_FORMAT, "png")))
            .get("counts_png");
  ...
  public void testJsonFacets() throws Exception {
    ...
    try {
      query(
          params(
              baseParams,
              "json.facet",
              "{f1:{type:heatmap, f:" + FIELD + ", geom:'" + testBox + "', distErr:0}}"));
      fail();
    } catch (SolrException e) {
      assertEquals(SolrException.ErrorCode.BAD_REQUEST.code, e.code());
    }
    try {
      query(
          params(
              baseParams,
              "json.facet",
              "{f1:{type:heatmap, f:" + FIELD + ", geom:'" + testBox + "', distErrPct:0}}"));
      fail();
    } catch (SolrException e) {
      assertEquals(SolrException.ErrorCode.BAD_REQUEST.code, e.code());
    }
        ...
        getHmObj(
                query(
                    params(
                        baseParams,
                        "json.facet",
                        "{f1:{type:heatmap, f:" + FIELD + ", geom:'" + testBox + "'}}")))
            .get("gridLevel")); // default
    assertEquals(
        3,
        getHmObj(
        ...
        getHmObj(
                query(
                    params(
                        baseParams,
                        "json.facet",
                        "{f1:{type:heatmap, f:" + FIELD + ", distErrPct:0.10}}")))
            .get("gridLevel"));
        ...
        getHmObj(
            query(
                params(
                    ...
                    "{f1:{type:heatmap, f:"
                        + FIELD
                        + " geom:'[\"50 20\" TO \"180 90\"]', gridLevel:4}}")));
    List<List<Integer>> counts = (List<List<Integer>>) hmObj.get("counts_ints2D");
    List<List<Integer>> expectedCounts1 =
        Arrays.asList(
            Arrays.asList(0, 0, 2, 1, 0, 0),
            Arrays.asList(0, 0, 1, 1, 0, 0),
            Arrays.asList(0, 1, 1, 1, 0, 0),
    ...
    QueryResponse response =
        query(
            params(
                ...
                "{!tag=excludeme}id:0", // filter to only be id:0
                "json.facet",
                "{f1:{type:heatmap, excludeTags:['excludeme'], f:"
                    + FIELD
                    + ", geom:'[\"50 20\" TO \"180 90\"]', gridLevel:4}}"));
    ...
    {
      // impractical example but nonetheless encloses the points of both doc3 and doc4 (both of
      // which are points)
      final String jsonHeatmap =
          "facet:{hm:{type:heatmap, f:"
              + FIELD
              + ", geom:'MULTIPOINT(70 60, 91 89)', distErrPct:0.2}}";
      response =
          query(
              params(
                  baseParams,
                  "json.facet",
                  "{"
                      + "q1:{type:query, q:'id:3', "
                      + jsonHeatmap
                      + " }, "
                      + "q2:{type:query, q:'id:4', "
                      + jsonHeatmap
                      + " } "
                      + "}"));
      {
        ...
        getHmObj(
            query(
                params(
                    ...
                    "{f1:{type:heatmap, f:"
                        + FIELD
                        + ", geom:'BUFFER(POINT(110 40), 7)', gridLevel:7}}")));
    counts = (List<List<Integer>>) hmObj.get("counts_ints2D");
    assertEquals(
        Arrays.asList(
            Arrays.asList(0, 1, 1, 1, 1, 1, 1, 0), // curved; we have a 0
        ...
        getHmObj(
                query(
                    params(
                        baseParams,
                        "json.facet",
                        "{f1:{type:heatmap, f:" + FIELD + ", geom:'ENVELOPE(0, 10, -80, -90)'}}")))
            .get("counts_ints2D"));
        ...
        getHmObj(
                query(
                    params(
                        baseParams,
                        "json.facet",
                        "{f1:{type:heatmap, f:" + FIELD + ", format:png }}")))
            .get("counts_png");
    assertTrue(v instanceof byte[]);
    // simply test we can read the image
    assertNotNull(FacetHeatmap.PngHelper.readImage((byte[]) v));
  ...
  private NamedList getHmObj(QueryResponse response) {
    ...
    final NamedList classicResp =
        (NamedList) response.getResponse().findRecursive("facet_counts", "facet_heatmaps", FIELD);
    if (classicResp != null) {
      return classicResp;
    }
    // JSON Facet
