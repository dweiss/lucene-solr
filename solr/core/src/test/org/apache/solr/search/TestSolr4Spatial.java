...
/**
 * Test Solr 4's new spatial capabilities from the new Lucene spatial module. Don't thoroughly test
 * it here because Lucene spatial has its own tests. Some of these tests were ported from Solr 3
 * spatial tests.
 */
public class TestSolr4Spatial extends SolrTestCaseJ4 {
  ...
  public void testBadShapeParse400() {
    assertQEx(
        ...
        req(
            ...
            "1000",
            "fq",
            "{!field f=" + fieldName + "}Intersects(NonexistentShape(89.9,-130 d=9))"),
        400);
    assertQEx(
        null,
        req(
  ...
  private void checkHits(
      String fieldName, String pt, double distKM, double sphereRadius, int count, int... docIds)
  ...
  private void checkHits(
    ...
    // check total length last; maybe response includes ids it shouldn't.  Nicer to check this last
    // instead of first so
  ...
  private String randomScoreMode() {
    return canCalcDistance
        ? new String[] {null, "none", "distance", "recipDistance"}[random().nextInt(4)]
        : "none";
  }
  ...
  public void testRangeSyntax() {
    ...
    assertQ(
        req(
            ...
            "*:*",
            "rows",
            "1000", // testing quotes in range too
            "fq",
            "{! "
                + (score == null ? "" : " score=" + score)
                + " df="
                + fieldName
                + "}[32,-80 TO \"33 , -79\"]"), // lower-left to upper-right
  ...
  public void testSort() throws Exception {
    ...
    if (random().nextBoolean()) {
      assertU(
          adoc("id", "999", fieldName, "70,70")); // far away from these queries; we filter it out
    } else {
    ...
    //
    //  NOTE: the rest work via the score of the spatial query. Generally, you should use geodist()
    // instead.
    //

    // test absence of score=distance means it doesn't score
    assertJQ(
        req("q", radiusQuery(3, 4, 9, null, null), "fl", "id,score"),
        1e-9,
  ...
  public void testSortMultiVal() throws Exception {
    assumeTrue("dist sorting not supported on field " + fieldName, canCalcDistance);
    assumeFalse(
        "Multivalue not supported for this field",
        fieldName.equals("pointvector") || isBBoxField(fieldName));

    assertU(adoc("id", "100", fieldName, "1,2")); // 1 point
    assertU(
        adoc(
            "id", "101", fieldName, "4,-1", fieldName,
            "3,5")); // 2 points, 2nd is pretty close to query point
    assertU(commit());
