...
public class TestMinMaxOnMultiValuedField extends SolrTestCaseJ4 {
  ...
  public static void beforeClass() throws Exception {
    ...
    checkFields(
        new String[] {
          "str", // no expectation on missing first/last
          "str_missf_",
          "str_missl_",
          "int_missf_",
          "int_missl_",
    ...
    checkFields(
        new String[] {
          "stxt_", // no expectation on missing first/last
          "stxt_missf_",
          "stxt_missl_"
        },
        new String[] {"_dv"});
    checkFields(
        new String[] {"stxt_"}, // no expectation on missing first/last
  ...
  private static void checkFields(String[] types, String[] suffixes) {
    ...
    for (String type : types) {
      for (String suffix : suffixes) {
        ...
        assertEquals(
            f + " doesn't have expected docValues status",
  ...
  private void testBasics(String intField, String longField, String floatField, String doubleField)
    ...
    assertTrue(
        "Unexpected long field",
        h.getCore().getLatestSchema().getField(longField).getType() instanceof LongValueFieldType);
    assertTrue(
        "Unexpected float field",
        h.getCore().getLatestSchema().getField(floatField).getType()
            instanceof FloatValueFieldType);
    assertTrue(
        "Unexpected double field",
        h.getCore().getLatestSchema().getField(doubleField).getType()
            instanceof DoubleValueFieldType);
    ...
    assertQ(
        req(
            ...
            "fl",
            "exists_max_i:exists(field(" + intField + ",max))",
            "fl",
            ...
            "fl",
            "exists_max_l:exists(field(" + longField + ",max))",
            "fl",
            ...
            "fl",
            "exists_max_f:exists(field(" + floatField + ",max))",
            "fl",
            ...
            "fl",
            "exists_max_d:exists(field(" + doubleField + ",max))",
            "fl",
  ...
  private void checkBasicStrings(final String field) {
    assertU(adoc(sdoc("id", "1", field, "dog", field, "xyz", field, "cat")));
    ...
    assertQ(
        req(
            ...
            "id:1",
            "fl",
            "exists_min_str:exists(field(" + field + ",min))",
            "fl",
            "exists_max_str:exists(field(" + field + ",max))",
            "fl",
            "min_str:field(" + field + ",min)",
            "fl",
            "max_str:field(" + field + ",max)"),
        "//*[@numFound='1']",
  ...
  public void testExpectedSortOrderingStrings() {
    testExpectedSortOrdering("val_strs_dv", false, null, "a", "cat", "dog", "wako", "xyz", "zzzzz");
  }

  public void testExpectedSortOrderingSortableText() {
    testExpectedSortOrdering(
        "val_stxt_s_dv", false, null, "a", "cat", "dog", "wako", "xyz", "zzzzz");
  }
  ...
  public void testExpectedSortMissingOrderings() {
    ...
    testSortMissingMinMax(
        "val_int", Integer.MIN_VALUE + 1L, -9999, 0, 99999, Integer.MAX_VALUE - 1L);
    testSortMissingMinMax(
        "val_long", Long.MIN_VALUE + 1L, -99999999L, 0, 9999999999L, Long.MAX_VALUE - 1L);
    testSortMissingMinMax(
        "val_float",
        Math.nextAfter(Float.NEGATIVE_INFINITY, 0F),
        -99.99F,
        0F,
        99.99F,
        Math.nextAfter(Float.POSITIVE_INFINITY, 0F));
    testSortMissingMinMax(
        "val_double",
        Math.nextAfter(Double.NEGATIVE_INFINITY, 0D),
        -99.99D,
        0D,
        99.99D,
        Math.nextAfter(Double.POSITIVE_INFINITY, 0F));
    testSortMissingMinMax(
        "val_date",
  ...
  public void testBadRequests() {
    ...
    assertQEx(
        "no error asking for bogus selector",
        "hoss",
    ...
    assertQEx(
        "no error asking for max on a non docVals field",
        "val_is_ndv_p",
    ...
    assertQEx(
        "no error mentioning field name when asking for max on type that doesn't support it",
        "cat_length",
        req("q", "*:*", "fl", "field(cat_length,'max')"),
        SolrException.ErrorCode.BAD_REQUEST);
    assertQEx(
        "no error mentioning type when asking for max on type that doesn't support it",
        "text_length",
        req("q", "*:*", "fl", "field(cat_length,'max')"),
        SolrException.ErrorCode.BAD_REQUEST);
    // type supports, but field doesn't have docValues
    assertQEx(
        "no error mentioning field name when asking for max on a non-dv str field",
        "cat",
        req("q", "*:*", "fl", "field(cat,'max')"),
        SolrException.ErrorCode.BAD_REQUEST);
    assertQEx(
        "no error mentioning 'docValues' when asking for max on a non-dv str field",
        "docValues",
        req("q", "*:*", "fl", "field(cat,'max')"),
        SolrException.ErrorCode.BAD_REQUEST);
    assertQEx(
        "no error mentioning field name when asking for max on a non-dv sortable text field",
        "val_stxt_s_nodv",
        req("q", "*:*", "fl", "field(val_stxt_s_nodv,'max')"),
        SolrException.ErrorCode.BAD_REQUEST);
    assertQEx(
        "no error mentioning 'docValues' when asking for max on a non-dv sortable field",
        "docValues",
        req("q", "*:*", "fl", "field(val_stxt_s_nodv,'max')"),
        SolrException.ErrorCode.BAD_REQUEST);
  }
  ...
  protected void testSimpleFloat(final String fieldname) {
    ...
    testSimpleValues(
        fieldname, float.class, Float.POSITIVE_INFINITY, 0.0F, Float.NEGATIVE_INFINITY);
  ...
  protected void testSimpleDouble(final String fieldname) {
    ...
    testSimpleValues(
        fieldname, double.class, Double.POSITIVE_INFINITY, 0.0D, Double.NEGATIVE_INFINITY);
  ...
  protected void testSimpleValues(
  ...
  private void testExpectedSortOrderingInt(final String f, final boolean trieFieldHack) {
    ...
    testExpectedSortOrdering(
        f, trieFieldHack, Integer.MIN_VALUE, -9999, 0, 1000, Integer.MAX_VALUE);
  ...
  private void testExpectedSortOrderingLong(final String f, final boolean trieFieldHack) {
    // first a quick test where every doc has a value
    testExpectedSortOrdering(
        f, trieFieldHack, Long.MIN_VALUE, -4200L, 0, 121212112, Long.MAX_VALUE);
  ...
  private void testExpectedSortOrderingFloat(final String f, final boolean trieFieldHack) {
    // first a quick test where every doc has a value
    testExpectedSortOrdering(
        f, trieFieldHack, Float.NEGATIVE_INFINITY, -15.0, 0F, 121212.112, Float.POSITIVE_INFINITY);
  ...
  /**
   * Given a <code>fieldPrefix</code> and a list of sorted values which may <em>not</em> contain
   * null, this method tests that sortMissingLast and sortMissingFirst fields using those prefixes
   * sort correctly when {@link #buildMultiValueSortedDocuments} is used to generate documents
   * containing these values <em>and</em> an additional document with no values in the field.
   *
   * <p>Permutations tested:
   *
   * <ul>
   *   <li><code>fieldPrefix</code> + <code>"_missf_s_dv"</code> asc
   *   <li><code>fieldPrefix</code> + <code>"_missf_s_dv"</code> desc
   *   <li><code>fieldPrefix</code> + <code>"_missl_s_dv"</code> asc
   *   <li><code>fieldPrefix</code> + <code>"_missl_s_dv"</code> desc
   * </ul>
   *
   * @see #buildMultiValueSortedDocuments
   * @see #testExpectedSortOrdering(String,List)
   */
  private void testSortMissingMinMax(final String fieldPrefix, Object... sortedValues) {
  ...
  /**
   * Given a (multivalued) field name and an (ascending) sorted list of values, this method uses
   * {@link #buildMultiValueSortedDocuments} to generate and test multiple function &amp; sort
   * permutations ...
   *
   * <ul>
   *   <li><code>f asc</code> (implicitly min)
   *   <li><code>field(f,min) asc</code>
   *   <li><code>field(f,min) desc</code>
   *   <li><code>f desc</code> (implicitly max)
   *   <li><code>field(f,max) desc</code>
   *   <li><code>field(f,max) asc</code>
   * </ul>
   *
   * <p><b>NOTE:</b> if the sortedValues includes "null" then the field must <em>NOT</em> use <code>
   * sortMissingFirst</code> or <code>sortMissingLast</code></b>
   *
   * @param f the field to test
   * @param trieFieldHack if this param and {@link #NUMERIC_POINTS_SYSPROP} are both true, then the
   *     <code>field(f,min|max)</code> functions will be wrapped in <code>def(...,0)</code> and the
   *     implicit <code>f asc|desc</code> syntax will not be tested -- see SOLR-8005 for the reason.
   * @param sortedValues the values to use when building the docs and validating the sort
   * @see #buildMultiValueSortedDocuments
   * @see #testExpectedSortOrdering(String,List)
   * @see #clearIndex
   */
  private void testExpectedSortOrdering(
      final String f, boolean trieFieldHack, Object... sortedValues) {

    SchemaField sf = h.getCore().getLatestSchema().getField(f);
    assertFalse(
        "this utility method does not work with fields that are sortMissingFirst|Last: " + f,
   ...
   * and then add the documents in a random order (to ensure the index insertion order is not a
   * factor) and then validate that a <code>*:*</code> query returns the documents in the original
   * order.
   *
   * @see #buildMultiValueSortedDocuments
   * @see #clearIndex
   */
  private void testExpectedSortOrdering(
      final String sort, final List<SolrInputDocument> sortedDocs) {
    clearIndex();
  ...
  /**
   * Given a (multivalued) field name and an (ascending) sorted list of values, this method will
   * generate a List of Solr Documents of the same size such that:
   *
   * <ul>
   *   <li>For each non-null value in the original list, the corrisponding document in the result
   *       will have that value in the specified field.
   *   <li>For each null value in the original list, the corrisponding document in teh result will
   *       have <em>NO</em> values in the specified field.
