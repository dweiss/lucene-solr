...
public class AsyncCallRequestStatusResponseTest extends SolrCloudTestCase {
  ...
  public void testAsyncCallStatusResponse() throws Exception {
    ...
    Create createCollection =
        CollectionAdminRequest.createCollection("asynccall", "conf", numShards, numReplicas);
    String asyncId = createCollection.processAsync(cluster.getSolrClient());

    waitForState(
        "Expected collection 'asynccall' to have "
            + numShards
            + " shards and "
    ...
    RequestStatusState state =
        AbstractFullDistribZkTestBase.getRequestStateAfterCompletion(
            asyncId, 30, cluster.getSolrClient());
