...
public class ReplicationFactorTest extends AbstractFullDistribZkTestBase {
  ...
  protected void sendNonDirectUpdateRequestReplica(
    ...
    try (HttpSolrClient solrServer = getHttpSolrClient(url)) {
      ...
      if (up.getParams() != null && up.getParams().get(UpdateRequest.MIN_REPFACT) != null) {
        // If "min_rf" was specified in the request, it must be in the response for back
        // compatibility
        assertNotNull(
            "Expecting min_rf to be in the response, since it was explicitly set in the request",
            hdr.get(UpdateRequest.MIN_REPFACT));
        assertEquals(
            "Unexpected min_rf in the response",
            Integer.parseInt(up.getParams().get(UpdateRequest.MIN_REPFACT)),
            hdr.get(UpdateRequest.MIN_REPFACT));
      }
    }
  }
  ...
  protected void doDelete(
      UpdateRequest req, String msg, int expectedRf, int retries, boolean minRfExplicit)
  ...
  private int runAndGetAchievedRf(UpdateRequest up, boolean minRfExplicit, int minRf)
