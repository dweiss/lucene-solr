...
public abstract class AbstractCloudBackupRestoreTestCase extends SolrCloudTestCase {
  ...
  private void testBackupAndRestore(String collectionName, int backupReplFactor) throws Exception {
    ...
    int computeRestoreMaxShardsPerNode =
        (int)
            Math.ceil(
                (restoreReplFactor * numShards / (double) cluster.getJettySolrRunners().size()));
    ...
    assertEquals(
        sameConfig ? "conf1" : "customConfigName",
  ...
  private Map<String, Integer> getShardToDocCountMap(
