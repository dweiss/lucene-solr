...
public class MultiThreadedOCPTest extends AbstractFullDistribZkTestBase {
  ...
  private void testFillWorkQueue() throws Exception {
    try (SolrClient client = createNewSolrClient("", getBaseUrl((HttpSolrClient) clients.get(0)))) {
        ...
        distributedQueue.offer(
            Utils.toJSON(
                Utils.makeMap(
                    ...
                    "A_COLL",
                    QUEUE_OPERATION,
                    MOCK_COLL_TASK.toLower(),
                    ASYNC,
                    Integer.toString(i),

                    // third task waits for a long time, and thus blocks the queue for all other
                    // tasks for A_COLL.
      ...
      // Enqueue a task on another collection not competing with the lock on A_COLL and see that it
      ...
      // We now check that either the B_COLL task has completed before the third (long running) task
      // on A_COLL,
      ...
      // Given the wait delay (500 iterations of 100ms), the task has plenty of time to complete, so
      // this is not expected.
      assertNotNull("Task on  B_COLL did not complete, can't test", taskCollB);
      // We didn't wait for the 3rd A_COLL task to complete (test can run quickly) but if it did, we
      // expect the B_COLL to have finished first.
      assertTrue(
