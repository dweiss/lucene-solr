...
public class DistribCursorPagingTest extends AbstractFullDistribZkTestBase {
  ...
  public void doRandomSortsOnLargeIndex() throws Exception {
    ...
    for (String f : allFieldNames) {
      for (String order : new String[] {" asc", " desc"}) {
        ...
        SentinelIntSet ids =
            assertFullWalkNoDups(
                numInitialDocs, params("q", "*:*", "fl", "id," + f, "rows", rows, "sort", sort));
  ...
  /**
   * Asks the LukeRequestHandler on the control client for a list of the fields in the schema and
   * then prunes that list down to just the fields that can be used for sorting, and returns them as
   * an immutable list in a deterministically random order.
   */
  ...
  /**
   * Given a QueryResponse returned by SolrServer.query, asserts that the numFound on the doc list
  ...
  /**
   * Given a QueryResponse returned by SolrServer.query, asserts that the "id" of the list of
   * documents returned matches the expected list
   *
   * @see org.apache.solr.client.solrj.SolrClient#query
   */
  ...
  /**
   * Given a QueryResponse returned by SolrServer.query, asserts that the response does include
   * {@link CursorMarkParams#CURSOR_MARK_NEXT} key and returns it
   *
   * @see org.apache.solr.client.solrj.SolrClient#query
   */
   ...
   * client as well as a control client. This is because term stat differences in a sharded setup
   * can result in different scores for documents compared to the control index -- which can affect
        ...
        if (ids.exists(id)) {
          ...
          try {
            ...
            throw new AssertionError(
                msg + ", found shard inconsistency that would explain it...", ae);
          }
          rsp = cloudClient.query(params("q", "id:" + id));
          throw new AssertionError(
              msg + ", don't know why; q=id:" + id + " gives: " + rsp.toString());
        }
        ids.put(id);
        assertFalse(
