...
public class OverseerRolesTest extends SolrCloudTestCase {
  ...
  private void waitForNewOverseer(
  ...
  public void testOverseerRole() throws Exception {
    ...
    getOverseerJetty()
        ...
        .offer(
            Utils.toJSON(
                new ZkNodeProps(
                    Overseer.QUEUE_OPERATION, OverseerAction.QUIT.toLower(), "id", leaderId)));
  ...
  public void testDesignatedOverseerRestarts() throws Exception {
    ...
    leaderJetty.start();
    waitForNewOverseer(
        15, overseer1, true); // assert that there is just a single leadership transition
  }
}
