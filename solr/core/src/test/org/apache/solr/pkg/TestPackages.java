...
public class TestPackages extends SolrCloudTestCase {
  ...
  public void testPluginLoading() throws Exception {
    ...
    try {
      ...
      TestDistribPackageStore.assertResponseValues(
          ...
          new GenericSolrRequest(
              ...
              new MapSolrParams(
                  (Map)
                      Utils.makeMap(
                          "collection", COLLECTION_NAME, WT, JAVABIN, "action", "plugins"))),
          Utils.makeMap(
      ...
      executeReq(
          ...
          cluster.getRandomJetty(random()),
          Utils.JSONCONSUMER,
          Utils.makeMap("class", "org.apache.solr.core.RuntimeLibSearchComponent", "Version", "1"));
      ...
      checkAllNodesForFile(
          cluster, FILE3, Utils.makeMap(":files:" + FILE3 + ":name", "runtimelibs_v3.jar"), false);
    } finally {
      cluster.shutdown();
    }
  }
  ...
  private void executeReq(
      String uri, JettySolrRunner jetty, Utils.InputStreamConsumer parser, Map expected)
      throws Exception {
  ...
  public void testAPI() throws Exception {
    ...
    try {
      ...
      TestDistribPackageStore.assertResponseValues(
          ...
          Utils.makeMap(
              ":packages:test_pkg[0]:version", "0.12", ":packages:test_pkg[0]:files[0]", FILE1));
              ...
              new MapWriterMap(
                  (Map)
                      Utils.fromJSON(
                          cluster.getZkClient().getData(SOLR_PKGS_PATH, null, new Stat(), true))),
          Utils.makeMap(
              ":packages:test_pkg[1]:version", "0.13", ":packages:test_pkg[1]:files[0]", FILE3));
              ...
              new MapWriterMap(
                  (Map)
                      Utils.fromJSON(
                          cluster.getZkClient().getData(SOLR_PKGS_PATH, null, new Stat(), true))),
          Utils.makeMap(
              ":packages:test_pkg[0]:version", "0.13", ":packages:test_pkg[0]:files[0]", FILE2));
  ...
  private void verifySchemaComponent(
      SolrClient client, String COLLECTION_NAME, String path, Map expected) throws Exception {
    SolrParams params =
        new MapSolrParams(
            (Map) Utils.makeMap("collection", COLLECTION_NAME, WT, JAVABIN, "meta", "true"));
  ...
  public static void postFileAndWait(
    ...
    TestDistribPackageStore.checkAllNodesForFile(
        cluster, path, Utils.makeMap(":files:" + path + ":sha512", sha512), false);
  }
  ...
  public static ByteBuffer persistZip(String loc, @SuppressWarnings({"rawtypes"}) Class... classes)
      throws IOException {
  ...
  public static ByteBuffer generateZip(@SuppressWarnings({"rawtypes"}) Class... classes)
      throws IOException {
