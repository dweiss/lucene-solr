...
public abstract class AbstractFullDistribZkTestBase extends AbstractDistribZkTestBase {
      ...
      try (ZkStateReader zkStateReader =
        ...
        try {
          ...
          zkStateReader
              ...
              .setData(
                  ZkStateReader.CLUSTER_PROPS,
                  Utils.toJSON(props.plus("urlScheme", "https")),
                  true);
        }
      }
    }
  ...
  protected List<JettySolrRunner> createJettys(int numJettys) throws Exception {
    ...
    List<CollectionAdminRequest> createReplicaRequests =
        Collections.synchronizedList(new ArrayList<>());
    @SuppressWarnings({"rawtypes"})
    List<CollectionAdminRequest> createPullReplicaRequests =
        Collections.synchronizedList(new ArrayList<>());
  ...
  protected void destroyServers() throws Exception {
    ...
    customThreadPool.submit(
        () ->
            Collections.singleton(commonCloudSolrClient).parallelStream()
                .forEach(
                    c -> {
    ...
    customThreadPool.submit(
        () ->
            Collections.singleton(controlClient).parallelStream()
                .forEach(
                    c -> {
                      IOUtils.closeQuietly(c);
                    }));
    ...
    customThreadPool.submit(
        () ->
            Collections.singletonList(controlClientCloud).parallelStream()
                .forEach(
                    c -> {
                      IOUtils.closeQuietly(c);
                    }));
    ...
    customThreadPool.submit(
        () ->
            Collections.singletonList(cloudClient).parallelStream()
                .forEach(
                    c -> {
                      IOUtils.closeQuietly(c);
                    }));
  ...
  protected CollectionAdminResponse createCollection(
    ...
    if (numShards == null) {
      String shardNames =
          (String) collectionProps.get(OverseerCollectionMessageHandler.SHARDS_PROP);
    ...
    if (numNrtReplicas == null) {
      numNrtReplicas =
          (Integer)
              OverseerCollectionMessageHandler.COLLECTION_PROPS_AND_DEFAULTS.get(
