...
public class TestFieldValueFeature extends TestRerankBase {
  ...
  public void before() throws Exception {
    ...
    assertU(
        adoc("id", "1", "title", "w1", "description", "w1", "popularity", "1", "isTrendy", "true"));
    assertU(
        adoc(
            "id",
            "2",
            ...
            "2"));
    assertU(
        adoc("id", "3", "title", "w3", "description", "w3", "popularity", "3", "isTrendy", "true"));
    assertU(
        adoc(
            "id", "4", "title", "w4", "description", "w4", "popularity", "4", "isTrendy", "false"));
    assertU(
        adoc("id", "5", "title", "w5", "description", "w5", "popularity", "5", "isTrendy", "true"));
    assertU(
        adoc(
            "id",
            "6",
    ...
    loadFeature("popularity", FieldValueFeature.class.getName(), "{\"field\":\"popularity\"}");
    ...
    assertJQ(
        ...
        "/response/docs/[0]/=={'[fv]':'"
            + FeatureLoggerTestUtils.toFeatureVector(
                "popularity", Float.toString(FIELD_VALUE_FEATURE_DEFAULT_VAL))
            + "'}");
  }
  ...
  public void testIfADocumentDoesntHaveAFieldASetDefaultValueIsReturned() throws Exception {
    ...
    loadModel(
        "popularity-model42",
        LinearModel.class.getName(),
        new String[] {"popularity42"},
        fstore,
        "{\"weights\":{\"popularity42\":1.0}}");
    ...
    assertJQ(
        "/query" + query.toQueryString(),
        "/response/docs/[0]/=={'[fv]':'"
            + FeatureLoggerTestUtils.toFeatureVector("popularity42", "42.0")
            + "'}");
  }
