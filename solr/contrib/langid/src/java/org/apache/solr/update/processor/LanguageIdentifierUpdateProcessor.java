 ...
 * href="https://lucene.apache.org/solr/guide/detecting-languages-during-indexing.html">Detecting
...
public abstract class LanguageIdentifierUpdateProcessor extends UpdateRequestProcessor
      ...
      if (params.get(MAP_LCMAP) != null) {
        for (String mapping : params.get(MAP_LCMAP).split("[, ]")) {
          ...
          if (keyVal.length == 2) {
            ...
            log.error(
                "Unsupported format for langid.map.lcmap: {}. Skipping this mapping.", mapping);
          }
        }
      }
      ...
      for (String fieldName : allMapFieldsSet) {
        if (doc.containsKey(fieldName)) {
          ...
          if (mapIndividual && mapIndividualFieldsSet.contains(fieldName)) {
            ...
            log.debug(
                "Mapping field {} using individually detected language {}", fieldName, fieldLang);
          } else {
            fieldLang = docLang;
    ...
    if (languages.size() == 0) {
      ...
      if (langWhitelist.isEmpty() || langWhitelist.contains(normalizedLang)) {
        ...
        if (lang.getCertainty() >= threshold) {
          ...
          log.debug(
              "Detected language below threshold {}, using fallback {}", threshold, fallbackLang);
          langStr = fallbackLang;
        }
      } else {
        if (log.isDebugEnabled()) {
  ...
  /**
   * Returns the name of the field to map the current contents into, so that they are properly
   * analyzed. For instance if the currentField is "text" and the code is "en", the new field would
   * by default be "text_en". This method also performs custom regex pattern replace if configured.
   * If enforceSchema=true and the resulting field name doesn't exist, then null is returned.
   *
  ...
  protected String getMappedField(String currentField, String language) {
    ...
    if (enforceSchema && schema.getFieldOrNull(newFieldName) == null) {
      log.warn(
          "Unsuccessful field name mapping from {} to {}, field does not exist and enforceSchema=true; skipping mapping.",
          currentField,
          newFieldName);
      return null;
    } else {
  ...
  /**
   * Returns a reader that streams String content from fields. This is more memory efficient than
