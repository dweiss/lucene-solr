...
public class StreamingTest extends SolrCloudTestCase {
  ...
  public void testUniqueStream() throws Exception {
    ...
    try {
      SolrParams sParams =
          StreamingTest.mapParams("q", "*:*", "fl", "id,a_s,a_i,a_f", "sort", "a_f asc,a_i asc");
  ...
  public void testTrace() throws Exception {
    ...
    try {
      ...
      SolrParams sParamsA =
          mapParams("q", "*:*", "fl", "id,a_s, a_i,a_f", "sort", "a_s asc,a_f   asc");
      CloudSolrStream stream = new CloudSolrStream(zkHost, COLLECTIONORALIAS, sParamsA);
  ...
  public void testZeroReducerStream() throws Exception {
    ...
    try {
      // Test with spaces in the parameter lists.
      SolrParams sParamsA =
          mapParams("q", "blah", "fl", "id,a_s, a_i,  a_f", "sort", "a_s asc  ,  a_f   asc");
      CloudSolrStream stream = new CloudSolrStream(zkHost, COLLECTIONORALIAS, sParamsA);
      ReducerStream rstream =
          new ReducerStream(
              stream,
  ...
  public void testExceptionStream() throws Exception {
    ...
    try {
      ...
      sParamsA =
          mapParams("q", "*:*", "fl", "a_s,a_i,a_f,score", "sort", "a_s asc", "qt", "/export");
      stream = new CloudSolrStream(zkHost, COLLECTIONORALIAS, sParamsA);
  ...
  private void checkSort(JettySolrRunner jetty, String field, String sortDir, String[] fields)
    ...
    //      }
    //    }
    //    SolrParams exportParams = mapParams("q", "*:*", "qt", "/export", "fl", "id," + field,
    // "sort", field + " " + sortDir + ",id asc");
  ...
  private void checkReturnValsForEmpty(String[] fields) throws IOException {
    ...
    SolrParams sParams =
        mapParams("q", "*:*", "qt", "/export", "fl", fl.toString(), "sort", "id asc");
  ...
  public void testTuple() throws Exception {
    ...
    try {
      SolrParams sParams =
          mapParams("q", "*:*", "fl", "id,a_s,a_i,a_f,s_multi,i_multi,f_multi", "sort", "a_s asc");
      CloudSolrStream stream = new CloudSolrStream(zkHost, COLLECTIONORALIAS, sParams);
      stream.setStreamContext(streamContext);
      List<Tuple> tuples = getTuples(stream);
      Tuple tuple = tuples.get(0);
  ...
  private void trySortWithQt(String which) throws Exception {
    ...
    SolrParams sParams =
        mapParams("q", "*:*", "qt", which, "fl", "id,b_sing", "sort", "b_sing asc,id asc");
    StreamContext streamContext = new StreamContext();
    SolrClientCache solrClientCache = new SolrClientCache();
    streamContext.setSolrClientCache(solrClientCache);
    CloudSolrStream stream = new CloudSolrStream(zkHost, COLLECTIONORALIAS, sParams);
    ...
    try {
      ...
      // Basic CloudSolrStream Test bools desc
      sParams =
          mapParams("q", "*:*", "qt", which, "fl", "id,b_sing", "sort", "b_sing desc,id desc");
      stream = new CloudSolrStream(zkHost, COLLECTIONORALIAS, sParams);
      stream.setStreamContext(streamContext);
      tuples = getTuples(stream);
      ...
      sParams =
          mapParams("q", "*:*", "qt", which, "fl", "id,dt_sing", "sort", "dt_sing desc,id asc");
      stream = new CloudSolrStream(zkHost, COLLECTIONORALIAS, sParams);
      stream.setStreamContext(streamContext);
      tuples = getTuples(stream);
      ...
      sParams =
          mapParams("q", "*:*", "qt", which, "fl", "id,dt_sing", "sort", "dt_sing asc,id desc");
      stream = new CloudSolrStream(zkHost, COLLECTIONORALIAS, sParams);
      stream.setStreamContext(streamContext);
      tuples = getTuples(stream);
  ...
  public void testTupleStreamGetShardsPreference() throws Exception {
    ...
    streamContext.setRequestParams(
        mapParams(
            ShardParams.SHARDS_PREFERENCE, ShardParams.SHARDS_PREFERENCE_REPLICA_TYPE + ":nrt"));
  ...
  protected boolean assertMaps(@SuppressWarnings({"rawtypes"}) List<Map> maps, int... ids)
      throws Exception {
