...
/**
 * The executor function wraps a stream with Tuples containing Streaming Expressions and executes
 * them in parallel. Sample syntax:
 *
 * <p>executor(thread=10, topic(storedExpressions, q="*:*", fl="expr_s, id", id="topic1"))
 *
 * <p>The Streaming Expression to execute is taken from the expr field in the Tuples.
 *
 * @since 6.3.0
 */
public class ExecutorStream extends TupleStream implements Expressible {
  ...
  private void init(TupleStream tupleStream, int threads, StreamFactory factory)
      throws IOException {
  ...
  public void open() throws IOException {
    executorService =
        ExecutorUtil.newMDCAwareFixedThreadPool(
            threads, new SolrNamedThreadFactory("ExecutorStream"));
    stream.open();
  }
  ...
  public static class StreamTask implements Runnable {
    ...
    public StreamTask(
        @SuppressWarnings({"rawtypes"}) ArrayBlockingQueue queue,
