...
/**
 * Stores the mapping of v1 API parameters to v2 API parameters for the collection API and the
 * configset API.
 */
public class CollectionApiMapping {
  ...
  public enum Meta implements CommandMeta {
    ...
    SYNC_SHARD(
        PER_COLLECTION_PER_SHARD_COMMANDS, POST, CollectionAction.SYNCSHARD, "synch-shard", null),
    ...
    DELETE_REPLICA_PROPERTY(
        PER_COLLECTION, POST, DELETEREPLICAPROP, "delete-replica-property", null),
    ...
    // mapping of old prefix to new for instance properties.a=val can be substituted with
    // property:{a:val}
    ...
    // lame... the Maps aren't typed simply because callers want to use Utils.makeMap which yields
    // object vals
    ...
    Meta(
      ...
      this.prefixParamsToAttrs =
          prefixParamsToAttrs == null
              ? Collections.emptyMap()
              : Collections.unmodifiableMap(prefixParamsToAttrs);
    }

    private static Map<String, String> reverseMap(
        Map<String, String> input) { // swap keys and values
    ...
    public Object getReverseParamSubstitute(String param) { // input is v1
      for (Map.Entry<String, String> e : prefixParamsToAttrs.entrySet()) {
        ...
        if (param.startsWith(prefixV1)) {
          return new Pair<>(
              prefixV2.substring(0, prefixV2.length() - 1), param.substring(prefixV1.length()));
        }
      }
      return paramsToAttrs.getOrDefault(param, param);
    }
