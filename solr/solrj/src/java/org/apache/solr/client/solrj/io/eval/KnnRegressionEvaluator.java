...
public class KnnRegressionEvaluator extends RecursiveObjectEvaluator implements ManyValueWorker {
  ...
  public KnnRegressionEvaluator(StreamExpression expression, StreamFactory factory)
      throws IOException {
  ...
  public Object doWork(Object... values) throws IOException {
    ...
    if (values[0] instanceof Matrix) {
      ...
      throw new IOException(
          "The first parameter for knnRegress should be the observation vector or matrix.");
    }
    ...
    return new KnnRegressionTuple(
        observations, outcomeData, k, distanceMeasure, map, scale, robust, bivariate);
  }
  ...
  public static class KnnRegressionTuple extends Tuple {
    ...
    public double[] scale(double[] predictors) {
      ...
      for (int i = 0; i < featureRows.length; i++) {
        ...
        combinedFeatureRow[featureRow.length] =
            predictors[i]; // Add the last feature from the predictor
      ...
      Array2DRowRealMatrix scaledObservationsMatrix =
          (Array2DRowRealMatrix) scaledFeatureMatrix.transpose();
    ...
    public Matrix scale(Matrix predictors) {
      ...
      Array2DRowRealMatrix observationTransposed =
          (Array2DRowRealMatrix) observationMatrix.transpose();
      ...
      for (int i = 0; i < observationFeatureRows.length; i++) {
        ...
        double[] combinedFeatureRow =
            new double[observationFeatureRow.length + predictorFeatureRow.length];
        System.arraycopy(
            observationFeatureRow, 0, combinedFeatureRow, 0, observationFeatureRow.length);
        System.arraycopy(
            predictorFeatureRow,
            0,
            combinedFeatureRow,
            observationFeatureRow.length,
      ...
      Array2DRowRealMatrix scaledTransposedPredictorMatrix =
          (Array2DRowRealMatrix) scaledPredictorMatrix.transpose();
