...
public class StreamFactory implements Serializable {
  ...
  public StreamFactory withFunctionName(
      String functionName, Supplier<Class<? extends Expressible>> clazz) {
    this.functionNames.put(functionName, clazz);
    return this;
  }
  ...
  public List<String> getValueOperands(StreamExpression expression) {
    return getOperandsOfType(expression, StreamExpressionValue.class).stream()
        .map(item -> ((StreamExpressionValue) item).getValue())
  ...
  public List<StreamExpressionNamedParameter> getNamedOperands(StreamExpression expression) {
    ...
    for (StreamExpressionParameter parameter :
        getOperandsOfType(expression, StreamExpressionNamedParameter.class)) {
  ...
  public List<StreamExpression> getExpressionOperands(StreamExpression expression) {
    ...
    for (StreamExpressionParameter parameter :
        getOperandsOfType(expression, StreamExpression.class)) {
      namedParameters.add((StreamExpression) parameter);
    }
    return namedParameters;
  }

  public List<StreamExpression> getExpressionOperands(
      StreamExpression expression, String functionName) {
    List<StreamExpression> namedParameters = new ArrayList<>();
    for (StreamExpressionParameter parameter :
        getOperandsOfType(expression, StreamExpression.class)) {
  ...
  public List<StreamExpressionParameter> getOperandsOfType(
      StreamExpression expression, @SuppressWarnings({"rawtypes"}) Class... clazzes) {
  ...
  @SuppressWarnings({"unchecked"})
  public List<StreamExpression> getExpressionOperandsRepresentingTypes(
      StreamExpression expression, @SuppressWarnings({"rawtypes"}) Class... clazzes) {
    ...
    for (StreamExpression streamExpression : allStreamExpressions) {
      Supplier<Class<? extends Expressible>> classSupplier =
          functionNames.get(streamExpression.getFunctionName());
  ...
  @SuppressWarnings({"unchecked"})
  public boolean doesRepresentTypes(
      StreamExpression expression, @SuppressWarnings({"rawtypes"}) Class... clazzes) {
    Supplier<Class<? extends Expressible>> classSupplier =
        functionNames.get(expression.getFunctionName());
    if (classSupplier != null) {
      for (@SuppressWarnings({"rawtypes"}) Class clazz : clazzes) {
        if (!clazz.isAssignableFrom(classSupplier.get())) {
          return false;
  ...
  public int getIntOperand(StreamExpression expression, String paramName, Integer defaultValue)
      throws IOException {
    StreamExpressionNamedParameter param = getNamedOperand(expression, paramName);

    if (null == param
        || null == param.getParameter()
        || !(param.getParameter() instanceof StreamExpressionValue)) {
    ...
    try {
      ...
      throw new IOException(
          String.format(
              Locale.ROOT,
              "invalid expression %s - %s '%s' is not a valid integer.",
              expression,
              paramName,
              nStr));
    }
  ...
  public boolean getBooleanOperand(
      StreamExpression expression, String paramName, Boolean defaultValue) throws IOException {
    StreamExpressionNamedParameter param = getNamedOperand(expression, paramName);
    ...
    if (classSupplier != null) {
      ...
      if (Expressible.class.isAssignableFrom(clazz) && TupleStream.class.isAssignableFrom(clazz)) {
        return (TupleStream)
            createInstance(
                clazz,
                new Class[] {StreamExpression.class, StreamFactory.class},
  ...
  public Metric constructMetric(StreamExpression expression) throws IOException {
    ...
    throw new IOException(
        String.format(
            Locale.ROOT,
            "Invalid metric expression %s - function '%s' is unknown (not mapped to a valid Metric)",
            expression,
            expression.getFunctionName()));
  }

  @SuppressWarnings({"unchecked", "rawtypes"})
  public StreamComparator constructComparator(
      String comparatorString, @SuppressWarnings({"rawtypes"}) Class comparatorType)
      throws IOException {
    if (comparatorString.contains(",")) {
      ...
      if (parts.length < 3) {
        throw new IOException(
            String.format(
                Locale.ROOT,
                "Invalid comparator expression %s - expecting 'left=right order'",
                comparatorString));
      }
      ...
      if (2 != parts.length) {
        throw new IOException(
            String.format(
                Locale.ROOT,
                "Invalid comparator expression %s - expecting 'field order'",
                comparatorString));
      }
  ...
  @SuppressWarnings({"unchecked", "rawtypes"})
  public StreamEqualitor constructEqualitor(String equalitorString, Class equalitorType)
      throws IOException {
      ...
      if (equalitorString.contains("=")) {
        ...
        if (2 != parts.length) {
          throw new IOException(
              String.format(
                  Locale.ROOT,
                  "Invalid equalitor expression %s - expecting fieldName=fieldName",
                  equalitorString));
        }
  ...
  public StreamOperation constructOperation(StreamExpression expression) throws IOException {
    ...
    if (classSupplier != null) {
      Class<? extends Expressible> clazz = classSupplier.get();
      if (Expressible.class.isAssignableFrom(clazz)
          && StreamOperation.class.isAssignableFrom(clazz)) {
  ...
  public org.apache.solr.client.solrj.io.eval.StreamEvaluator constructEvaluator(
      String expressionClause) throws IOException {
    return constructEvaluator(StreamExpressionParser.parse(expressionClause));
  }

  @SuppressWarnings({"rawtypes"})
  public org.apache.solr.client.solrj.io.eval.StreamEvaluator constructEvaluator(
      StreamExpression expression) throws IOException {
    String function = expression.getFunctionName();
    Supplier<Class<? extends Expressible>> classSupplier = functionNames.get(function);
    ...
    if (classSupplier != null) {
      Class<? extends Expressible> clazz = classSupplier.get();
      if (Expressible.class.isAssignableFrom(clazz)
          && StreamEvaluator.class.isAssignableFrom(clazz)) {
        return (org.apache.solr.client.solrj.io.eval.StreamEvaluator)
            createInstance(
                clazz,
                new Class[] {StreamExpression.class, StreamFactory.class},
                new Object[] {expression, this});
  ...
  public <T> T createInstance(Class<T> clazz, Class<?>[] paramTypes, Object[] params)
      throws IOException {
    Constructor<T> ctor;
    try {
      ...
      if (null != e.getMessage()) {
        throw new IOException(
            String.format(
                Locale.ROOT,
                "Unable to construct instance of %s caused by %s",
                clazz.getName(),
                e.getMessage()),
            e);
      } else {
        throw new IOException(
            String.format(Locale.ROOT, "Unable to construct instance of %s", clazz.getName()), e);
      }
    }
  }
