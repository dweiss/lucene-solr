...
public class CartesianProductStream extends TupleStream implements Expressible {
  ...
  public CartesianProductStream(StreamExpression expression, StreamFactory factory)
    ...
    List<StreamExpressionParameter> evaluateAsExpressions =
        factory.getOperandsOfType(expression, StreamExpressionValue.class);
    StreamExpressionNamedParameter orderByExpression =
        factory.getNamedOperand(expression, "productSort");
    // validate expression contains only what we want.
    if (expression.getParameters().size()
        != streamExpressions.size()
            + evaluateAsExpressions.size()
            + (null == orderByExpression ? 0 : 1)) {
      throw new IOException(
          String.format(
              Locale.ROOT,
              "Invalid %s expression %s - unknown operands found",
              functionName,
              expression));
    }
    ...
    orderBy =
        null == orderByExpression
            ...
            : factory.constructComparator(
                ((StreamExpressionValue) orderByExpression.getParameter()).getValue(),
    ...
    for (StreamExpressionParameter evaluateAsExpression : evaluateAsExpressions) {
      ...
      if (fullString.toLowerCase(Locale.ROOT).contains(" as ")) {
        String[] parts =
            fullString.split("(?i) as "); // ensure we are splitting in a case-insensitive way
        if (2 != parts.length) {
          throw new IOException(
              String.format(
                  Locale.ROOT,
      ...
      if (null == evaluator || null == asNamePart) {
        throw new IOException(
            String.format(
                Locale.ROOT,
                "Invalid %s expression %s - failed to parse evaluator '%s'",
                functionName,
                expression,
                originalFullString));
      }
    ...
    for (NamedEvaluator evaluator : evaluators) {
      expression.addParameter(
          String.format(
              Locale.ROOT,
              "%s as %s",
              evaluator.getEvaluator().toExpression(factory),
              evaluator.getName()));
    }

    if (orderBy != null) {
      expression.addParameter(
          new StreamExpressionNamedParameter("productSort", orderBy.toExpression(factory)));
    }
  ...
  private LinkedList<Tuple> generateTupleList(Tuple original) throws IOException {
    ...
    int[] workingIndexes =
        new int[evaluators.size()]; // java language spec ensures all values are 0
    do {
  ...
  private boolean iterate(
    ...
    // this assumes evaluators and indexes are the same length, which is ok cause we created it so
    // we know it is
