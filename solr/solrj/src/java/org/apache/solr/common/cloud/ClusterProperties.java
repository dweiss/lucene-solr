 ...
 *
 * <p>Note that all methods on this class make calls to ZK on every invocation. For read-only
 * eventually-consistent uses, clients should instead call {@link
...
public class ClusterProperties {
   ...
   * 2. Custom property: it can be created by third-party extensions and should start with prefix
   * <b>"ext."</b> and it's recommended to also add prefix of plugin name or company name or package
  ...
  private void validatePropertyName(String propertyName) {
    if (!ZkStateReader.KNOWN_CLUSTER_PROPS.contains(propertyName)
      ...
      throw new SolrException(
          ...
          "Not a known cluster property or starts with prefix "
              + EXT_PROPRTTY_PREFIX
              + ", propertyName: "
              + propertyName);
    }
